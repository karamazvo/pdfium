...
int ReentrantToInt32Helper(v8::Isolate* isolate, v8::Local<v8::Value> value) {
  if (value.IsEmpty()) {
    return 0;
  }
...
float ReentrantToFloatHelper(v8::Isolate* isolate, v8::Local<v8::Value> value) {
  return static_cast<float>(ReentrantToDoubleHelper(isolate, value));
}
...
v8::Local<v8::Array> ReentrantToArrayHelper(v8::Isolate* isolate,
  ...
  v8::TryCatch squash_exceptions(isolate);
  v8::Local<v8::Context> context = isolate->GetCurrentContext();
  return v8::Local<v8::Array>::Cast(value->ToObject(context).ToLocalChecked());
}
...
v8::Local<v8::Value> ReentrantGetObjectPropertyHelper(
  ...
  if (!obj->Get(isolate->GetCurrentContext(),
...
std::vector<WideString> ReentrantGetObjectPropertyNamesHelper(
  ...
  v8::Local<v8::Context> context = isolate->GetCurrentContext();
  if (!obj->GetPropertyNames(context).ToLocal(&val)) {
    return std::vector<WideString>();
  }
...
bool ReentrantHasObjectOwnPropertyHelper(v8::Isolate* isolate,
  ...
  return obj->HasRealNamedProperty(pContext, hKey).FromJust();
}
...
bool ReentrantSetObjectOwnPropertyHelper(v8::Isolate* isolate,
                                         ...
                                         ByteStringView bsUTF8PropertyName,
                                         v8::Local<v8::Value> value) {
  if (obj.IsEmpty() || value.IsEmpty()) {
    return false;
  }

  v8::TryCatch squash_exceptions(isolate);
  v8::Local<v8::String> name = NewStringHelper(isolate, bsUTF8PropertyName);
  return obj->DefineOwnProperty(isolate->GetCurrentContext(), name, value)
      .FromMaybe(false);
}
...
bool ReentrantPutObjectPropertyHelper(v8::Isolate* isolate,
  ...
  if (obj.IsEmpty() || pPut.IsEmpty()) {
    return false;
  }

  v8::TryCatch squash_exceptions(isolate);
  v8::Local<v8::String> name = NewStringHelper(isolate, bsUTF8PropertyName);
  v8::Maybe<bool> result = obj->Set(isolate->GetCurrentContext(), name, pPut);
...
void ReentrantDeleteObjectPropertyHelper(v8::Isolate* isolate,
                                         ...
                                         ByteStringView bsUTF8PropertyName) {
  v8::TryCatch squash_exceptions(isolate);
  obj->Delete(isolate->GetCurrentContext(),
              fxv8::NewStringHelper(isolate, bsUTF8PropertyName))
      .FromJust();
}
...
bool ReentrantPutArrayElementHelper(v8::Isolate* isolate,
  ...
  v8::Maybe<bool> result =
      array->Set(isolate->GetCurrentContext(),
                 pdfium::checked_cast<uint32_t>(index), value);
  return result.IsJust() && result.FromJust();
}
