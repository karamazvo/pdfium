...
void CJS_EventContext::OnDoc_Open(const WideString& strTargetName) {
  ...
  str_target_name_ = strTargetName;
}
...
void CJS_EventContext::OnField_MouseEnter(bool bModifier,
  ...
  modifier_ = bModifier;
  shift_ = bShift;
  str_target_name_ = pTarget->GetFullName();
}
...
void CJS_EventContext::OnField_Keystroke(WideString* strChange,
  ...
  wide_str_change_ex_ = strChangeEx;
  key_down_ = KeyDown;
  modifier_ = bModifier;
  isel_end_ = pSelEnd;
  isel_start_ = pSelStart;
  shift_ = bShift;
  str_target_name_ = pTarget->GetFullName();
  value_ = pValue;
...
void CJS_EventContext::OnField_Calculate(CPDF_FormField* pSource,
  ...
  if (pSource)
    str_source_name_ = pSource->GetFullName();
  str_target_name_ = pTarget->GetFullName();
  value_ = pValue;
  pb_rc_ = pRc;
}
...
void CJS_EventContext::Initialize(Kind kind) {
  kind_ = kind;
  str_target_name_.clear();
  str_source_name_.clear();
  wide_str_change_ = nullptr;
  wide_str_change_du_.clear();
  wide_str_change_ex_.clear();
  commit_key_ = -1;
  key_down_ = false;
  modifier_ = false;
  shift_ = false;
...
WideString& CJS_EventContext::Change() {
  return wide_str_change_ ? *wide_str_change_ : wide_str_change_du_;
}
...
int CJS_EventContext::SelEnd() const {
  return isel_end_ ? *isel_end_ : sel_end_du_;
}

int CJS_EventContext::SelStart() const {
  return isel_start_ ? *isel_start_ : sel_start_du_;
}

void CJS_EventContext::SetSelEnd(int value) {
  int& target = isel_end_ ? *isel_end_ : sel_end_du_;
  target = value;
}

void CJS_EventContext::SetSelStart(int value) {
  int& target = isel_start_ ? *isel_start_ : sel_start_du_;
  target = value;
}
