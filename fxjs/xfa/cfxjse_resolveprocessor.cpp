...
bool CFXJSE_ResolveProcessor::Resolve(v8::Isolate* pIsolate, NodeData& rnd) {
  ...
  if (!rnd.cur_object_->IsNode()) {
    if (rnd.dw_styles_ & XFA_ResolveFlag::kAttributes) {
      return ResolveForAttributeRs(rnd.cur_object_, &rnd.result_,
                                   rnd.ws_name_.AsStringView());
    }
    return false;
  }
  if (rnd.dw_styles_ & XFA_ResolveFlag::kAnyChild) {
    return ResolveAnyChild(pIsolate, rnd);
  }

  if (rnd.ws_name_.GetLength()) {
    wchar_t wch = rnd.ws_name_[0];
    switch (wch) {
      case '$':
  ...
  if (rnd.uhash_name_ == XFA_HASHCODE_This && rnd.level_ == 0) {
  ...
  if (rnd.cur_object_->GetElementType() == XFA_Element::Xfa) {
    CXFA_Object* pObjNode =
        engine_->GetDocument()->GetXFAObject(rnd.uhash_name_);
    if (pObjNode) {
      rnd.result_.objects.emplace_back(pObjNode);
    } else if (rnd.uhash_name_ == XFA_HASHCODE_Xfa) {
      rnd.result_.objects.emplace_back(rnd.cur_object_);
    } else if ((rnd.dw_styles_ & XFA_ResolveFlag::kAttributes) &&
               ResolveForAttributeRs(rnd.cur_object_, &rnd.result_,
                                     rnd.ws_name_.AsStringView())) {
      return true;
    }
    if (!rnd.result_.objects.empty()) {
      FilterCondition(pIsolate, rnd.ws_condition_, &rnd);
    }

    return !rnd.result_.objects.empty();
  }
  if (!ResolveNormal(pIsolate, rnd) && rnd.uhash_name_ == XFA_HASHCODE_Xfa) {
...
bool CFXJSE_ResolveProcessor::ResolveAnyChild(v8::Isolate* pIsolate,
  ...
  WideStringView wsName = rnd.ws_name_.AsStringView();
  WideString wsCondition = rnd.ws_condition_;
...
bool CFXJSE_ResolveProcessor::ResolveDollar(v8::Isolate* pIsolate,
                                            NodeData& rnd) {
  WideString wsName = rnd.ws_name_;
  WideString wsCondition = rnd.ws_condition_;
...
bool CFXJSE_ResolveProcessor::ResolveExcalmatory(v8::Isolate* pIsolate,
  ...
  rndFind.ws_name_ = rnd.ws_name_.Last(rnd.ws_name_.GetLength() - 1);
  rndFind.uhash_name_ = static_cast<XFA_HashCode>(
      FX_HashCode_GetW(rndFind.ws_name_.AsStringView()));
  rndFind.level_ = rnd.level_ + 1;
  rndFind.dw_styles_ = XFA_ResolveFlag::kChildren;
  rndFind.ws_condition_ = rnd.ws_condition_;
...
bool CFXJSE_ResolveProcessor::ResolveNumberSign(v8::Isolate* pIsolate,
                                                NodeData& rnd) {
  WideString wsName = rnd.ws_name_.Last(rnd.ws_name_.GetLength() - 1);
  WideString wsCondition = rnd.ws_condition_;
  ...
  NodeData rndFind;
  rndFind.level_ = rnd.level_ + 1;
  rndFind.dw_styles_ = rnd.dw_styles_;
  rndFind.dw_styles_ |= XFA_ResolveFlag::kTagName;
  rndFind.dw_styles_.Clear(XFA_ResolveFlag::kAttributes);
  rndFind.ws_name_ = std::move(wsName);
  rndFind.uhash_name_ = static_cast<XFA_HashCode>(
      FX_HashCode_GetW(rndFind.ws_name_.AsStringView()));
  rndFind.ws_condition_ = wsCondition;
...
bool CFXJSE_ResolveProcessor::ResolveNormal(v8::Isolate* pIsolate,
  ...
  Mask<XFA_ResolveFlag> dwStyles = rnd.dw_styles_;
  WideString& wsName = rnd.ws_name_;
  XFA_HashCode uNameHash = rnd.uhash_name_;
  WideString& wsCondition = rnd.ws_condition_;

  NodeData rndFind;
  rndFind.ws_name_ = rnd.ws_name_;
  rndFind.ws_condition_ = rnd.ws_condition_;
  rndFind.level_ = rnd.level_ + 1;
  rndFind.uhash_name_ = uNameHash;
  ...
  if ((dwStyles & XFA_ResolveFlag::kProperties) && pVariablesNode) {
    if (pVariablesNode->GetClassHashCode() == uNameHash) {
      ...
      WideString wsSaveCondition = std::move(rndFind.ws_condition_);
      ResolveNormal(pIsolate, rndFind);
      rndFind.ws_condition_ = std::move(wsSaveCondition);
      rnd.result_.objects.insert(rnd.result_.objects.end(),
                                 rndFind.result_.objects.begin(),
                                 rndFind.result_.objects.end());
  ...
  if (dwStyles & XFA_ResolveFlag::kSiblings) {
    ...
    rndFind.dw_styles_ = dwSubStyles;
    while (child) {
      if (child == curNode) {
        if (dwStyles & XFA_ResolveFlag::kTagName) {
          if (uCurClassHash == uNameHash)
      ...
      if (bInnerSearch) {
        rndFind.cur_object_ = child;
        WideString wsOriginCondition = std::move(rndFind.ws_condition_);
        Mask<XFA_ResolveFlag> dwOriginStyle = rndFind.dw_styles_;
        rndFind.dw_styles_ = dwOriginStyle | XFA_ResolveFlag::kALL;
        ResolveNormal(pIsolate, rndFind);
        rndFind.dw_styles_ = dwOriginStyle;
        rndFind.ws_condition_ = std::move(wsOriginCondition);
        rnd.result_.objects.insert(rnd.result_.objects.end(),
                                   rndFind.result_.objects.begin(),
                                   rndFind.result_.objects.end());
        rndFind.result_.objects.clear();
...
int32_t CFXJSE_ResolveProcessor::GetFilter(WideStringView wsExpression,
  ...
  int32_t nConditionCount = 0;
  {
    ...
    while (nStart < iLength) {
      ...
      if (wCur == '.') {
        if (nNameCount == 0) {
          rnd.dw_styles_ |= XFA_ResolveFlag::kAnyChild;
          continue;
        }
        if (wPrev == '\\') {
...
void CFXJSE_ResolveProcessor::ConditionArray(size_t iCurIndex,
  ...
  if (bAll) {
    if (pRnd->dw_styles_ & XFA_ResolveFlag::kCreateNode) {
      if (pRnd->dw_styles_ & XFA_ResolveFlag::kBind) {
    ...
    } else if (pRnd->dw_styles_ & XFA_ResolveFlag::kBindNew) {
...
void CFXJSE_ResolveProcessor::FilterCondition(v8::Isolate* pIsolate,
  ...
  if (nLen == 0) {
    if (pRnd->dw_styles_ & XFA_ResolveFlag::kALL) {
      return;
    }
    if (iFoundCount == 1)
      return;
