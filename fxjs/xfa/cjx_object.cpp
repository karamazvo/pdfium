...
void CJX_Object::className(v8::Isolate* isolate,
  ...
  *pValue = fxv8::NewStringHelper(isolate, GetXFAObject()->GetClassName());
}
...
void CJX_Object::ScriptAttributeString(v8::Isolate* isolate,
  ...
  WideString wsValue = fxv8::ReentrantToWideStringHelper(isolate, *pValue);
...
void CJX_Object::ScriptAttributeBool(v8::Isolate* isolate,
                                     ...
                                     XFA_Attribute eAttribute) {
  if (bSetting) {
    SetBoolean(eAttribute, fxv8::ReentrantToBooleanHelper(isolate, *pValue),
               true);
    return;
  }
  *pValue = fxv8::NewStringHelper(isolate, GetBoolean(eAttribute) ? "1" : "0");
}
...
void CJX_Object::ScriptAttributeInteger(v8::Isolate* isolate,
                                        ...
                                        XFA_Attribute eAttribute) {
  if (bSetting) {
    SetInteger(eAttribute, fxv8::ReentrantToInt32Helper(isolate, *pValue),
               true);
    return;
  }
  *pValue = fxv8::NewNumberHelper(isolate, GetInteger(eAttribute));
}
...
void CJX_Object::ScriptSomFontColor(v8::Isolate* isolate,
  ...
  if (bSetting) {
    auto [r, g, b] =
        StrToRGB(fxv8::ReentrantToWideStringHelper(isolate, *pValue));
    FX_ARGB color = ArgbEncode(0xff, r, g, b);
    font->SetColor(color);
    return;
  }

  *pValue = ColorToV8String(isolate, font->GetColor());
}
...
void CJX_Object::ScriptSomFillColor(v8::Isolate* isolate,
  ...
  *pValue = ColorToV8String(isolate, borderfill->GetFillColor());
}
...
void CJX_Object::ScriptSomBorderColor(v8::Isolate* isolate,
  ...
  *pValue = ColorToV8String(isolate, color);
}
...
void CJX_Object::ScriptSomBorderWidth(v8::Isolate* isolate,
  ...
  WideString wsThickness = fxv8::ReentrantToWideStringHelper(isolate, *pValue);
...
void CJX_Object::ScriptSomMessage(v8::Isolate* isolate,
      ...
      switch (iMessageType) {
        case SOMMessageType::kValidationMessage:
          validate->SetScriptMessageText(
              fxv8::ReentrantToWideStringHelper(isolate, *pValue));
          break;
  ...
  *pValue = fxv8::NewStringHelper(isolate, wsMessage.ToUTF8().AsStringView());
}
...
void CJX_Object::ScriptSomValidationMessage(v8::Isolate* isolate,
                                            ...
                                            bool bSetting,
                                            XFA_Attribute eAttribute) {
  ScriptSomMessage(isolate, pValue, bSetting,
...
void CJX_Object::ScriptSomDefaultValue(v8::Isolate* isolate,
  ...
  if (eType == XFA_Element::Field) {
    static_cast<CJX_Field*>(this)->defaultValue(isolate, pValue, bSetting,
                                                XFA_Attribute::Unknown);
    return;
  }
  if (eType == XFA_Element::Draw) {
    static_cast<CJX_Draw*>(this)->defaultValue(isolate, pValue, bSetting,
                                               XFA_Attribute::Unknown);
    return;
  }
  if (eType == XFA_Element::Boolean) {
    static_cast<CJX_Boolean*>(this)->defaultValue(isolate, pValue, bSetting,
                                                  XFA_Attribute::Unknown);
    return;
  }
  ...
  if (bSetting) {
    ...
    if (pValue && !(pValue->IsEmpty() || fxv8::IsNull(*pValue) ||
      ...
      wsNewValue = fxv8::ReentrantToWideStringHelper(isolate, *pValue);
    }
  ...
  if (content.IsEmpty() && eType != XFA_Element::Text &&
    ...
    *pValue = fxv8::NewNumberHelper(isolate, FXSYS_wtoi(content.c_str()));
  } else if (eType == XFA_Element::Float || eType == XFA_Element::Decimal) {
    CFGAS_Decimal decimal(content.AsStringView());
    *pValue = fxv8::NewNumberHelper(isolate, decimal.ToFloat());
  } else {
    *pValue = fxv8::NewStringHelper(isolate, content.ToUTF8().AsStringView());
  }
}
...
void CJX_Object::ScriptSomMandatory(v8::Isolate* isolate,
  ...
  if (bSetting) {
    validate->SetNullTest(fxv8::ReentrantToWideStringHelper(isolate, *pValue));
    return;
  }
...
void CJX_Object::ScriptSomInstanceIndex(v8::Isolate* isolate,
  ...
  int32_t iTo = fxv8::ReentrantToInt32Helper(isolate, *pValue);
