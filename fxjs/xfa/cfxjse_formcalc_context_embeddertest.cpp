...
TEST_F(CFXJSEFormCalcContextEmbedderTest, SetXFAEventChange) {
  ...
  EXPECT_EQ(L"changed", params.ws_change_);
}
...
TEST_F(CFXJSEFormCalcContextEmbedderTest, SetXFAEventFullTextFails) {
  ...
  CXFA_EventParam params(XFA_EVENT_Unknown);
  params.ws_full_text_ = L"Original Full Text";
  ...
  EXPECT_TRUE(Execute(test));
  EXPECT_EQ(L"Original Full Text", params.ws_full_text_);
}
...
TEST_F(CFXJSEFormCalcContextEmbedderTest, ComplexTextChangeEvent) {
  ...
  CXFA_EventParam params(XFA_EVENT_Unknown);
  params.ws_change_ = L"g";
  params.ws_prev_text_ = L"abcd";
  params.sel_start_ = 1;
  params.sel_end_ = 3;
  ...
  EXPECT_EQ(L"abcd", params.ws_prev_text_);
  EXPECT_EQ(L"agd", params.GetNewText());
  EXPECT_EQ(L"g", params.ws_change_);
  EXPECT_EQ(1, params.sel_start_);
  ...
  EXPECT_EQ(L"abcd", params.ws_prev_text_);
  EXPECT_EQ(L"xyz", params.ws_change_);
