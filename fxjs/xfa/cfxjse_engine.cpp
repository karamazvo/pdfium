...
CFXJSE_Engine::ResolveObjectsWithBindNode(CXFA_Object* refObject,
  ...
  while (true) {
    ...
    if (bNextCreate) {
      ...
      if (node_helper_->CreateNode(rndFind.ws_name_, rndFind.ws_condition_,
    ...
    while (i < nNodes) {
      ...
      if (((dwStyles & XFA_ResolveFlag::kBind) ||
        ...
        i = resolve_processor_->IndexForDataBind(rndBind.ws_condition_, nNodes);
        bDataBind = true;
      }
  ...
  if ((dwStyles & XFA_ResolveFlag::kCreateNode) ||
    ...
    if (node_helper_->create_parent_) {
      ...
      node_helper_->CreateNodeForCondition(rndFind.ws_condition_);
