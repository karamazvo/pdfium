...
bool CBC_ASCIIEncoder::Encode(CBC_EncoderContext* context) {
  size_t n = DetermineConsecutiveDigitCount(context->msg_, context->os_);
  if (n >= 2) {
    std::optional<wchar_t> code = EncodeASCIIDigits(
        context->msg_[context->os_], context->msg_[context->os_ + 1]);
    if (!code.has_value())
      return false;
  ...
  CBC_HighLevelEncoder::Encoding newMode = CBC_HighLevelEncoder::LookAheadTest(
      context->msg_, context->os_, GetEncodingMode());
