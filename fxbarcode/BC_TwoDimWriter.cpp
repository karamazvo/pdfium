...
bool CBC_TwoDimWriter::RenderResult(pdfium::span<const uint8_t> code,
  ...
  nput_width_ = codeWidth;
  nput_height_ = codeHeight;
  int32_t tempWidth = nput_width_ + 2;
  int32_t tempHeight = nput_height_ + 2;
  ...
  output_ = std::make_unique<CBC_CommonBitMatrix>(nput_width_, nput_height_);
  for (int32_t y = 0; y < nput_height_; ++y) {
    for (int32_t x = 0; x < nput_width_; ++x) {
      if (code[x + y * nput_width_] == 1) {
        output_->Set(x, y);
      }
    }
  }
