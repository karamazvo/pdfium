...
std::vector<uint8_t> EncodeBGRPNG(pdfium::span<const uint8_t> bgr_input,
  ...
  CHECK_LE(expected_minimum_row_byte_width.ValueOrDie(), row_byte_width);
  ...
  CHECK_NE(0, intermediate_bgra_buf.size());  // 0 means conversion problems.
  {
    ...
    for (size_t y = 0; y < pdfium::checked_cast<size_t>(height); y++) {
      for (size_t x = 0; x < pdfium::checked_cast<size_t>(width); x++) {
        ...
        pdfium::span<uint8_t> dst_pixel = dst.subspan(x * 4).first(4u);
        pdfium::span<const uint8_t> src_pixel = src.subspan(x * 3).first(3u);

        fxcrt::spancpy(dst_pixel.first(3u), src_pixel);  // Copy BGR channels.
        dst_pixel[3] = 0xFF;  // Set alpha channel to "opaque".
      }
      src = src.subspan(row_byte_width);
