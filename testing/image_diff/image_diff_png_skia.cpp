...
std::vector<uint8_t> EncodeBGRPNG(pdfium::span<const uint8_t> bgr_input,
  ...
  // Crude integer overflow protection.  All test images are expected to be
  // smaller than this.
  CHECK_LE(width, 10000u);
  CHECK_LE(height, 10000u);

  CHECK_LE(width * 3, row_byte_width);
  CHECK_LE(row_byte_width * height, bgr_input.size());

  std::vector<uint8_t> intermediate_bgra_buf;
  size_t intermediate_bgra_stride = width * 4;
  {
    intermediate_bgra_buf.resize(intermediate_bgra_stride * height);
...
std::vector<uint8_t> EncodeRGBAPNG(pdfium::span<const uint8_t> input,
  ...
  // Crude integer overflow protection.  All test images are expected to be
  // smaller than this.
  CHECK_LE(width, 10000);
  CHECK_LE(height, 10000);

  CHECK_LE(width * 4, row_byte_width);
...
std::vector<uint8_t> EncodeGrayPNG(pdfium::span<const uint8_t> input,
  ...
  CHECK_LE(width * 1, row_byte_width);
