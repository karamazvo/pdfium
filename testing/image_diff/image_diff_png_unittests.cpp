...
TEST(ImageDiffPng, EncodeBGRAPNGAndDiscardTransparency) {
  std::vector<uint8_t> bgrx_input = {1, 2, 3, /*ignored_alpha=*/4};
  ...
  int output_width = -1;
  int output_height = -1;
  std::vector<uint8_t> rgba_output = DecodePNG(
      png,
      /*reverse_byte_order=*/false /*=RGBA*/, &output_width, &output_height);
...
TEST(ImageDiffPng, EncodeBGRAPNGAndPreserveTransparency) {
  std::vector<uint8_t> bgra_input = {1, 2, 3, 4};
