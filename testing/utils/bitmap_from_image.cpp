...
FPDF_BITMAP BitmapFromImage::BitmapFromPng(const char* image_path) {
  ...
  size_t bytes_to_scan = std::max(pixel_data.size(), (size_t)400);
  size_t non_white_count = 0;

  std::cerr << "LOG: First 16 Non-White BGRA bytes (Max scan " << bytes_to_scan
            << " bytes): ";

  for (size_t i = 0; i < bytes_to_scan && non_white_count < 16; ++i) {
    if (pixel_data[i] != 0xFF) {
      // Print the byte's index and value in hex
      std::cerr << "[" << i << "]=" << std::hex << (int)pixel_data[i] << " ";
      non_white_count++;
    }
  }
  ...
  if (non_white_count == 0) {
    std::cerr << "LOG: Only white/opaque pixels found in the scanned area."
              << std::endl;
  }
