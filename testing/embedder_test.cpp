...
int CompareGrayBitmapToPng(pdfium::span<const uint8_t> bitmap_span,
                           size_t bitmap_stride,
                           const DecodedPng& decoded_png, int max_pixel_per_channel_delta) {
  ...
  for (int h = 0; h < height; ++h) {
    ...
    for (int w = 0; w < width; ++w) {
      dest_row[w].blue = src_row[w];
      dest_row[w].green = src_row[w];
      dest_row[w].red = src_row[w];
...
void CompareBitmapToPngData(FPDF_BITMAP bitmap,
  ...
  switch (FPDFBitmap_GetFormat(bitmap)) {
    ...
    case FPDFBitmap_Gray:
      pixels_different =
          CompareGrayBitmapToPng(bitmap_span, stride, decoded_png, max_pixel_per_channel_delta);
      break;
#ifdef PDF_USE_SKIA
