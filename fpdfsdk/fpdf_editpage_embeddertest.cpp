...
TEST_F(FPDFEditPageEmbedderTest, Bug378464305) {
  ...
  // Add a new path and render.
  constexpr int kObjectCountWithNewPath = 9;
  ...
  {
    ScopedFPDFBitmap bitmap = RenderLoadedPage(page.get());
    CompareBitmap(bitmap.get(), page_width, page_height, checksum);
...

  {
    ...
    EXPECT_EQ(kObjectCountWithNewPath - 1, FPDFPage_CountObjects(saved_page));
...

  {
    ...
    ScopedFPDFBitmap bitmap = RenderSavedPage(saved_page);
    // TODO(crbug.com/378464305): Should be `checksum`.
