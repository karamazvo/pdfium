...
TEST_F(FPDFEditPageEmbedderTest, PageObjectIsUsed) {
  const char* one_rectangle_unused_checksum = []() {
    if (CFX_DefaultRenderDevice::UseSkiaRenderer()) {
  ...
  // Mark one of the page objects as unused.  It is still present in the page.
  FPDF_PAGEOBJECT page_obj = FPDFPage_GetObject(page.get(), 4);
  ASSERT_TRUE(page_obj);
  ASSERT_TRUE(FPDFPageObj_SetIsUsed(page_obj, /*used=*/false));
  EXPECT_TRUE(FPDFPage_GenerateContent(page.get()));
  EXPECT_EQ(8, FPDFPage_CountObjects(page.get()));

  {
    ...
    // The absence of the unused page object affects the rendered result.
    ScopedFPDFBitmap bitmap = RenderSavedPage(saved_page);
    CompareBitmap(bitmap.get(), page_width, page_height,
                  one_rectangle_unused_checksum);
