...
FPDF_EXPORT FPDF_BOOL FPDF_CALLCONV FPDF_GetFileVersion(FPDF_DOCUMENT doc,
  ...
  *fileVersion = pParser->GetFileVersion();
  ...
  const CPDF_Dictionary* pRoot = pDoc->GetRoot();
  if (pRoot) {
    RetainPtr<const CPDF_String> version_str = pRoot->GetStringFor("Version");
    RetainPtr<const CPDF_Object> version_obj = pRoot->GetObjectFor("Version");
    RetainPtr<const CPDF_Name> version_name;
    if (version_obj && version_obj->IsName()) {
      version_name = pdfium::WrapRetain(version_obj->AsName());
    }
    ByteString version;
    if (version_str) {
      version = version_str->GetString();
    } else if (version_name) {
      version = version_name->GetString();
    }
    if (!version.IsEmpty()) {
