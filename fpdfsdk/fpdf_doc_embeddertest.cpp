...
TEST_F(FPDFDocEmbedderTest, DeletePageAndRender) {
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    const char* expected = page_paths[i];
    CompareBitmapToPngWithExpectationSuffix(bitmap.get(), expected);
  }
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    ASSERT_TRUE(page);
    ScopedFPDFBitmap bitmap = RenderLoadedPage(page.get());
    const char* expected = page_paths[i + 1];
    CompareBitmapToPngWithExpectationSuffix(bitmap.get(), expected);
  }
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    const char* expected = page_paths[adjusted_index];
    CompareBitmapToPngWithExpectationSuffix(bitmap.get(), expected);
  }
}
