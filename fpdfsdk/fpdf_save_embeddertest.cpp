...
class FPDFSaveWithParamsEmbedderTest : public FPDFSaveEmbedderTest {
 public:
  ...
  static constexpr FPDF_SAVE_PARAMS kSaveParamsWithSubset{
      .version = 1,
      .flags = 0,
      .file_version = 15,
...
TEST_F(FPDFSaveWithParamsEmbedderTest, SaveWithBadVersion) {
  ASSERT_TRUE(OpenDocument("hello_world.pdf"));
  constexpr FPDF_SAVE_PARAMS kSaveParamsBadVersion{.version = 0,
                                                   .flags = FPDF_NO_INCREMENTAL,
                                                   .file_version = 15,
                                                   .subset_new_fonts = false};
  EXPECT_FALSE(FPDF_SaveWithParams(document(), this, &kSaveParamsBadVersion));
}

TEST_F(FPDFSaveWithParamsEmbedderTest, SaveWithoutSubsetWithoutNewText) {
  ASSERT_TRUE(OpenDocument("hello_world.pdf"));
  EXPECT_TRUE(FPDF_SaveWithParams(document(), this, &kSaveParamsWithoutSubset));
  EXPECT_THAT(GetString(), StartsWith("%PDF-1.5\r\n"));
  EXPECT_EQ(805u, GetString().size());
}

TEST_F(FPDFSaveWithParamsEmbedderTest, SaveWithoutSubsetWithNewText) {
  ASSERT_TRUE(OpenDocument("hello_world.pdf"));
  ScopedPage page = LoadScopedPage(0);
  ASSERT_TRUE(page);
