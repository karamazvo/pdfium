...
TEST_F(FPDFEditEmbedderTest, SetPositionsBadParams) {
  ...
  ASSERT_EQ(2, FPDFPage_CountObjects(page.get()));
  static constexpr auto kSamplePositions = std::to_array<const float>(
  ...
  EXPECT_FALSE(FPDFText_SetPositions(page_object, kSamplePositions.data(), 11));
  EXPECT_FALSE(FPDFText_SetPositions(page_object, kSamplePositions.data(), 13));
}
