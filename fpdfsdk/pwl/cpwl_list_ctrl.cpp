...
CFX_PointF CPWL_ListCtrl::InToOut(const CFX_PointF& point) const {
  CFX_FloatRect rcPlate = rc_plate_;
  return CFX_PointF(point.x - (pt_scroll_pos_.x - rcPlate.left),
                    point.y - (pt_scroll_pos_.y - rcPlate.top));
}
...
void CPWL_ListCtrl::InvalidateItem(int32_t nItemIndex) {
  ...
  if (nItemIndex == -1) {
    if (!notify_flag_) {
      notify_flag_ = true;
      CFX_FloatRect rcRefresh = rc_plate_;
...
CFX_FloatRect CPWL_ListCtrl::GetContentRectInternal() const {
  return InnerToOuter(rc_content_);
}
...
void CPWL_ListCtrl::ReArrange(int32_t nItemIndex) {
  ...
  rc_content_ = CFX_FloatRect(0.0f, fPosY, 0.0f, 0.0f);
  SetScrollInfo();
}
...
void CPWL_ListCtrl::AddItem(const WideString& str) {
  ...
  pListItem->SetFontSize(ffont_size_);
