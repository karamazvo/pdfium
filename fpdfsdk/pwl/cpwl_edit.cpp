...
bool CPWL_Edit::RepositionChildWnd() {
  ...
  if (this_observed->caret_ && !creation_params_.dwFlags.es_text_overflow) {
...
void CPWL_Edit::OnCreated() {
  ...
  if (CPWL_ScrollBar* pScroll = GetVScrollBar()) {
    creation_params_.dwFlags.win_auto_transparent = false;
...
void CPWL_Edit::SetParamByFlag() {
  if (creation_params_.dwFlags.es_right) {
    edit_impl_->SetAlignmentH(2);
  } else if (creation_params_.dwFlags.es_middle) {
    edit_impl_->SetAlignmentH(1);
  } else {
    edit_impl_->SetAlignmentH(0);
  }

  if (creation_params_.dwFlags.es_center) {
    edit_impl_->SetAlignmentV(1);
  } else {
    edit_impl_->SetAlignmentV(0);
  }

  if (creation_params_.dwFlags.es_password) {
    edit_impl_->SetPasswordChar('*');
  }

  edit_impl_->SetMultiLine(creation_params_.dwFlags.es_multi_line);
  edit_impl_->SetAutoReturn(creation_params_.dwFlags.es_auto_return);
  edit_impl_->SetAutoFontSize(creation_params_.dwFlags.win_auto_font_size);
  edit_impl_->SetAutoScroll(creation_params_.dwFlags.es_auto_scroll);
  edit_impl_->EnableUndo(creation_params_.dwFlags.es_undo);

  if (creation_params_.dwFlags.es_text_overflow) {
...
void CPWL_Edit::SetCharArray(int32_t nCharArray) {
  if (!creation_params_.dwFlags.es_char_array || nCharArray <= 0) {
    return;
  }
  ...
  if (!creation_params_.dwFlags.win_auto_font_size) {
    return;
  }
...
bool CPWL_Edit::OnMouseWheel(Mask<FWL_EVENTFLAG> nFlag,
  ...
  if (!creation_params_.dwFlags.es_multi_line) {
    return false;
  }
...
bool CPWL_Edit::OnLButtonDown(Mask<FWL_EVENTFLAG> nFlag,
  ...
  if (creation_params_.dwFlags.es_text_overflow || ClientHitTest(point)) {
