...
bool CPWL_Edit::RepositionChildWnd() {
  ...
  if (this_observed->caret_ && !HasFlag(CPWL_Wnd::Styles::kEditTextOverflow)) {
...
void CPWL_Edit::OnCreated() {
  ...
  if (CPWL_ScrollBar* pScroll = GetVScrollBar()) {
    pScroll->RemoveFlag(CPWL_Wnd::Styles::kWindowAutoTransparent);
...
void CPWL_Edit::SetParamByFlag() {
  if (HasFlag(CPWL_Wnd::Styles::kEditRight)) {
    edit_impl_->SetAlignmentH(2);
  } else if (HasFlag(CPWL_Wnd::Styles::kEditMiddle)) {
    edit_impl_->SetAlignmentH(1);
  } else {
    edit_impl_->SetAlignmentH(0);
  }

  if (HasFlag(CPWL_Wnd::Styles::kEditCenter)) {
    edit_impl_->SetAlignmentV(1);
  } else {
    edit_impl_->SetAlignmentV(0);
  }

  if (HasFlag(CPWL_Wnd::Styles::kEditPassword)) {
    edit_impl_->SetPasswordChar('*');
  }

  edit_impl_->SetMultiLine(HasFlag(CPWL_Wnd::Styles::kEditMultiline));
  edit_impl_->SetAutoReturn(HasFlag(CPWL_Wnd::Styles::kEditAutoReturn));
  edit_impl_->SetAutoFontSize(HasFlag(CPWL_Wnd::Styles::kWindowAutoFontSize));
  edit_impl_->SetAutoScroll(HasFlag(CPWL_Wnd::Styles::kEditAutoScroll));
  edit_impl_->EnableUndo(HasFlag(CPWL_Wnd::Styles::kEditUndo));

  if (HasFlag(CPWL_Wnd::Styles::kEditTextOverflow)) {
...
void CPWL_Edit::SetCharArray(int32_t nCharArray) {
  if (!HasFlag(CPWL_Wnd::Styles::kEditCharArray) || nCharArray <= 0) {
    return;
  }
  ...
  if (!HasFlag(CPWL_Wnd::Styles::kWindowAutoFontSize)) {
    return;
  }
...
bool CPWL_Edit::OnMouseWheel(Mask<FWL_EVENTFLAG> nFlag,
  ...
  if (!HasFlag(CPWL_Wnd::Styles::kEditMultiline)) {
    return false;
  }
...
void CPWL_Edit::CreateEditCaret(const CreateParams& cp) {
  ...
  CreateParams ecp = cp;
  ecp.dwFlags = CPWL_Wnd::Styles::kWindowNoRefreshClip;
  ecp.dwBorderWidth = 0;
...
bool CPWL_Edit::OnLButtonDown(Mask<FWL_EVENTFLAG> nFlag,
  ...
  if (HasFlag(CPWL_Wnd::Styles::kEditTextOverflow) || ClientHitTest(point)) {
