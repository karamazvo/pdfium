...
bool CPWL_Edit::RepositionChildWnd() {
  ...
  if (this_observed->caret_ && !HasFlag(PStyles::kPesTextOverflow)) {
...
void CPWL_Edit::OnCreated() {
  ...
  if (CPWL_ScrollBar* pScroll = GetVScrollBar()) {
    pScroll->RemoveFlag(PStyles::kPwsAutoTransparent);
...
void CPWL_Edit::SetParamByFlag() {
  if (HasFlag(PStyles::kPesRight)) {
    edit_impl_->SetAlignmentH(2);
  } else if (HasFlag(PStyles::kPesMiddle)) {
    edit_impl_->SetAlignmentH(1);
  } else {
    edit_impl_->SetAlignmentH(0);
  }

  if (HasFlag(PStyles::kPesCenter)) {
    edit_impl_->SetAlignmentV(1);
  } else {
    edit_impl_->SetAlignmentV(0);
  }

  if (HasFlag(PStyles::kPesPassword)) {
    edit_impl_->SetPasswordChar('*');
  }

  edit_impl_->SetMultiLine(HasFlag(PStyles::kPesMultiline));
  edit_impl_->SetAutoReturn(HasFlag(PStyles::kPesAutoReturn));
  edit_impl_->SetAutoFontSize(HasFlag(PStyles::kPwsAutoFontSize));
  edit_impl_->SetAutoScroll(HasFlag(PStyles::kPesAutoScroll));
  edit_impl_->EnableUndo(HasFlag(PStyles::kPesUndo));

  if (HasFlag(PStyles::kPesTextOverflow)) {
...
void CPWL_Edit::SetCharArray(int32_t nCharArray) {
  if (!HasFlag(PStyles::kPesCharArray) || nCharArray <= 0) {
    return;
  }
...
bool CPWL_Edit::OnMouseWheel(Mask<FWL_EVENTFLAG> nFlag,
  ...
  if (!HasFlag(PStyles::kPesMultiline)) {
    return false;
  }
...
void CPWL_Edit::CreateEditCaret(const CreateParams& cp) {
  ...
  CreateParams ecp = cp;
  ecp.dwFlags = PStyles::kPwsNoRefreshClip;
  ecp.dwBorderWidth = 0;
...
bool CPWL_Edit::OnLButtonDown(Mask<FWL_EVENTFLAG> nFlag,
  ...
  if (HasFlag(PStyles::kPesTextOverflow) || ClientHitTest(point)) {
