...
void CPWL_ListBox::OnCreated() {
  ...
  SetHoverSel(GetCreationParams()->dwFlags.ls_hover_sel);
  list_ctrl_->SetMultipleSel(GetCreationParams()->dwFlags.ls_multiple_sel);
  list_ctrl_->SetFontSize(GetCreationParams()->fFontSize);

  hover_sel_ = GetCreationParams()->dwFlags.ls_hover_sel;
}
