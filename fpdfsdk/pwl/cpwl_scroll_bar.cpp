...
void CPWL_ScrollBar::SetScrollRange(float fMin,
  ...
  this_observed->s_data_.SetScrollRange(fMin, fMax);
  this_observed->s_data_.SetClientWidth(fClientWidth);

  if (FXSYS_IsFloatSmaller(this_observed->s_data_.ScrollRange.GetWidth(),
                           0.0f)) {
...
void CPWL_ScrollBar::SetScrollPos(float fPos) {
  float fOldPos = s_data_.fScrollPos;
  s_data_.SetPos(fPos);
  if (!FXSYS_IsFloatEqual(s_data_.fScrollPos, fOldPos)) {
...
void CPWL_ScrollBar::SetScrollStep(float fBigStep, float fSmallStep) {
  s_data_.SetBigStep(fBigStep);
  s_data_.SetSmallStep(fSmallStep);
}
...
bool CPWL_ScrollBar::MovePosButton(bool bRefresh) {
  ...
  if (this_observed->pos_button_->IsVisible()) {
    ...
    float fTop = this_observed->TrueToFace(s_data_.fScrollPos);
    float fBottom =
        this_observed->TrueToFace(s_data_.fScrollPos + s_data_.fClientWidth);
...
void CPWL_ScrollBar::OnPosButtonMouseMove(const CFX_PointF& point) {
  ...
  float fOldScrollPos = s_data_.fScrollPos;
  float fNewPos = FaceToTrue(old_pos_button_ + point.y - old_pos_);
  if (mouse_down_) {
    if (FXSYS_IsFloatSmaller(fNewPos, s_data_.ScrollRange.fMin)) {
      fNewPos = s_data_.ScrollRange.fMin;
    }

    if (FXSYS_IsFloatBigger(fNewPos, s_data_.ScrollRange.fMax)) {
      fNewPos = s_data_.ScrollRange.fMax;
    }

    s_data_.SetPos(fNewPos);

    if (!FXSYS_IsFloatEqual(fOldScrollPos, s_data_.fScrollPos)) {
      if (!MovePosButton(true))
        return;
...
float CPWL_ScrollBar::TrueToFace(float fTrue) {
  ...
  float fFactWidth = s_data_.ScrollRange.GetWidth() + s_data_.fClientWidth;
...
void CPWL_ScrollBar::OnTimerFired() {
  PWL_SCROLL_PRIVATEDATA sTemp = s_data_;
  if (min_or_max_) {
    s_data_.SubSmall();
  } else
    s_data_.AddSmall();
