...
void CPWL_ScrollBar::SetScrollRange(float fMin,
  ...
  this_observed->sdata_.SetScrollRange(fMin, fMax);
  this_observed->sdata_.SetClientWidth(fClientWidth);

  if (FXSYS_IsFloatSmaller(this_observed->sdata_.ScrollRange.GetWidth(),
                           0.0f)) {
...
void CPWL_ScrollBar::SetScrollPos(float fPos) {
  float fOldPos = sdata_.fScrollPos;
  sdata_.SetPos(fPos);
  if (!FXSYS_IsFloatEqual(sdata_.fScrollPos, fOldPos)) {
...
void CPWL_ScrollBar::SetScrollStep(float fBigStep, float fSmallStep) {
  sdata_.SetBigStep(fBigStep);
  sdata_.SetSmallStep(fSmallStep);
}
...
bool CPWL_ScrollBar::MovePosButton(bool bRefresh) {
  ...
  if (this_observed->pos_button_->IsVisible()) {
    ...
    float fTop = this_observed->TrueToFace(sdata_.fScrollPos);
    float fBottom =
        this_observed->TrueToFace(sdata_.fScrollPos + sdata_.fClientWidth);
...
void CPWL_ScrollBar::OnPosButtonLBDown(const CFX_PointF& point) {
  ...
  if (pos_button_) {
    CFX_FloatRect rcPosButton = pos_button_->GetWindowRect();
    old_pos_ = point.y;
    fold_pos_button_ = rcPosButton.top;
  }
}
...
void CPWL_ScrollBar::OnPosButtonMouseMove(const CFX_PointF& point) {
  ...
  float fOldScrollPos = sdata_.fScrollPos;
  float fNewPos = FaceToTrue(fold_pos_button_ + point.y - old_pos_);
  if (mouse_down_) {
    if (FXSYS_IsFloatSmaller(fNewPos, sdata_.ScrollRange.fMin)) {
      fNewPos = sdata_.ScrollRange.fMin;
    }

    if (FXSYS_IsFloatBigger(fNewPos, sdata_.ScrollRange.fMax)) {
      fNewPos = sdata_.ScrollRange.fMax;
    }

    sdata_.SetPos(fNewPos);

    if (!FXSYS_IsFloatEqual(fOldScrollPos, sdata_.fScrollPos)) {
      if (!MovePosButton(true))
        return;
...
void CPWL_ScrollBar::NotifyScrollWindow() {
  ...
  pParent->ScrollWindowVertically(origin_info_.fContentMax - sdata_.fScrollPos);
}
...
float CPWL_ScrollBar::TrueToFace(float fTrue) {
  ...
  float fFactWidth = sdata_.ScrollRange.GetWidth() + sdata_.fClientWidth;
...
void CPWL_ScrollBar::OnTimerFired() {
  PWL_SCROLL_PRIVATEDATA sTemp = sdata_;
  if (min_or_max_) {
    sdata_.SubSmall();
  } else
    sdata_.AddSmall();
