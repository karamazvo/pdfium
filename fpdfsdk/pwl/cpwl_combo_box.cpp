...
bool CPWL_ComboBox::RepositionChildWnd() {
  ...
  if (this_observed->popup_) {
    const float fOldWindowHeight = this_observed->rc_old_window_.Height();
...
bool CPWL_ComboBox::SetPopup(bool bPopup) {
  ...
  if (bPopup == this_observed->popup_) {
    return true;
  }
  ...
  if (!bPopup) {
    this_observed->popup_ = false;
    return Move(this_observed->rc_old_window_, true, true);
  }
  ...
  this_observed->rc_old_window_ = this_observed->CPWL_Wnd::GetWindowRect();
  this_observed->popup_ = bPopup;
  this_observed->bottom_ = bBottom;

  CFX_FloatRect rcWindow = this_observed->rc_old_window_;
  if (bBottom) {
