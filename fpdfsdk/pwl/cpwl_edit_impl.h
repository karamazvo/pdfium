...
class CPWL_EditImpl {
 public:
  class Iterator {
   ...
   private:
    ...
    UnownedPtr<CPVT_VariableText::Iterator> vtiterator_;
  };
 ...
 private:
   ...
   private:
    struct LineRect {
      LineRect(const CPVT_WordRange& wrLine, const CFX_FloatRect& rcLine)
          : wr_line_(wrLine), rc_line_(rcLine) {}

      CPVT_WordRange wr_line_;
      CFX_FloatRect rc_line_;
    };
  ...
  std::unique_ptr<Provider> vtprovider_;
  std::unique_ptr<CPVT_VariableText> vt_;  // Must outlive |vtprovider_|.
  ...
  CFX_PointF pt_caret_;
  UndoStack undo_;
  CFX_FloatRect rc_old_content_;
};
