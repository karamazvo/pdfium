...
void CPWL_EditImpl::SetFontMap(IPVT_FontMap* pFontMap) {
  vtprovider_ = std::make_unique<Provider>(pFontMap);
  vt_->SetProvider(vtprovider_.get());
}
...
IPVT_FontMap* CPWL_EditImpl::GetFontMap() {
  return vtprovider_ ? vtprovider_->GetFontMap() : nullptr;
}

void CPWL_EditImpl::SetPlateRect(const CFX_FloatRect& rect) {
  vt_->SetPlateRect(rect);
  pt_scroll_pos_ = CFX_PointF(rect.left, rect.top);
}
...
void CPWL_EditImpl::SetContentChanged() {
  if (notify_) {
    ...
    if (rcContent.Width() != rc_old_content_.Width() ||
        rcContent.Height() != rc_old_content_.Height()) {
      rc_old_content_ = rcContent;
    }
  }
}
...
CFX_PointF CPWL_EditImpl::VTToEdit(const CFX_PointF& point) const {
  ...
  return CFX_PointF(point.x - (pt_scroll_pos_.x - rcPlate.left),
                    point.y - (pt_scroll_pos_.y + fPadding - rcPlate.top));
}
...
void CPWL_EditImpl::SetScrollPosX(float fx) {
  ...
  if (vt_->IsValid()) {
    if (!FXSYS_IsFloatEqual(pt_scroll_pos_.x, fx)) {
      pt_scroll_pos_.x = fx;
      Refresh();
    }
  }
...
void CPWL_EditImpl::SetScrollPosY(float fy) {
  ...
  if (vt_->IsValid()) {
    if (!FXSYS_IsFloatEqual(pt_scroll_pos_.y, fy)) {
      pt_scroll_pos_.y = fy;
      Refresh();
...
void CPWL_EditImpl::SetScrollLimit() {
  if (vt_->IsValid()) {
    ...
    if (rcPlate.Width() > rcContent.Width()) {
      ...
      if (FXSYS_IsFloatSmaller(pt_scroll_pos_.x, rcContent.left)) {
        SetScrollPosX(rcContent.left);
      } else if (FXSYS_IsFloatBigger(pt_scroll_pos_.x,
    ...
    if (rcPlate.Height() > rcContent.Height()) {
      ...
      if (FXSYS_IsFloatSmaller(pt_scroll_pos_.y,
                               rcContent.bottom + rcPlate.Height())) {
        SetScrollPosY(rcContent.bottom + rcPlate.Height());
      } else if (FXSYS_IsFloatBigger(pt_scroll_pos_.y, rcContent.top)) {
...
void CPWL_EditImpl::Refresh() {
  if (enable_refresh_ && vt_->IsValid()) {
    ...
    refresh_.NoAnalyse();
    pt_refresh_scroll_pos_ = pt_scroll_pos_;
...
void CPWL_EditImpl::OnVK_UP(bool bShift) {
  ...
  SetCaret(vt_->GetUpWordPlace(wp_caret_, pt_caret_));
  if (bShift) {
    if (sel_state_.IsEmpty()) {
...
void CPWL_EditImpl::OnVK_DOWN(bool bShift) {
  ...
  SetCaret(vt_->GetDownWordPlace(wp_caret_, pt_caret_));
  if (bShift) {
    if (sel_state_.IsEmpty()) {
      sel_state_.Set(wp_old_caret_, wp_caret_);
    } else
...
void CPWL_EditImpl::SetCaretOrigin() {
  ...
  CPVT_Line line;
  if (pIterator->GetWord(word)) {
    pt_caret_.x = word.ptWord.x + word.fWidth;
    pt_caret_.y = word.ptWord.y;
  } else if (pIterator->GetLine(line)) {
    pt_caret_.x = line.ptLine.x;
    pt_caret_.y = line.ptLine.y;
  }
}
