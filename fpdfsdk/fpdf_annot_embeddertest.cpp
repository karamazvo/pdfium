...
TEST_F(FPDFAnnotEmbedderTest, ExtractInkMultiple) {
...

  {
    ...
    // Note that the expectations are the rendered coordinates, which are
    // different from the coordinates specified in the PDF itself. In this case,
    // PDFium generated the missing appearance stream, and adjusted the
    // coordinates in that process.
    FS_RECTF rect;
    ASSERT_TRUE(FPDFAnnot_GetRect(annot.get(), &rect));
    EXPECT_EQ(350.320435f, rect.left);
    EXPECT_EQ(582.330750f, rect.bottom);
    EXPECT_EQ(476.836121f, rect.right);
    EXPECT_EQ(683.035034f, rect.top);
  }
  {
