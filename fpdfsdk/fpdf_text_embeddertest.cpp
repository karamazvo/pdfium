...
TEST_F(FPDFTextEmbedderTest, Text) {
  ...
  CompareFS_RECTF({4.0f, 3.0f, 2.0f, 1.0f}, rect);
  ...
  CompareFS_RECTF({40.664001f, 46.664001f, 47.419998f, 60.692001f}, rect);
...
TEST_F(FPDFTextEmbedderTest, TextVertical) {
  ...
  CompareFS_RECTF_Three_Places({4, 16, 170.308, 178.984}, rect);

  EXPECT_TRUE(FPDFText_GetLooseCharBox(textpage.get(), 2, &rect));
  CompareFS_RECTF_Three_Places({4, 16, 159.292, 170.308}, rect);
}
...
TEST_F(FPDFTextEmbedderTest, AnnotLinks) {
  ...
  while (FPDFLink_Enumerate(page.get(), &start_pos, &link_annot)) {
    ...
    if (start_pos == 1 || start_pos == 2) {
      ...
      CompareFS_RECTF_Three_Places({66.0, 196.0, 529.0, 544.0}, link_rect);
