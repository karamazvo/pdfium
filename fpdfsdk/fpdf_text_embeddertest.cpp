...
TEST_F(FPDFTextEmbedderTest, Text) {
  ...
  CompareFS_RECT_DOUBLE({1, 2, 3, 4}, {left, right, bottom, top});
  EXPECT_FALSE(
  ...
  CompareFS_RECT_DOUBLE_Three_Places({41.12, 46.208, 49.892, 55.652},
                                     {left, right, bottom, top});

  FS_RECTF rect = {4.0f, 1.0f, 3.0f, 2.0f};
  EXPECT_FALSE(FPDFText_GetLooseCharBox(nullptr, 4, &rect));
  CompareFS_RECTF({4.0f, 2.0f, 1.0f, 3.0f}, rect);
  ...
  CompareFS_RECTF({40.664001f, 47.419998f, 60.692001f, 46.664001f}, rect);
  ...
  CompareFS_RECT_DOUBLE_Three_Places({20.800, 135.040, 96.688, 111.600},
                                     {left, right, bottom, top});
  ...
  EXPECT_FALSE(
      FPDFText_GetRect(textpage.get(), -1, &left, &top, &right, &bottom));
  CompareFS_RECT_DOUBLE({0.0, 0.0, 0.0, 0.0}, {left, right, bottom, top});
...
TEST_F(FPDFTextEmbedderTest, TextVertical) {
  ...
  EXPECT_TRUE(
      FPDFText_GetCharBox(textpage.get(), 1, &left, &right, &bottom, &top));
#if BUILDFLAG(IS_MAC)
  CompareFS_RECT_DOUBLE_Three_Places({7.168, 12.808, 171.4, 178.06},
                                     {left, right, bottom, top});
#else
  CompareFS_RECT_DOUBLE_Three_Places({7.276, 12.808, 171.364, 178.288},
                                     {left, right, bottom, top});
#endif
  EXPECT_TRUE(
      FPDFText_GetCharBox(textpage.get(), 2, &left, &right, &bottom, &top));
#if BUILDFLAG(IS_MAC)
  CompareFS_RECT_DOUBLE_Three_Places({9.472, 10.528, 160.492, 169.324},
                                     {left, right, bottom, top});
#else
  CompareFS_RECT_DOUBLE_Three_Places({9.772, 11.56, 160.348, 170.188},
                                     {left, right, bottom, top});
#endif

  FS_RECTF rect;
  EXPECT_TRUE(FPDFText_GetLooseCharBox(textpage.get(), 1, &rect));
  CompareFS_RECTF_Three_Places({4, 170.308, 178.984, 16}, rect);

  EXPECT_TRUE(FPDFText_GetLooseCharBox(textpage.get(), 2, &rect));
  CompareFS_RECTF_Three_Places({4, 159.292, 170.308, 16}, rect);
}
...
TEST_F(FPDFTextEmbedderTest, WebLinks) {
  ...
  CompareFS_RECT_DOUBLE_Three_Places({50.828, 187.904, 108.700, 97.516},
                                     {left, right, bottom, top});
  ...
  CompareFS_RECT_DOUBLE({-1.0, -1.0, -1.0, -1.0}, {left, right, bottom, top});
  ...
  CompareFS_RECT_DOUBLE({-2.0, -2.0, -2.0, -2.0}, {left, right, bottom, top});
...
TEST_F(FPDFTextEmbedderTest, AnnotLinks) {
  ...
  while (FPDFLink_Enumerate(page.get(), &start_pos, &link_annot)) {
    ...
    if (start_pos == 1 || start_pos == 2) {
      ...
      CompareFS_RECTF_Three_Places({66.0, 529.0, 544.0, 196.0}, link_rect);
...
TEST_F(FPDFTextEmbedderTest, SmallType3Glyph) {
  ...
  ASSERT_TRUE(
      FPDFText_GetCharBox(text_page.get(), 1, &left, &right, &bottom, &top));
  CompareFS_RECT_DOUBLE({62.007999420166016, 62.007999420166016, 50.0, 50.0},
                        {left, right, bottom, top});
  ASSERT_TRUE(
      FPDFText_GetCharBox(text_page.get(), 2, &left, &right, &bottom, &top));
  CompareFS_RECT_DOUBLE({86.0, 88.400001525878906, 50.0, 50.240001678466797},
                        {left, right, bottom, top});
  ASSERT_TRUE(
      FPDFText_GetCharBox(text_page.get(), 3, &left, &right, &bottom, &top));
  CompareFS_RECT_DOUBLE({86.010002136230469, 86.010002136230469, 50.0, 50.0},
                        {left, right, bottom, top});
  ASSERT_TRUE(
      FPDFText_GetCharBox(text_page.get(), 4, &left, &right, &bottom, &top));
  CompareFS_RECT_DOUBLE(
...
TEST_F(FPDFTextEmbedderTest, BigtableTextRects) {
  ...
  for (size_t i = 0; i < kExpectedRects.size(); ++i) {
    ...
    CompareFS_RECT_DOUBLE_Three_Places(
        {kExpectedRects[i].left, kExpectedRects[i].right, kExpectedRects[i].top,
