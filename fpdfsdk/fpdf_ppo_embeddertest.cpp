...
TEST_F(FPDFPPOEmbedderTest, XFAMoveTest) {
  ...
  // Load target_doc with OpenDocument for XFA extension
  ASSERT_TRUE(OpenDocument("injected_xfa_multipage_1.pdf"));
  FPDF_DOCUMENT target_doc = document();
  ASSERT_TRUE(target_doc);
  EXPECT_EQ(FPDF_GetFormType(target_doc), FORMTYPE_XFA_FOREGROUND);
  EXPECT_EQ(FPDF_GetPageCount(target_doc), 5);

  // Capture initial checksums of target_doc (all original pages)
  std::vector<std::string> target_doc_checksums;
  for (int i = 0; i < 5; i++) {
    target_doc_checksums.push_back(GetPageChecksum(target_doc, i));
  }

  ASSERT_TRUE(FPDF_MovePages(target_doc, [1, 2, 3], 3, 0));

  const int new_count = FPDF_GetPageCount(target_doc);
  // 5 Initial + 3 Inserted
  EXPECT_EQ(new_count, 5);

  // TODO - debug prints - remove before pushing this to the CL
  printf("Debug - Page order after move:\n");
  for (int i = 0; i < new_count; i++) {
    printf("Page %d: checksum %u\n", i, GetPageChecksum(target_doc, i));
  }

  printf("Debug - Original checksums:\n");
  for (int i = 0; i < 5; i++) {
    printf("Original page %d: checksum %u\n", i, target_doc_checksums[i]);
  }

  // Expected [src1, src2, src3, src0, src4]
  EXPECT_EQ(GetPageChecksum(target_doc, 0), target_doc_checksums[1]);
  EXPECT_EQ(GetPageChecksum(target_doc, 1), target_doc_checksums[2]);
  EXPECT_EQ(GetPageChecksum(target_doc, 2), target_doc_checksums[3]);
  EXPECT_EQ(GetPageChecksum(target_doc, 3), target_doc_checksums[0]);
  EXPECT_EQ(GetPageChecksum(target_doc, 4), target_doc_checksums[4]);
}
