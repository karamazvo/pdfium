...
class TestAsyncLoader final : public FX_DOWNLOADHINTS, FX_FILEAVAIL {
 ...
 private:
  ...
  int GetBlockImpl(unsigned long pos, pdfium::span<unsigned char> buf) {
    if (!IsDataAvailImpl(pos, buf.size())) {
      return 0;
    }
    const unsigned long end = pdfium::checked_cast<unsigned long>(
        std::min(file_contents_.size(), buf.size() + static_cast<size_t>(pos)));
    if (end <= pos) {
      return 0;
    }
