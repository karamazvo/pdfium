...
FPDF_GetPageDictionary(FPDF_DOCUMENT doc, int page_index) {
  const CPDF_Document* document = CPDFDocumentFromFPDFDocument(doc);
  if (!document) {
    return nullptr;
  }
  return FPDFDictFromCPDFDict(document->GetPageDictionary(page_index));
}
...
FPDF_DictionaryGetString(FPDF_DICTIONARY dictionary, FPDF_BYTESTRING key) {
  ...
  return {true, reinterpret_cast<FPDF_STRING>(dict->GetStringFor(key).c_str())};
}
