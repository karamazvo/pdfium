...
FPDF_DictionaryGetString(FPDF_DICTIONARY dictionary, FPDF_BYTESTRING key) {
  ...
  fxcrt::RetainPtr<const CPDF_Object> string_ptr =
      dict->GetDirectObjectFor(key);
  if (!string_ptr) {
    return {false, nullptr};
  }

  FPDF_STRING result_value =
      reinterpret_cast<FPDF_STRING>(string_ptr->GetString().c_str());
