...
bool GetParamValueHelper(FPDF_PAGEOBJECTMARK mark,
  ...
  if ((expected_type == ParamValueType::Int && !pObj->IsNumber()) ||
      (expected_type == ParamValueType::Float && !pObj->IsNumber()) ||
      (expected_type == ParamValueType::String && !pObj->IsString()) ||
      (expected_type == ParamValueType::Blob && !pObj->IsString())) {
    return false;
  }
...
FPDFPageObjMark_GetParamIntValue(FPDF_PAGEOBJECTMARK mark,
                                 FPDF_BYTESTRING key,
                                 int* out_value) {
  return GetParamValueHelper(mark, key, out_value, ParamValueType::Int);
}
