...
class FPDFViewEmbedderTest : public EmbedderTest {
 protected:
  ...
  void TestRenderPageSkpToPng(FPDF_PAGE page, std::string_view png_name) {
    ...
    CompareBitmapToPngWithExpectationSuffix(bitmap.get(), png_name, 0);
  }
#endif  // defined(PDF_USE_SKIA)
 ...
 private:
  ...
  void RenderPageToBitmapAndCheckToPng(FPDF_PAGE page,
    ...
    if (fuzzy) {
      ...
      CompareBitmapToPngWithExpectationSuffix(bitmap, expectation_png_name, 0);
    }
  }
...
TEST_F(FPDFViewEmbedderTest, RenderAnnotsGrayScale) {
...
#elif BUILDFLAG(IS_APPLE) && !defined(ARCH_CPU_ARM64)
      return "Need image from gerrit bots";
#else
  ...
  TestRenderPageBitmapWithInternalMemory(page.get(), FPDFBitmap_Gray,
                                         gray_checksum);
  EncodePng(RenderLoadedPage(page.get()).get());
}
