...
RetainPtr<CPDF_Array> CreateWidthsArray(
  ...
  for (auto it = widths.begin(); it != widths.end(); ++it) {
    ...
    // it->first - cid
    // it->second - glyph width

    auto next_it = std::next(it);

    if (next_it != widths.end() && next_it->first == it->first + 1 && next_it->second == it->second) {
      ...
      while (next_it != widths.end() && next_it->first == it->first + 1 && next_it->second == it->second) {
        ++it;
        next_it = std::next(it);
      }
      widths_array->AppendNew<CPDF_Number>(static_cast<int>(it->first));
    ...
    // a group may contain single item c [w]
