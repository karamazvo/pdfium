...
class CPDFXFA_Context final : public CPDF_Document::Extension,
 ...
 private:
  ...
  std::vector<RetainPtr<CPDFXFA_Page>> xfapage_list_;

  // Can't outlive |form_fill_env_|.
  std::unique_ptr<CPDFXFA_DocEnvironment> doc_env_;

  FXGCScopedHeap gcheap_;
  cppgc::Persistent<CXFA_FFApp> xfaapp_;           // can't outlive |gcheap_|
  cppgc::Persistent<CXFA_FFDoc> xfadoc_;           // Can't outlive |gcheap_|
  cppgc::Persistent<CXFA_FFDocView> xfadoc_view_;  // Can't outlive |gcheap_|
};
