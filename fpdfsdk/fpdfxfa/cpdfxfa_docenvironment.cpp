...
bool CPDFXFA_DocEnvironment::ExportSubmitFile(FPDF_FILEHANDLER* pFileHandler,
  ...
  for (size_t i = 1; i < pArray->size(); i += 2) {
    ...
    if (bsType == "config" && !flags.config) {
      continue;
    }
    if (bsType == "template" && !flags.template_data) {
      continue;
    }
    if (bsType == "localeSet" && !flags.locale_set) {
      continue;
    }
    if (bsType == "datasets" && !flags.datasets) {
      continue;
    }
    if (bsType == "xmpmeta" && !flags.xmp_meta) {
      continue;
    }
    if (bsType == "xfdf" && !flags.xfdf) {
      continue;
    }
    if (bsType == "form" && !flags.form) {
      continue;
    }
