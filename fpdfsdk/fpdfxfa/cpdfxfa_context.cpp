...
void CPDFXFA_Context::SetFormFillEnv(
  ...
  if (xfadoc_ && xfadoc_->GetXFADoc()) {
    ...
    xfadoc_.Clear();
    xfaapp_.Clear();
    FXGC_ForceGarbageCollection(gcheap_.get());
  }
...
bool CPDFXFA_Context::LoadXFADoc() {
  ...
  xfadoc_ = cppgc::MakeGarbageCollected<CXFA_FFDoc>(
      gcheap_->GetAllocationHandle(), xfaapp_, doc_env_.get(), pdfdoc_,
      gcheap_.get());
...
RetainPtr<CPDFXFA_Page> CPDFXFA_Context::GetOrCreateXFAPage(int page_index) {
  ...
  if (fxcrt::IndexInBounds(xfapage_list_, page_index)) {
    if (xfapage_list_[page_index]) {
      return xfapage_list_[page_index];
    }
  } else {
    page_count_ = GetPageCount();
    xfapage_list_.resize(page_count_);
  }
  ...
  if (fxcrt::IndexInBounds(xfapage_list_, page_index)) {
    xfapage_list_[page_index] = pPage;
  }
...
RetainPtr<CPDFXFA_Page> CPDFXFA_Context::GetXFAPage(
  ...
  for (auto& pTempPage : xfapage_list_) {
...
uint32_t CPDFXFA_Context::DeletePage(int page_index) {
  ...
  // this |page_index| then |xfapage_list_| may have size < |page_index| even
  // if it's a valid page in the document.
  uint32_t page_obj_num = pdfdoc_->DeletePage(page_index);

  if (fxcrt::IndexInBounds(xfapage_list_, page_index)) {
    xfapage_list_.erase(xfapage_list_.begin() + page_index);
    for (int i = page_index; i < fxcrt::CollectionSize<int>(xfapage_list_);
         i++) {
      if (xfapage_list_[i]) {
        xfapage_list_[i]->SetXFAPageViewIndex(i);
      }
    }
  }
