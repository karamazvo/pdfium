...
CPDFSDK_PageView::CPDFSDK_PageView(CPDFSDK_FormFillEnvironment* pFormFillEnv,
                                   IPDF_Page* page)
    : age_(page), form_fill_env_(pFormFillEnv) {
  DCHECK(age_);
...
CPDFSDK_PageView::~CPDFSDK_PageView() {
  if (!age_->AsXFAPage()) {
    ...
    // `age_` to be freed, which will cause issues if we try to cleanup the
    // pageview pointer in `age_`. So, reset the pageview pointer before doing
    // anything else.
    age_->AsPDFPage()->SetView(nullptr);
  }
...
CPDF_Document* CPDFSDK_PageView::GetPDFDocument() {
  return age_->GetDocument();
}

CPDF_Page* CPDFSDK_PageView::GetPDFPage() const {
  return ToPDFPage(age_);
}
...
#ifdef PDF_ENABLE_XFA
  RetainPtr<CPDFXFA_Page> protector(ToXFAPage(age_));
...
void CPDFSDK_PageView::UpdateRects(const std::vector<CFX_FloatRect>& rects) {
  for (const auto& rc : rects)
    form_fill_env_->Invalidate(age_, rc.GetOuterRect());
}

void CPDFSDK_PageView::UpdateView(CPDFSDK_Annot* pAnnot) {
  CFX_FloatRect rcWindow = pAnnot->GetRect();
  form_fill_env_->Invalidate(age_, rcWindow.GetOuterRect());
}

int CPDFSDK_PageView::GetPageIndex() const {
#ifdef PDF_ENABLE_XFA
  CPDF_Document::Extension* pContext = age_->GetDocument()->GetExtension();
  if (pContext && pContext->ContainsExtensionFullForm()) {
    CXFA_FFPageView* pPageView = age_->AsXFAPage()->GetXFAPageView();
