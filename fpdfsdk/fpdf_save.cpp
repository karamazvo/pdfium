...
bool SaveXFADocumentData(
  ...
  CPDF_Document* cpdf_doc = context->GetPDFDoc();
  if (!cpdf_doc) {
    return false;
  }

  RetainPtr<CPDF_Dictionary> root = cpdf_doc->GetMutableRoot();
  if (!root) {
    return false;
  }
  ...
  // L"datasets"
  {
    ...
    if (context->SaveDatasetsPackage(file_write) && file_write->GetSize() > 0) {
      if (datasets_index != -1) {
        ...
        auto data_stream = cpdf_doc->NewIndirect<CPDF_Stream>(
            file_write, cpdf_doc->New<CPDF_Dictionary>());
        int last_index = fxcrt::CollectionSize<int>(*xfa_array) - 2;
        xfa_array->InsertNewAt<CPDF_String>(last_index, "datasets");
        xfa_array->InsertNewAt<CPDF_Reference>(last_index + 1, cpdf_doc,
...
FPDF_EXPORT FPDF_BOOL FPDF_CALLCONV FPDF_SaveAsCopy(FPDF_DOCUMENT document,
  ...
  FPDF_SAVE_PARAMS params = {
      .flags = flags, .file_version = 0, .subset_new_fonts = false};
...
FPDF_SaveWithVersion(FPDF_DOCUMENT document,
                     ...
                     int file_version) {
  FPDF_SAVE_PARAMS params = {
      .flags = flags, .file_version = file_version, .subset_new_fonts = false};
  return FPDF_SaveWithParams(document, file_write, &params);
}
...
FPDF_SaveWithParams(FPDF_DOCUMENT document,
...
#ifdef PDF_ENABLE_XFA
  auto* context = static_cast<CPDFXFA_Context*>(cpdf_doc->GetExtension());
  if (context) {
    std::vector<RetainPtr<IFX_SeekableStream>> file_list;
  ...
  CPDF_Creator file_maker(
      cpdf_doc, pdfium::MakeRetain<CPDFSDK_FileWriteAdapter>(file_write));
