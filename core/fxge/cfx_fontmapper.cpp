...
RetainPtr<CFX_Face> CFX_FontMapper::UseInternalSubst(
  ...
  if (base_font < kNumStandardFonts) {
    if (!standard_faces_[base_font]) {
      standard_faces_[base_font] =
          CFX_Face::New(nullptr, font_mgr_->GetStandardFont(base_font), 0);
    }
  ...
  if (FontFamilyIsRoman(pitch_family)) {
    ...
    if (!generic_serif_face_) {
      generic_serif_face_ =
          CFX_Face::New(nullptr, font_mgr_->GetGenericSerifFont(), 0);
    }
  ...
  if (!generic_sans_face_) {
    generic_sans_face_ =
        CFX_Face::New(nullptr, font_mgr_->GetGenericSansFont(), 0);
  }
