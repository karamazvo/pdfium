...
RetainPtr<CFX_Face> CFX_FontMapper::GetCachedTTCFace(void* font_handle,
  ...
  RetainPtr<CFX_FontMgr::FontCacheEntry> cahce_entry =
  ...
  if (!cahce_entry) {
    ...
    cahce_entry = font_mgr_->AddTTCFontCacheEntry(ttc_size, checksum,
                                                  std::move(font_data));
  }
  size_t font_offset = ttc_size - data_size;
  uint32_t face_index =
      GetTTCIndex(cahce_entry->FontData().first(ttc_size), font_offset);
  RetainPtr<CFX_Face> face(cahce_entry->GetFace(face_index));
  if (face) {
    return face;
  }

  face = CFX_Face::New(font_mgr_, cahce_entry,
                       cahce_entry->FontData().first(ttc_size), face_index);
  if (!face) {
    return nullptr;
  }

  cahce_entry->SetFace(face_index, face.Get());
  return face;
}
...
RetainPtr<CFX_Face> CFX_FontMapper::GetCachedFace(void* font_handle,
  ...
  if (!cahce_entry) {
    ...
    cahce_entry = font_mgr_->AddFontCacheEntry(subst_name, weight, is_italic,
                                               std::move(font_data));
  }
  RetainPtr<CFX_Face> face(cahce_entry->GetFace(0));
  if (face) {
    return face;
  }

  face = CFX_Face::New(font_mgr_, cahce_entry,
                       cahce_entry->FontData().first(data_size), 0);
  if (!face) {
    return nullptr;
  }

  cahce_entry->SetFace(0, face.Get());
  return face;
}
