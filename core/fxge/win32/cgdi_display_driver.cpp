...
bool CGdiDisplayDriver::GetDIBits(RetainPtr<CFX_DIBitmap> bitmap,
  ...
  HBITMAP hbmp = CreateCompatibleBitmap(h_dc_, width, height);
  HDC hDCMemory = CreateCompatibleDC(h_dc_);
  HBITMAP holdbmp = (HBITMAP)SelectObject(hDCMemory, hbmp);
  BitBlt(hDCMemory, 0, 0, width, height, h_dc_, left, top, SRCCOPY);
...
bool CGdiDisplayDriver::StretchDIBits(RetainPtr<const CFX_DIBBase> bitmap,
  ...
  if (bitmap->IsAlphaFormat()) {
    auto* pPlatform =
        static_cast<CWin32Platform*>(CFX_GEModule::Get()->GetPlatform());
    if (pPlatform->gdiplus_ext_.IsAvailable()) {
      return pPlatform->gdiplus_ext_.StretchDIBits(h_dc_, std::move(bitmap),
                                                   dest_left, dest_top,
