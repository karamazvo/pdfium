...
CFX_Win32FontInfo::CFX_Win32FontInfo() : hdc_(CreateCompatibleDC(nullptr)) {}
...
void CFX_Win32FontInfo::EnumFontList(CFX_FontMapper* pMapper) {
  ...
  EnumFontFamiliesExA(hdc_, &lf, reinterpret_cast<FONTENUMPROCA>(FontEnumProc),
...
size_t CFX_Win32FontInfo::GetFontData(void* hFont,
  ...
  pdfium::ScopedSelectObject select_object(hdc_, static_cast<HFONT>(hFont));
  table = fxcrt::FromBE32(table);
  size_t size = ::GetFontData(hdc_, table, 0, buffer.data(),
...
bool CFX_Win32FontInfo::GetFaceName(void* hFont, ByteString* name) {
  pdfium::ScopedSelectObject select_object(hdc_, static_cast<HFONT>(hFont));
  char facebuf[100];
  if (::GetTextFaceA(hdc_, std::size(facebuf), facebuf) == 0) {
    return false;
  }
