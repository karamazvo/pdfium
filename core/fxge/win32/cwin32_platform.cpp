...
CFX_Win32FontInfo::CFX_Win32FontInfo() : h_dc_(CreateCompatibleDC(nullptr)) {}
...
void CFX_Win32FontInfo::EnumFontList(CFX_FontMapper* pMapper) {
  ...
  EnumFontFamiliesExA(h_dc_, &lf, reinterpret_cast<FONTENUMPROCA>(FontEnumProc),
...
size_t CFX_Win32FontInfo::GetFontData(void* hFont,
  ...
  pdfium::ScopedSelectObject select_object(h_dc_, static_cast<HFONT>(hFont));
  table = fxcrt::FromBE32(table);
  size_t size = ::GetFontData(h_dc_, table, 0, buffer.data(),
...
bool CFX_Win32FontInfo::GetFaceName(void* hFont, ByteString* name) {
  pdfium::ScopedSelectObject select_object(h_dc_, static_cast<HFONT>(hFont));
  char facebuf[100];
  if (::GetTextFaceA(h_dc_, std::size(facebuf), facebuf) == 0) {
    return false;
  }
