...
CPSPrinterDriver::CPSPrinterDriver(HDC hDC,
    ...
    : hdc_(hDC), psrenderer_(ps_font_tracker, encoder_iface) {
  ...
  horz_size_ = ::GetDeviceCaps(hdc_, HORZSIZE);
  vert_size_ = ::GetDeviceCaps(hdc_, VERTSIZE);
  width_ = ::GetDeviceCaps(hdc_, HORZRES);
  height_ = ::GetDeviceCaps(hdc_, VERTRES);
  bits_per_pixel_ = ::GetDeviceCaps(hdc_, BITSPIXEL);

  psrenderer_.Init(pdfium::MakeRetain<CPSOutput>(hdc_, output_mode), level,
                   width_, height_);
  HRGN hRgn = ::CreateRectRgn(0, 0, 1, 1);
  if (::GetClipRgn(hdc_, hRgn) == 1) {
