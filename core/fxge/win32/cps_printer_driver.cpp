...
CPSPrinterDriver::CPSPrinterDriver(HDC hDC,
    ...
    : h_dc_(hDC), psrenderer_(ps_font_tracker, encoder_iface) {
  ...
  horz_size_ = ::GetDeviceCaps(h_dc_, HORZSIZE);
  vert_size_ = ::GetDeviceCaps(h_dc_, VERTSIZE);
  width_ = ::GetDeviceCaps(h_dc_, HORZRES);
  height_ = ::GetDeviceCaps(h_dc_, VERTRES);
  bits_per_pixel_ = ::GetDeviceCaps(h_dc_, BITSPIXEL);

  psrenderer_.Init(pdfium::MakeRetain<CPSOutput>(h_dc_, output_mode), level,
                   width_, height_);
  HRGN hRgn = ::CreateRectRgn(0, 0, 1, 1);
  if (::GetClipRgn(h_dc_, hRgn) == 1) {
