...
CPSPrinterDriver::CPSPrinterDriver(HDC hDC,
  ...
  horz_size_ = DeviceCapsAPI::GetHorzSize(dc_handle_);
  vert_size_ = DeviceCapsAPI::GetVertSize(dc_handle_);
  width_ = DeviceCapsAPI::GetWidth(dc_handle_);
  height_ = DeviceCapsAPI::GetHeight(dc_handle_);
  bits_per_pixel_ = DeviceCapsAPI::GetBitsPerPixel(dc_handle_);
...
int CPSPrinterDriver::GetPixelHeight() const {
  return height_;
}

int CPSPrinterDriver::GetBitsPerPixel() const {
  return bits_per_pixel_;
}

int CPSPrinterDriver::GetRenderCaps() const {
  return 0;
}

int CPSPrinterDriver::GetHorzSize() const {
  return horz_size_;
}

int CPSPrinterDriver::GetVertSize() const {
  return vert_size_;
}
