...
CPSOutput::CPSOutput(HDC hDC, OutputMode mode) : hdc_(hDC), mode_(mode) {}
    ...
    *(reinterpret_cast<uint16_t*>(buffer)) = static_cast<uint16_t>(send_len);
    ...
    switch (mode_) {
      case OutputMode::kExtEscape:
        ExtEscape(hdc_, PASSTHROUGH, static_cast<int>(send_len + 2),
                  reinterpret_cast<const char*>(buffer), 0, nullptr);
        break;
      case OutputMode::kGdiComment:
        GdiComment(hdc_, static_cast<UINT>(send_len + 2), buffer);
        break;
    }
