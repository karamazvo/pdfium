...
CPSOutput::CPSOutput(HDC hDC, OutputMode mode) : h_dc_(hDC), mode_(mode) {}
    ...
    *(reinterpret_cast<uint16_t*>(buffer)) = static_cast<uint16_t>(send_len);
    ...
    switch (mode_) {
      case OutputMode::kExtEscape:
        ExtEscape(h_dc_, PASSTHROUGH, static_cast<int>(send_len + 2),
                  reinterpret_cast<const char*>(buffer), 0, nullptr);
        break;
      case OutputMode::kGdiComment:
        GdiComment(h_dc_, static_cast<UINT>(send_len + 2), buffer);
        break;
    }
