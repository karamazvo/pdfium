...
bool CFX_RenderDevice::DrawNormalText(pdfium::span<const TextCharPos> pCharPos,
  ...
  FontAntiAliasingMode anti_alias =
      GetFTRenderMode(FontAntiAliasingMode::kMONO);
  ...
  if (is_text_smooth) {
    if (GetDeviceType() == DeviceType::kDisplay && bpp_ > 1) {
      if (!CFX_GEModule::Get()->GetFontMgr()->FTLibrarySupportsHinting()) {
        ...
        anti_alias = GetFTRenderMode(FontAntiAliasingMode::kNORMAL);
        ...
        anti_alias = GetFTRenderMode(FontAntiAliasingMode::kLCD);
        normalize = true;
      } else if (bpp_ < 16) {
  ...
  for (auto [charpos, glyph] : fxcrt::Zip(pCharPos, pdfium::span(glyphs))) {
    ...
    glyph.origin_.x = anti_alias < GetFTRenderMode(FontAntiAliasingMode::kLCD)
  ...
  if (anti_alias < GetFTRenderMode(FontAntiAliasingMode::kLCD) &&
      glyphs.size() > 1) {
  ...
  if (anti_alias == GetFTRenderMode(FontAntiAliasingMode::kMONO)) {
    auto bitmap = pdfium::MakeRetain<CFX_DIBitmap>();
  ...
  for (const TextGlyphPos& glyph : glyphs) {
    ...
    if (anti_alias == GetFTRenderMode(FontAntiAliasingMode::kNORMAL)) {
