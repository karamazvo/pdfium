...
void DrawNormalTextHelper(const RetainPtr<CFX_DIBitmap>& bitmap,
                          const RetainPtr<const CFX_DIBitmap>& pGlyph,
                          int nrows,
                          int left,
                          int top,
                          int start_col,
...
bool CFX_RenderDevice::DrawNormalText(pdfium::span<const TextCharPos> pCharPos,
  ...
  for (const TextGlyphPos& glyph : glyphs) {
    ...
    if (anti_alias == FontAntiAliasingMode::kNormal) {
      if (!bitmap->CompositeMask(
              point.value().x, point.value().y, ncols, nrows, glyph_bitmap,
              fill_color, /*src_left=*/0, /*src_top=*/0, BlendMode::kNormal,
              /*pClipRgn=*/nullptr, /*bRgbByteOrder=*/false)) {
        return false;
      }
      continue;
    }
