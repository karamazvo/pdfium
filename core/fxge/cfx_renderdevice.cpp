...
enum class CFX_RenderDevice::AARenderMode { kNORMAL, kLIGHT, kMONO, kLCD, kLCD_V, kSDF, kMAX };

int CFX_RenderDevice::GetFTRenderMode(AARenderMode mode) {
  switch (mode) {
    case AARenderMode::kNORMAL:
      return FT_RENDER_MODE_NORMAL;
    case AARenderMode::kLIGHT:
      return FT_RENDER_MODE_LIGHT;
    case AARenderMode::kMONO:
      return FT_RENDER_MODE_MONO;
    case AARenderMode::kLCD:
      return FT_RENDER_MODE_LCD;
    case AARenderMode::kLCD_V:
      return FT_RENDER_MODE_LCD_V;
    case AARenderMode::kSDF:
      return FT_RENDER_MODE_SDF;
    case AARenderMode::kMAX:
...
bool CFX_RenderDevice::DrawNormalText(pdfium::span<const TextCharPos> pCharPos,
  ...
  int anti_alias = GetFTRenderMode(AARenderMode::kNORMAL);
  ...
  if (is_text_smooth) {
    if (GetDeviceType() == DeviceType::kDisplay && bpp_ > 1) {
      if (!CFX_GEModule::Get()->GetFontMgr()->FTLibrarySupportsHinting()) {
        ...
        anti_alias = GetFTRenderMode(AARenderMode::kNORMAL);
        ...
        anti_alias = GetFTRenderMode(AARenderMode::kLCD);
        normalize = true;
      } else if (bpp_ < 16) {
  ...
  for (auto [charpos, glyph] : fxcrt::Zip(pCharPos, pdfium::span(glyphs))) {
    ...
    glyph.origin_.x = anti_alias < GetFTRenderMode(AARenderMode::kLCD)
  ...
  if (anti_alias < GetFTRenderMode(AARenderMode::kLCD) && glyphs.size() > 1) {
  ...
  if (anti_alias == GetFTRenderMode(AARenderMode::kMONO)) {
    auto bitmap = pdfium::MakeRetain<CFX_DIBitmap>();
  ...
  if (anti_alias == GetFTRenderMode(AARenderMode::kLCD)) {
