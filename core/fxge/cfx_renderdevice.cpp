...
FXDIB_Format GetCreateCompatibleBitmapFormat(int render_caps,
  ...
  if (render_caps & fxcrt::to_underlying(RenderCapsFlag::kByteMaskOutput)) {
    return FXDIB_Format::k8bppMask;
  }
#if defined(PDF_USE_SKIA)
  if (use_argb_premul &&
      (render_caps & fxcrt::to_underlying(RenderCapsFlag::kPremultipliedAlpha))) {
    return FXDIB_Format::kBgraPremul;
  }
#endif
  if (render_caps & fxcrt::to_underlying(RenderCapsFlag::kAlphaOutput)) {
...
#if defined(PDF_USE_SKIA)
    if (render_caps_ & fxcrt::to_underlying(RenderCapsFlag::kFillStrokePath)) {
...
bool CFX_RenderDevice::DrawFillStrokePath(
    ...
    uint32_t stroke_color,
    const CFX_FillRenderOptions& fill_options) {
  if (!(render_caps_ & fxcrt::to_underlying(RenderCapsFlag::kGetBits))) {
    return false;
  }
  CFX_FloatRect bbox;
  if (pGraphState) {
...
bool CFX_RenderDevice::GetDIBits(RetainPtr<CFX_DIBitmap> bitmap,
                                 int left,
                                 int top) const {
  return (render_caps_ & fxcrt::to_underlying(RenderCapsFlag::kGetBits)) &&
...
bool CFX_RenderDevice::SetDIBitsWithBlend(RetainPtr<const CFX_DIBBase> bitmap,
  ...
  if ((blend_mode == BlendMode::kNormal ||
       (render_caps_ & fxcrt::to_underlying(RenderCapsFlag::kBlendMode))) &&
      (!bitmap->IsAlphaFormat() ||
       (render_caps_ & fxcrt::to_underlying(RenderCapsFlag::kAlphaImage)))) {
    return device_driver_->SetDIBits(std::move(bitmap), /*color=*/0, src_rect,
