...
void CFX_RenderDevice::InitDeviceInfo() {
  ...
  pp_ = device_driver_->GetDeviceCaps(FXDC_BITS_PIXEL);
...
bool CFX_RenderDevice::DrawNormalText(pdfium::span<const TextCharPos> pCharPos,
  ...
  if (is_text_smooth) {
    if (GetDeviceType() == DeviceType::kDisplay && pp_ > 1) {
      if (!CFX_GEModule::Get()->GetFontMgr()->FTLibrarySupportsHinting()) {
        ...
        // This case doesn't apply to Skia since Skia always have |pp_| = 32.
        anti_alias = FT_RENDER_MODE_NORMAL;
      } else {
        // Whether Skia uses LCD optimization should strictly follow the
        // rendering options provided by |text_options|. No change needs to be
  ...
  for (auto [charpos, glyph] :
    ...
    glyph.fdevice_origin_ = text2Device.Transform(charpos.origin_);
    glyph.origin_.x = anti_alias < FT_RENDER_MODE_LCD
                          ? FXSYS_roundf(glyph.fdevice_origin_.x)
                          : static_cast<int>(floor(glyph.fdevice_origin_.x));
    glyph.origin_.y = FXSYS_roundf(glyph.fdevice_origin_.y);
  ...
  for (const TextGlyphPos& glyph : glyphs) {
    ...
    ncols /= 3;
    int x_subpixel = static_cast<int>(glyph.fdevice_origin_.x * 3) % 3;
