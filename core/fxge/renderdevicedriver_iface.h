...
class RenderDeviceDriverIface {
 public:
  ...
  [[nodiscard]] virtual bool SetClip_PathFill(
      const CFX_Path& path,
      const CFX_Matrix* pObject2Device,
      const CFX_FillRenderOptions& fill_options) = 0;
  [[nodiscard]] virtual bool SetClip_PathStroke(
      const CFX_Path& path,
      const CFX_Matrix* pObject2Device,
      const CFX_GraphStateData* pGraphState);
  [[nodiscard]] virtual bool DrawPath(
      const CFX_Path& path,
      const CFX_Matrix* pObject2Device,
      const CFX_GraphStateData* pGraphState,
      uint32_t fill_color,
      ...
      const CFX_FillRenderOptions& fill_options) = 0;
  [[nodiscard]] virtual bool FillRect(const FX_RECT& rect, uint32_t fill_color);
  [[nodiscard]] virtual bool DrawCosmeticLine(const CFX_PointF& ptMoveTo,
                                              const CFX_PointF& ptLineTo,
                                              uint32_t color);

  virtual FX_RECT GetClipBox() const = 0;
  [[nodiscard]] virtual bool GetDIBits(RetainPtr<CFX_DIBitmap> bitmap,
                                       int left,
                                       int top) const;
  virtual RetainPtr<const CFX_DIBitmap> GetBackDrop() const;
  [[nodiscard]] virtual bool SetDIBits(RetainPtr<const CFX_DIBBase> bitmap,
                                       uint32_t color,
                                       const FX_RECT& src_rect,
                                       int dest_left,
                                       int dest_top,
                                       BlendMode blend_type) = 0;
  [[nodiscard]] virtual bool StretchDIBits(RetainPtr<const CFX_DIBBase> bitmap,
                                           uint32_t color,
                                           int dest_left,
                                           int dest_top,
                                           int dest_width,
  ...
  virtual StartResult StartDIBits(RetainPtr<const CFX_DIBBase> bitmap,
                                  ...
                                  const FXDIB_ResampleOptions& options,
                                  BlendMode blend_type) = 0;
  [[nodiscard]] virtual bool ContinueDIBits(CFX_AggImageRenderer* handle,
                                            PauseIndicatorIface* pPause);
  [[nodiscard]] virtual bool DrawDeviceText(
  ...
  [[nodiscard]] virtual bool DrawShading(const CPDF_ShadingPattern& pattern,
                                         const CFX_Matrix& matrix,
                                         const FX_RECT& clip_rect,
                                         int alpha);
  [[nodiscard]] virtual bool SetBitsWithMask(
  ...
  [[nodiscard]] virtual bool MultiplyAlpha(float alpha) = 0;
  ...
  [[nodiscard]] virtual bool MultiplyAlphaMask(
      RetainPtr<const CFX_DIBitmap> mask) = 0;
};
