...
void CFX_FontMgr::FontDesc::SetFace(size_t index, CFX_Face* face) {
  CHECK_LT(index, std::size(ttcfaces_));
  ttcfaces_[index].Reset(face);
}

CFX_Face* CFX_FontMgr::FontDesc::GetFace(size_t index) const {
  CHECK_LT(index, std::size(ttcfaces_));
  return ttcfaces_[index].Get();
}

CFX_FontMgr::CFX_FontMgr()
    : ftlibrary_(FTLibraryInitHelper()),
      builtin_mapper_(std::make_unique<CFX_FontMapper>(this)),
      ftlibrary_supports_hinting_(SetLcdFilterMode() ||
...
RetainPtr<CFX_FontMgr::FontDesc> CFX_FontMgr::GetCachedTTCFontDesc(
    size_t ttc_size,
    uint32_t checksum) {
  auto it = ttcface_map_.find({ttc_size, checksum});
  return it != ttcface_map_.end() ? pdfium::WrapRetain(it->second.Get())
                                  : nullptr;
}
...
RetainPtr<CFX_FontMgr::FontDesc> CFX_FontMgr::AddCachedTTCFontDesc(
  ...
  ttcface_map_[{ttc_size, checksum}].Reset(pNewDesc.Get());
  return pNewDesc;
}
...
RetainPtr<CFX_Face> CFX_FontMgr::NewFixedFace(RetainPtr<FontDesc> pDesc,
  ...
  RetainPtr<CFX_Face> face =
      CFX_Face::New(ftlibrary_.get(), std::move(pDesc), span,
...
bool CFX_FontMgr::FreeTypeVersionSupportsHinting() const {
  ...
  FT_Int minor;
  FT_Int patch;
  FT_Library_Version(ftlibrary_.get(), &major, &minor, &patch);
...
bool CFX_FontMgr::SetLcdFilterMode() const {
  return FT_Library_SetLcdFilter(ftlibrary_.get(), FT_LCD_FILTER_DEFAULT) !=
