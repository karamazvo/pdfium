...
class CFX_FolderFontInfo : public SystemFontInfoIface {
 ...
 protected:
  ...
  class FontFaceInfo {
   public:
    enum class CharsetFlags : uint8_t {
      kNone = 0,
      kAnsi = 1 << 0,
      kSymbol = 1 << 1,
      kShiftJis = 1 << 2,
    ...
    static CharsetFlags GetCharset(FX_Charset charset);
    ...
    bool IsEligibleForFindFont(CharsetFlags flag, FX_Charset charset) const;
    ...
    uint32_t styles_ = 0;
    fxcrt::Mask<CharsetFlags> charsets_;
  };
