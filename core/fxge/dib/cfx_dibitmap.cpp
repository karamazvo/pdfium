  ...
  for (int row = 0; row < height; ++row) {
    ...
    pdfium::span<const uint8_t> src_span =
        source->GetScanline(src_top + row)
            ...
            .first(static_cast<size_t>(width) * bytes_per_pixel);
      ...
      for (int row = rect.top; row < rect.bottom; row++) {
        pdfium::span<uint8_t> dest_span =
            GetWritableScanline(row)
                .subspan(static_cast<size_t>(rect.left))
                .first(static_cast<size_t>(width));
        if (src_alpha == 255) {
  ...
  if (src_alpha == 255) {
    for (int row = rect.top; row < rect.bottom; row++) {
      UNSAFE_TODO({
        uint8_t* dest_scan =
            GetWritableScanline(row)
                .subspan(static_cast<size_t>(rect.left) * bytes_per_pixel)
                .data();
