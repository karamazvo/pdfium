...
void CFX_DIBitmap::Clear(uint32_t color) {
  ...
  switch (GetFormat()) {
    ...
    case FXDIB_Format::kRgb: {
      const FX_RGBA_STRUCT<uint8_t> rgba = ArgbToRGBAStruct(color);
      if (rgba.red == rgba.green && rgba.green == rgba.blue) {
        fxcrt::Fill(buffer, rgba.red);
      } else {
        const FX_RGB_STRUCT<uint8_t> rgb(rgba.red, rgba.green, rgba.blue);
        for (int row = 0; row < m_Height; row++) {
          fxcrt::Fill(GetWritableScanlineAs<FX_RGB_STRUCT<uint8_t>>(row), rgb);
        }
      }
      break;
    }
    ...
    case FXDIB_Format::kArgb: {
      ...
      for (int row = 0; row < m_Height; row++) {
        fxcrt::Fill(GetWritableScanline(row), color);
      }
      break;
    }
  }
