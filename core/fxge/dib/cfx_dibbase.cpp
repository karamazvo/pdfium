...
void ConvertBuffer_Rgb2ArgbPremul(
  ...
  for (int row = 0; row < height; ++row) {
    ...
    auto src_span =
        src_bitmap->GetScanlineAs<const FX_BGR_STRUCT<uint8_t>>(src_top + row)
            .subspan(src_left);
...
void ConvertBuffer_ArgbPremulToRgb(
  ...
  for (int row = 0; row < height; ++row) {
    auto dest_span = fxcrt::reinterpret_span<FX_BGR_STRUCT<uint8_t>>(
        dest_buf.subspan(Fx2DSizeOrDie(row, dest_pitch)));
    auto src_span =
        src_bitmap->GetScanlineAs<const FX_BGRA_STRUCT<uint8_t>>(src_top + row)
            .subspan(src_left);
    for (auto [input, output] : fxcrt::Zip(src_span, dest_span)) {
...
#endif
      CHECK_EQ(pSrcBitmap->GetFormat(), FXDIB_Format::kArgb);
