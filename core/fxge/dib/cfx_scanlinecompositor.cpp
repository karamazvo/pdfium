...
void AlphaMerge(T& output, const V& input, uint8_t alpha) {
...
void CompositePixelArgb2RgbNonSeparableBlend(
  ...
  AlphaMerge(output, blended_color, src_alpha);
}
...
template <typename DestPixelStruct>
void CompositePixelArgb2RgbNonBlend(const FX_BGRA_STRUCT<uint8_t>& input,
                                    uint8_t clip,
                                    DestPixelStruct& output) {
  const uint8_t src_alpha = input.alpha * clip / 255;
  if (src_alpha == 255) {
  ...
  AlphaMerge(output, input, src_alpha);
}
...
void CompositeRowArgb2RgbNoBlend(
  ...
  if (clip_span.empty()) {
    for (auto [input, output] : fxcrt::Zip(src_span, dest_span)) {
      CompositePixelArgb2RgbNonBlend(input, /*clip=*/255, output);
    }
    return;
  }
  ...
  for (auto [input, clip, output] :
       fxcrt::Zip(src_span, clip_span, dest_span)) {
    CompositePixelArgb2RgbNonBlend(input, clip, output);
  }
}
