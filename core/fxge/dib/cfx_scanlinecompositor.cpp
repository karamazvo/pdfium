...
void CompositeRow_Argb2Rgb_Blend(
  ...
  if (clip_span.empty()) {
    for (auto [input, output] : fxcrt::Zip(src_span, dest_span)) {
      ...
      if (non_separable_blend) {
        blended_color = RgbBlend(blend_type, input, output);
      } else {
        blended_color = {
      ...
      output.blue =
          FXDIB_ALPHA_MERGE(output.blue, blended_color.blue, input.alpha);
      output.green =
          FXDIB_ALPHA_MERGE(output.green, blended_color.green, input.alpha);
      output.red =
          FXDIB_ALPHA_MERGE(output.red, blended_color.red, input.alpha);
    }
    return;
  }
  ...
  for (auto [input, clip, output] :
    ...
    output.blue = FXDIB_ALPHA_MERGE(output.blue, blended_color.blue, src_alpha);
    output.green =
        FXDIB_ALPHA_MERGE(output.green, blended_color.green, src_alpha);
    output.red = FXDIB_ALPHA_MERGE(output.red, blended_color.red, src_alpha);
  }
}
