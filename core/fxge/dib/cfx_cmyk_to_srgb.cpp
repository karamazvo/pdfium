...
FX_RGB_STRUCT<uint8_t> AdobeCMYK_to_sRGB1(uint8_t c,
  ...
  int ndex_ = (fix_m + 4096) >> 13;
  int y_index = (fix_y + 4096) >> 13;
  int k_index = (fix_k + 4096) >> 13;
  const auto& start_rgb =
      kCMYK[IndexFromCMYK(c_index, ndex_, y_index, k_index)];
  ...
  const auto& c1_rgb = kCMYK[IndexFromCMYK(c1_index, ndex_, y_index, k_index)];
  ...
  const int rate_ = (fix_m - (ndex_ << 13)) * (ndex_ - m1_index);
  ...
  const auto& y1_rgb = kCMYK[IndexFromCMYK(c_index, ndex_, y1_index, k_index)];
  ...
  const auto& k1_rgb = kCMYK[IndexFromCMYK(c_index, ndex_, y_index, k1_index)];
