...
SkColor4f MakeColor4f(float r, float g, float b) {
  ...
  // For temporary compatibility with the previous implementation, we round the color components
  // to a byte, and then redivide back to a normalized float.
  // When we can update the expected images in the test suite, this rounding step should be
  // eliminated, since it only throws away information from the PDF (and is slower).
...
bool AddColors(const CPDF_ExpIntFunc* func,
  ...
  colors.push_back(MakeColor4f(begin_values[0], begin_values[1], begin_values[2]));
...
bool CFX_SkiaDeviceDriver::DrawShading(const CPDF_ShadingPattern& pattern,
  ...
  if (shading_type == kAxialShading) {
    ...
    paint.setShader(SkShaders::TwoPointConicalGradient(
        start, start_r, end, end_r, {{sk_colors, sk_pos, SkTileMode::kClamp}, {}}));
    if (clipStart || clipEnd) {
