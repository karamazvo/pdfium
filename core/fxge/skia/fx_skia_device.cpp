...
bool AddColors(const CPDF_ExpIntFunc* func,
  ...
  colors.push_back({std::clamp(begin_values[0], 0.0f, 1.0f),
                    std::clamp(begin_values[1], 0.0f, 1.0f),
                    std::clamp(begin_values[2], 0.0f, 1.0f),
                    1});
  colors.push_back({std::clamp(end_values[0], 0.0f, 1.0f),
                    std::clamp(end_values[1], 0.0f, 1.0f),
                    std::clamp(end_values[2], 0.0f, 1.0f),
                    1});
  return true;
}
  ...
  if (shading_type == kAxialShading) {
    ...
    if (clipStart || clipEnd) {
      if (clipStart && start_r) {
        skClip.addCircle(start, start_r);
      }
      if (clipEnd) {
        skClip.addCircle(end, end_r, SkPathDirection::kCCW);
      } else {
