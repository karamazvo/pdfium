...
bool CFX_SkiaDeviceDriver::DrawPath(const CFX_Path& path,
  ...
  if (fill_options.fill_type != CFX_FillRenderOptions::FillType::kNoFill &&
    ...
    // should not affect each other with transparency. See section 11.4.6 of in
    // ISO 32000-1:2008.
    //
    ...
    if (stroke_alpha && group_knockout_) {
      ...
      SkPaint mask_paint = stroke_paint;
      ...
      mask_paint.setStyle(SkPaint::kStroke_Style);
      mask_paint.setColor(stroke_color);
      DrawPathImpl(stroke_path, mask_paint);
