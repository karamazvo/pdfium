...
// Scale a scalar in range [0.0, 1.0] to 0-255.
...
bool AddColors(const CPDF_ExpIntFunc* func,
  ...
  colors.push_back(SkColorSetRGB(FloatToByte(begin_values[0]),
                                 FloatToByte(begin_values[1]),
                                 FloatToByte(begin_values[2])));
  colors.push_back(SkColorSetRGB(FloatToByte(end_values[0]),
                                 FloatToByte(end_values[1]),
                                 FloatToByte(end_values[2])));
  return true;
}
...
void ClipAngledGradient(pdfium::span<const SkPoint, 2> pts,
  ...
  float minPerpDist = FLT_MAX;
  float maxPerpDist = -FLT_MAX;
