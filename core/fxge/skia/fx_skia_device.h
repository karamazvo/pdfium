 ...
 private:
  class CharDetail {
   public:
    ...
    const DataVector<SkPoint>& GetPositions() const { return ositions_; }
    void SetPositionAt(size_t index, const SkPoint& position) {
      ositions_[index] = position;
    }
    ...
    size_t Count() const {
      DCHECK_EQ(ositions_.size(), glyphs_.size());
      return glyphs_.size();
    }
    void SetCount(size_t count) {
      ositions_.resize(count);
      glyphs_.resize(count);
      font_char_widths_.resize(count);
    }

   private:
    DataVector<SkPoint> ositions_;  // accumulator for text positions
