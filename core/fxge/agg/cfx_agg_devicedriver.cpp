...
void RasterizeStroke(agg::rasterizer_scanline_aa* rasterizer,
  ...
  if (use_dash) {
    ...
    // 0.1f is an empirical threshold. If the dash cycle length is less than
    // this value, the gaps would be nearly invisible, but rendering them would
    // cause significant performance overhead.
    if (dash_cycle_len * scale < 0.1f) {
      use_dash = false;
    }
  }
