...
int CFX_AggDeviceDriver::GetDeviceCaps(DeviceCapsId caps_id) const {
  switch (caps_id) {
    ...
    case DeviceCapsId::kRenderCaps: {
      int flags = static_cast<int>(RenderCapsFlag::kGetBits) |
                  static_cast<int>(RenderCapsFlag::kAlphaPath) |
                  static_cast<int>(RenderCapsFlag::kAlphaImage) |
                  static_cast<int>(RenderCapsFlag::kBlendMode) |
                  static_cast<int>(RenderCapsFlag::kSoftClip);
      if (bitmap_->IsAlphaFormat()) {
        flags |= static_cast<int>(RenderCapsFlag::kAlphaOutput);
      } else if (bitmap_->IsMaskFormat()) {
        CHECK_NE(bitmap_->GetBPP(), 1);  // Matches format CHECKs in the ctor.
        flags |= static_cast<int>(RenderCapsFlag::kByteMaskOutput);
      }
      return flags;
    }
  }
