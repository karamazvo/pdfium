...
CFX_AggRenderer::CFX_AggRenderer(const RetainPtr<CFX_DIBitmap>& pDevice,
    ...
    : m_Alpha(FXARGB_A(color)),
      ...
      device_(pDevice),
      clip_rgn_(pClipRgn),
      m_CompositeSpanFunc(GetCompositeSpanFunc(device_)) {
  if (device_->GetBPP() == 8) {
    DCHECK(!rgb_byte_order_);
    ...
    while (true) {
      ...
      if (clip_mask_) {
        ...
        clip_pos = clip_mask_->GetBuffer().data() +
                   (y - m_ClipBox.top) * clip_mask_->GetPitch() + x -
                   m_ClipBox.left;
      }
...
void CFX_AggDeviceDriver::RestoreState(bool bKeepSaved) {
  ...
  if (bKeepSaved) {
    if (m_StateStack.back())
      clip_rgn_ = std::make_unique<CFX_AggClipRgn>(*m_StateStack.back());
  } else {
    clip_rgn_ = std::move(m_StateStack.back());
