...
bool CFX_Font::LoadFile(RetainPtr<IFX_SeekableReadStream> pFile,
  ...
  CFX_Face::FTStreamRecAndFace StreamRecAndFace = CFX_Face::OpenFromStream(
      CFX_GEModule::Get()->GetFontMgr()->GetFTLibrary(), pFile, nFaceIndex);
  face_ = std::move(StreamRecAndFace.face);
  owned_stream_rec_ = std::move(StreamRecAndFace.ft_stream);
  return !!face_;
}
