...
bool CFX_Font::LoadFile(RetainPtr<IFX_SeekableReadStream> pFile,
  ...
  CFX_Face::FTStreamRecAndFace stream_rec_and_face = CFX_Face::OpenFromStream(
      CFX_GEModule::Get()->GetFontMgr()->GetFTLibrary(), pFile, nFaceIndex);
  face_ = std::move(stream_rec_and_face.face);
  owned_stream_rec_ = std::move(stream_rec_and_face.ft_stream);
  return !!face_;
}
