...
RetainPtr<CFX_Face> CFX_Face::OpenFromStream(
  ...
  FXFT_StreamRec* ft_stream =
      static_cast<FXFT_StreamRec*>(ft_scalloc(sizeof(FXFT_StreamRec), 1));
  if (!ft_stream) {
    return nullptr;
  }
  *ft_stream = {};  // Aggregate initialization.
  static_assert(std::is_aggregate_v<std::remove_pointer_t<decltype(ft_stream)>>);
  ...
  ftArgs.stream = ft_stream;

  RetainPtr<CFX_Face> face = Open(library, &ftArgs, face_index);
  if (!face) {
    ft_sfree(ft_stream);
    return nullptr;
  }
  face->owned_file_ = std::move(font_stream);
  return face;
}
#endif
