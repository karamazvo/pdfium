...
#if BUILDFLAG(IS_ANDROID) || defined(PDF_ENABLE_XFA)
CFX_Face::FTStreamRecAndFace::FTStreamRecAndFace() = default;

CFX_Face::FTStreamRecAndFace::FTStreamRecAndFace(
    std::unique_ptr<FXFT_StreamRec> ft_stream,
    RetainPtr<CFX_Face> face)
    : ft_stream(std::move(ft_stream)), face(std::move(face)) {}
...
CFX_Face::FTStreamRecAndFace CFX_Face::OpenFromStream(
    FT_Library library,
  ...
  face->owned_font_stream = std::move(font_stream);
  return {std::move(ft_stream), std::move(face)};
}
