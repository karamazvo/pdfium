...
FX_RECT CFX_Face::GetBBox() const {
  ...
  if (skia_typeface_) {
    ...
    // Skia bounds are 1-pt based, so scale by units per em.
    float upem = GetUnitsPerEm();
    CHECK_EQ(ft_result.left, static_cast<int32_t>(bounds.fLeft * upem));
    CHECK_EQ(ft_result.bottom, static_cast<int32_t>(bounds.fBottom * upem));
    CHECK_EQ(ft_result.right, static_cast<int32_t>(bounds.fRight * upem));
    CHECK_EQ(ft_result.top, static_cast<int32_t>(bounds.fTop * upem));
  }
#endif
  return ft_result;
}
...
int CFX_Face::GetGlyphTTWidth() const {
  ...
  if (skia_typeface_) {
    ...
    CHECK_EQ(ft_result, static_cast<int>(width));
  }
#endif
  return ft_result;
}
