...
std::unique_ptr<CFX_GlyphBitmap> CFX_Face::RenderGlyph(
  ...
  const FXDIB_Format format = GetFTRenderMode(anti_alias) == FT_RENDER_MODE_MONO
  ...
  UNSAFE_TODO({
    if (GetFTRenderMode(anti_alias) != FT_RENDER_MODE_MONO &&
        bitmap.pixel_mode == FT_PIXEL_MODE_MONO) {
      unsigned int bytes =
          GetFTRenderMode(anti_alias) == FT_RENDER_MODE_LCD ? 3 : 1;
