...
RetainPtr<CFX_Face> CFX_Face::New(RetainPtr<Retainable> desc,
  ...
  // Private ctor.
  auto result = pdfium::WrapRetain(new CFX_Face(face_rec, std::move(desc)));
#if defined(PDF_USE_SKIA)
  CHECK(result->GetOrCreateSkTypeface());
#endif
  return result;
}
...
#if defined(PDF_USE_SKIA)
  const_cast<CFX_Face*>(this)->GetOrCreateSkTypeface();
  if (skia_typeface_) {
    return skia_typeface_->isFixedPitch();
  }
#endif
...
bool CFX_Face::IsItalic() const {
  ...
  const_cast<CFX_Face*>(this)->GetOrCreateSkTypeface();
  if (skia_typeface_) {
    return skia_typeface_->isItalic();
  }
#endif
...
uint16_t CFX_Face::GetUnitsPerEm() const {
  ...
  const_cast<CFX_Face*>(this)->GetOrCreateSkTypeface();
  if (skia_typeface_) {
    return pdfium::checked_cast<uint16_t>(skia_typeface_->getUnitsPerEm());
  }
#endif
...
#if defined(PDF_USE_SKIA)
  if (GetOrCreateSkTypeface()) {
    if (buffer.empty()) {
      return skia_typeface_->getTableSize(table);
    }
    return skia_typeface_->getTableData(table, 0, buffer.size(), buffer.data());
  }
#endif
...
int CFX_Face::GetGlyphCount() const {
  ...
  const_cast<CFX_Face*>(this)->GetOrCreateSkTypeface();
  if (skia_typeface_) {
    return skia_typeface_->countGlyphs();
  }
#endif
