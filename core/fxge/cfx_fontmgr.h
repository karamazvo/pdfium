...
class CFX_FontMgr {
 public:
  class SystemFontCacheEntry final : public Retainable, public Observable {
   public:
   ...
   private:
    explicit SystemFontCacheEntry(FixedSizeDataVector<uint8_t> data);
    ~SystemFontCacheEntry() override;
  ...
  RetainPtr<SystemFontCacheEntry> GetCachedFontDesc(const ByteString& face_name,
                                                    int weight,
                                                    bool bItalic);
  RetainPtr<SystemFontCacheEntry> AddCachedFontDesc(
      const ByteString& face_name,
      int weight,
      bool bItalic,
      FixedSizeDataVector<uint8_t> data);

  RetainPtr<SystemFontCacheEntry> GetCachedTTCFontDesc(size_t ttc_size,
                                                       uint32_t checksum);
  RetainPtr<SystemFontCacheEntry> AddCachedTTCFontDesc(
      size_t ttc_size,
      uint32_t checksum,
      FixedSizeDataVector<uint8_t> data);

  RetainPtr<CFX_Face> NewFixedFace(RetainPtr<SystemFontCacheEntry> desc,
 ...
 private:
  ...
  std::map<std::tuple<ByteString, int, bool>, ObservedPtr<SystemFontCacheEntry>>
      face_map_;
  std::map<std::tuple<size_t, uint32_t>, ObservedPtr<SystemFontCacheEntry>>
      ttc_face_map_;
