...
class CFX_FontMgr {
 public:
  class FontDesc final : public Retainable, public Observable {
   ...
   private:
    ...
    std::array<ObservedPtr<CFX_Face>, 16> ttcfaces_;
  };
  ...
  FXFT_LibraryRec* GetFTLibrary() const { return ftlibrary_.get(); }
  bool FTLibrarySupportsHinting() const { return ftlibrary_supports_hinting_; }
 ...
 private:
  ...
  ScopedFXFTLibraryRec const ftlibrary_;
  std::unique_ptr<CFX_FontMapper> builtin_mapper_;
  std::map<std::tuple<ByteString, int, bool>, ObservedPtr<FontDesc>> face_map_;
  std::map<std::tuple<size_t, uint32_t>, ObservedPtr<FontDesc>> ttcface_map_;
  const bool ftlibrary_supports_hinting_;
};
