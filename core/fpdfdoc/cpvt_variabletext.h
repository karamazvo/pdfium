...
class CPVT_VariableText {
 public:
  ...
  void SetPasswordChar(uint16_t wSubWord) { wsub_word_ = wSubWord; }
  ...
  void SetFontSize(float fFontSize) { ffont_size_ = fFontSize; }
  ...
  float GetFontSize() const { return ffont_size_; }
  ...
  uint16_t GetSubWord() const { return wsub_word_; }

  float GetPlateWidth() const { return rc_plate_.right - rc_plate_.left; }
  float GetPlateHeight() const { return rc_plate_.top - rc_plate_.bottom; }
 ...
 private:
  ...
  uint16_t wsub_word_ = 0;
  int32_t limit_char_ = 0;
  int32_t char_array_ = 0;
  int32_t alignment_ = 0;
  float fline_leading_ = 0.0f;
  float ffont_size_ = 0.0f;
  std::vector<std::unique_ptr<CPVT_Section>> section_array_;
  UnownedPtr<Provider> vtprovider_;
  std::unique_ptr<Iterator> vtiterator_;
  CFX_FloatRect rc_plate_;
  CPVT_FloatRect rc_content_;
};
