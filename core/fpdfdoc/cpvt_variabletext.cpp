...
float CPVT_VariableText::GetFontAscent(int32_t nFontIndex,
  ...
  float ascent = vtprovider_ ? vtprovider_->GetTypeAscent(nFontIndex) : 0;
  return ascent * fFontSize * kFontScale;
}

float CPVT_VariableText::GetFontDescent(int32_t nFontIndex,
                                        float fFontSize) const {
  float descent = vtprovider_ ? vtprovider_->GetTypeDescent(nFontIndex) : 0;
...
int CPVT_VariableText::GetCharWidth(int32_t nFontIndex,
  ...
  return vtprovider_->GetCharWidth(nFontIndex, word);
}

int32_t CPVT_VariableText::GetWordFontIndex(uint16_t word,
                                            FX_Charset charset,
                                            int32_t nFontIndex) {
  return vtprovider_ ? vtprovider_->GetWordFontIndex(word, charset, nFontIndex)
                     : -1;
}

int32_t CPVT_VariableText::GetDefaultFontIndex() {
  return vtprovider_ ? vtprovider_->GetDefaultFontIndex() : -1;
}

CPVT_VariableText::Iterator* CPVT_VariableText::GetIterator() {
  if (!vtiterator_) {
    vtiterator_ = std::make_unique<CPVT_VariableText::Iterator>(this);
  }
  return vtiterator_.get();
}
...
CFX_PointF CPVT_VariableText::GetBTPoint() const {
  return CFX_PointF(rc_plate_.left, rc_plate_.top);
}

CFX_PointF CPVT_VariableText::GetETPoint() const {
  return CFX_PointF(rc_plate_.right, rc_plate_.bottom);
}
