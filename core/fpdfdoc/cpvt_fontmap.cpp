...
CPVT_FontMap::CPVT_FontMap(CPDF_Document* pDoc,
    ...
    : document_(pDoc),
      ...
      s_def_font_alias_(sDefFontAlias) {}
...
void CPVT_FontMap::SetupAnnotSysPDFFont() {
  ...
  if (ValidateFontResourceDict(pFontList.Get()) &&
      !pFontList->KeyExist(s_sys_font_alias_)) {
    pFontList->SetNewFor<CPDF_Reference>(s_sys_font_alias_, document_,
...
ByteString CPVT_FontMap::GetPDFFontAlias(int32_t nFontIndex) {
  switch (nFontIndex) {
    case 0:
      return s_def_font_alias_;
    case 1:
      if (!sys_font_) {
        SetupAnnotSysPDFFont();
      }
      return s_sys_font_alias_;
    default:
      return ByteString();
  }
}
