...
CPDF_BAFontMap::CPDF_BAFontMap(CPDF_Document* pDocument,
  ...
  default_font_ = GetAnnotDefaultFont(&s_default_font_name_);
  if (default_font_) {
    auto maybe_charset = default_font_->GetSubstFontCharset();
    if (maybe_charset.has_value()) {
      nCharset = maybe_charset.value();
    } else if (s_default_font_name_ == "Wingdings" ||
               s_default_font_name_ == "Wingdings2" ||
               s_default_font_name_ == "Wingdings3" ||
               s_default_font_name_ == "Webdings") {
      nCharset = FX_Charset::kSymbol;
    } else {
      nCharset = FX_Charset::kANSI;
    }
    AddFontData(default_font_, s_default_font_name_, nCharset);
    AddFontToAnnotDict(default_font_, s_default_font_name_);
  }
...
void CPDF_BAFontMap::AddFontToAnnotDict(const RetainPtr<CPDF_Font>& pFont,
  ...
  if (ToDictionary(pAPDict->GetObjectFor(s_aptype_))) {
    return;
  }

  RetainPtr<CPDF_Stream> stream = pAPDict->GetMutableStreamFor(s_aptype_);
  if (!stream) {
    stream =
        document_->NewIndirect<CPDF_Stream>(document_->New<CPDF_Dictionary>());
    pAPDict->SetNewFor<CPDF_Reference>(s_aptype_, document_,
                                       stream->GetObjNum());
  }
