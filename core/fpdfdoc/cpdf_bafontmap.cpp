...
CPDF_BAFontMap::CPDF_BAFontMap(CPDF_Document* pDocument,
  ...
  default_font_ = GetAnnotDefaultFont(&sdefault_font_name_);
  if (default_font_) {
    auto maybe_charset = default_font_->GetSubstFontCharset();
    if (maybe_charset.has_value()) {
      nCharset = maybe_charset.value();
    } else if (sdefault_font_name_ == "Wingdings" ||
               sdefault_font_name_ == "Wingdings2" ||
               sdefault_font_name_ == "Wingdings3" ||
               sdefault_font_name_ == "Webdings") {
      nCharset = FX_Charset::kSymbol;
    } else {
      nCharset = FX_Charset::kANSI;
    }
    AddFontData(default_font_, sdefault_font_name_, nCharset);
    AddFontToAnnotDict(default_font_, sdefault_font_name_);
  }
...
void CPDF_BAFontMap::AddFontToAnnotDict(const RetainPtr<CPDF_Font>& pFont,
  ...
  if (ToDictionary(pAPDict->GetObjectFor(saptype_))) {
    return;
  }

  RetainPtr<CPDF_Stream> stream = pAPDict->GetMutableStreamFor(saptype_);
  if (!stream) {
    stream =
        document_->NewIndirect<CPDF_Stream>(document_->New<CPDF_Dictionary>());
    pAPDict->SetNewFor<CPDF_Reference>(saptype_, document_,
                                       stream->GetObjNum());
  }
