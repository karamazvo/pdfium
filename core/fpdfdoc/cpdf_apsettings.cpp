...
CPDF_ApSettings::TextPosition CPDF_ApSettings::GetTextPosition() const {
  constexpr int kCaption = fxcrt::to_underlying(TextPosition::kCaption);
  constexpr int kMaxValue = fxcrt::to_underlying(TextPosition::kOverlaid);
  int value = dict_ ? dict_->GetIntegerFor("TP", kCaption) : kCaption;
  if (value < kCaption || value > kMaxValue) {
    value = kCaption;
  }
