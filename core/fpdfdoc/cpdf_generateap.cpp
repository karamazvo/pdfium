...
bool CPDF_GenerateAP::GenerateDefaultAppearanceWithColor(
  ...
  CPDF_DefaultAppearance default_appearance(
      GetDefaultAppearanceString(annot_dict, acroform_dict));
  float font_size = 0;
  auto maybe_font_name = default_appearance.GetFont(&font_size);
  if (!maybe_font_name.has_value()) {
    return false;
  }

  ByteString new_default_appearance_font_name_and_size =
      StringFromFontNameAndSize(maybe_font_name.value(), font_size);
  ...
  return GenerateAnnotAP(doc, annot_dict, CPDF_Annot::Subtype::FREETEXT);
}
