...
bool GenerateFreeTextAP(CPDF_Document* doc, CPDF_Dictionary* annot_dict) {
  ...
  RetainPtr<CPDF_Dictionary> ap_dict =
      annot_dict->GetMutableDictFor(pdfium::annotation::kAP);
  RetainPtr<CPDF_Stream> normal_stream =
      ap_dict ? ap_dict->GetMutableStreamFor("N") : nullptr;
  RetainPtr<CPDF_Dictionary> resources_dict;
  if (normal_stream) {
    ...
    if (!cloned && !ValidateOrCreateFontResources(doc, stream_dict, font_dict,
                                                  font_name)) {
      return false;
    }
  ...
  if (color_array) {
    appearance_stream << "q\n";
    CFX_Color color = fpdfdoc::CFXColorFromArray(*color_array);
    appearance_stream << GenerateColorAP(color, PaintOperation::kFill);
    WriteRect(appearance_stream, rect) << " re f\nQ\n";
  }
  ...
  CFX_PointF offset(0.0f, (content_rect.Height() - rect.Height()) / 2.0f);
  ...
  if (body.GetLength() > 0) {
    ...
    if (content_rect.Width() > rect.Width() ||
        content_rect.Height() > rect.Height()) {
      WriteRect(appearance_stream, rect) << " re\nW\nn\n";
    }
