...
class CJBig2_GRRDProc {
 ...
 private:
  ...
  std::array<pdfium::span<const uint8_t>, 3> GetRowRefs(uint32_t h);
  std::array<pdfium::span<const uint8_t>, 3> GetRowRefsDy(uint32_t h);
  bool TypicalPrediction(int x,
                         int val,
                         pdfium::span<pdfium::span<const uint8_t>, 3> row_refs);
};
