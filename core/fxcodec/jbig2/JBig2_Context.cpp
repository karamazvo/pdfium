...
JBig2_Result CJBig2_Context::DecodeSequential(PauseIndicatorIface* pPause) {
  ...
  while (stream_->getByteLeft() >= JBIG2_MIN_SEGMENT_SIZE) {
    ...
    if (segment_->dwdata_length_ != 0xffffffff) {
      FX_SAFE_UINT32 new_offset = offset_;
      new_offset += segment_->dwdata_length_;
...
CJBig2_Segment* CJBig2_Context::FindReferredTableSegmentByIndex(
  ...
  int32_t count = 0;
  for (int32_t i = 0; i < pSegment->nreferred_to_segment_count_; ++i) {
    CJBig2_Segment* pSeg =
        FindSegmentByNumber(pSegment->referred_to_segment_numbers_[i]);
    if (pSeg && pSeg->c_flags_.s.type == kTableType) {
      if (count == nIndex)
        return pSeg;
      ++count;
    }
...
JBig2_Result CJBig2_Context::ParseSegmentHeader(CJBig2_Segment* pSegment) {
  if (stream_->readInteger(&pSegment->number_) != 0 ||
      stream_->read1Byte(&pSegment->c_flags_.c) != 0) {
    return JBig2_Result::kFailure;
  }
  ...
  if ((cTemp >> 5) == 7) {
    if (stream_->readInteger(
            (uint32_t*)&pSegment->nreferred_to_segment_count_) != 0) {
      return JBig2_Result::kFailure;
    }
    pSegment->nreferred_to_segment_count_ &= 0x1fffffff;
    if (pSegment->nreferred_to_segment_count_ > kJBig2MaxReferredSegmentCount) {
      return JBig2_Result::kFailure;
    }
  } else {
    ...
    pSegment->nreferred_to_segment_count_ = cTemp >> 5;
  }
  uint8_t cSSize = pSegment->number_ > 65536 ? 4
                   : pSegment->number_ > 256 ? 2
                                             : 1;
  uint8_t cPSize = pSegment->c_flags_.s.page_association_size ? 4 : 1;
  if (pSegment->nreferred_to_segment_count_) {
    pSegment->referred_to_segment_numbers_.resize(
        pSegment->nreferred_to_segment_count_);
    for (int32_t i = 0; i < pSegment->nreferred_to_segment_count_; ++i) {
      switch (cSSize) {
        case 1:
          if (stream_->read1Byte(&cTemp) != 0) {
  ...
  if (cPSize == 1) {
    ...
    pSegment->dwpage_association_ = cTemp;
  } else if (stream_->readInteger(&pSegment->dwpage_association_) != 0) {
    return JBig2_Result::kFailure;
  }
  if (stream_->readInteger(&pSegment->dwdata_length_) != 0) {
    return JBig2_Result::kFailure;
  }
...
JBig2_Result CJBig2_Context::ProcessingParseSegmentData(
    CJBig2_Segment* pSegment,
    PauseIndicatorIface* pPause) {
  switch (pSegment->c_flags_.s.type) {
    case 0:
    ...
    case 48: {
      ...
      pPageInfo->w_max_stripe_size_ = striping_info & 0x7fff;
      ...
      if (!buf_specified_) {
        uint32_t height =
            bMaxHeight ? pPageInfo->w_max_stripe_size_ : pPageInfo->height_;
    ...
    case 49:
      ...
      return JBig2_Result::kEndReached;
    case 50:
      stream_->addOffset(pSegment->dwdata_length_);
      break;
    case 51:
      return JBig2_Result::kEndReached;
    case 52:
...
JBig2_Result CJBig2_Context::ParseHalftoneRegion(CJBig2_Segment* pSegment,
  ...
  if (pSegment->nreferred_to_segment_count_ != 1) {
    return JBig2_Result::kFailure;
  }

  CJBig2_Segment* pSeg =
      FindSegmentByNumber(pSegment->referred_to_segment_numbers_[0]);
  if (!pSeg || (pSeg->c_flags_.s.type != 16)) {
    return JBig2_Result::kFailure;
  }
  ...
  if (pSegment->c_flags_.s.type != 20) {
    if (!buf_specified_) {
      const auto& pPageInfo = page_info_list_.back();
      if (pPageInfo->is_striped_ && ri.y + ri.height > page_->height()) {
        page_->Expand(ri.y + ri.height, pPageInfo->default_pixel_value_);
...
JBig2_Result CJBig2_Context::ParseGenericRegion(CJBig2_Segment* pSegment,
  ...
  if (grd_->MMR) {
    ...
    }
    {
      ...
      if (processing_status_ == FXCODEC_STATUS::kDecodeToBeContinued) {
        if (pSegment->c_flags_.s.type != 36) {
          if (!buf_specified_) {
            const auto& pPageInfo = page_info_list_.back();
...
JBig2_Result CJBig2_Context::ParseGenericRefinementRegion(
  ...
  if (pSegment->nreferred_to_segment_count_ > 0) {
    int32_t i;
    for (i = 0; i < pSegment->nreferred_to_segment_count_; ++i) {
      pSeg = FindSegmentByNumber(pSegment->referred_to_segment_numbers_[0]);
      if (!pSeg)
        return JBig2_Result::kFailure;

      if (pSeg->c_flags_.s.type == 4 || pSeg->c_flags_.s.type == 20 ||
          pSeg->c_flags_.s.type == 36 || pSeg->c_flags_.s.type == 40) {
        break;
      }
    }
    if (i >= pSegment->nreferred_to_segment_count_) {
      return JBig2_Result::kFailure;
    }
  ...
  stream_->alignByte();
  stream_->addOffset(2);
  if (pSegment->c_flags_.s.type != 40) {
    if (!buf_specified_) {
      JBig2PageInfo* pPageInfo = page_info_list_.back().get();
      if (pPageInfo->is_striped_ && ri.y + ri.height > page_->height()) {
        page_->Expand(ri.y + ri.height, pPageInfo->default_pixel_value_);
