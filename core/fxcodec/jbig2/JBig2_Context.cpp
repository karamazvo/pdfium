...
CJBig2_Segment* CJBig2_Context::FindReferredTableSegmentByIndex(
  ...
  for (int32_t i = 0; i < pSegment->referred_to_segment_count_; ++i) {
    ...
    if (pSeg && pSeg->cflags_.s.type == kTableType) {
      if (count == nIndex)
        return pSeg;
      ++count;
    }
...
JBig2_Result CJBig2_Context::ParseSegmentHeader(CJBig2_Segment* pSegment) {
  if (stream_->readInteger(&pSegment->dw_number_) != 0 ||
      stream_->read1Byte(&pSegment->cflags_.c) != 0) {
    return JBig2_Result::kFailure;
  }
  ...
  uint8_t cSSize = pSegment->dw_number_ > 65536 ? 4
                   : pSegment->dw_number_ > 256 ? 2
                                                : 1;
  uint8_t cPSize = pSegment->cflags_.s.page_association_size ? 4 : 1;
  if (pSegment->referred_to_segment_count_) {
    ...
    for (int32_t i = 0; i < pSegment->referred_to_segment_count_; ++i) {
      ...
      if (pSegment->referred_to_segment_numbers_[i] >= pSegment->dw_number_) {
        return JBig2_Result::kFailure;
      }
    }
  }
  ...
  pSegment->dw_data_offset_ = stream_->getOffset();
...
JBig2_Result CJBig2_Context::ProcessingParseSegmentData(
    CJBig2_Segment* pSegment,
    PauseIndicatorIface* pPause) {
  switch (pSegment->cflags_.s.type) {
    case 0:
    ...
    case 48: {
      ...
      if (stream_->readInteger(&pPageInfo->dw_width_) != 0 ||
          stream_->readInteger(&pPageInfo->dw_height_) != 0 ||
          stream_->readInteger(&pPageInfo->dw_resolution_x_) != 0 ||
          stream_->readInteger(&pPageInfo->dw_resolution_y_) != 0 ||
      ...
      pPageInfo->wmax_stripe_size_ = striping_info & 0x7fff;
      bool bMaxHeight = (pPageInfo->dw_height_ == 0xffffffff);
      ...
      if (!buf_specified_) {
        uint32_t height =
            bMaxHeight ? pPageInfo->wmax_stripe_size_ : pPageInfo->dw_height_;
        page_ = std::make_unique<CJBig2_Image>(pPageInfo->dw_width_, height);
      }
...
JBig2_Result CJBig2_Context::ParseSymbolDict(CJBig2_Segment* pSegment) {
  ...
  CJBig2_CompoundKey key(pSegment->key_, pSegment->dw_data_offset_);
...
JBig2_Result CJBig2_Context::ParseHalftoneRegion(CJBig2_Segment* pSegment,
  ...
  if (!pSeg || (pSeg->cflags_.s.type != 16)) {
    return JBig2_Result::kFailure;
  }
...
JBig2_Result CJBig2_Context::ParseGenericRefinementRegion(
  ...
  if (pSegment->referred_to_segment_count_ > 0) {
    ...
    for (i = 0; i < pSegment->referred_to_segment_count_; ++i) {
      ...
      if (pSeg->cflags_.s.type == 4 || pSeg->cflags_.s.type == 20 ||
          pSeg->cflags_.s.type == 36 || pSeg->cflags_.s.type == 40) {
        break;
      }
    }
