...
pdfium::span<const uint8_t> CJBig2_Image::span() const {
  // SAFETY: If `data_` is owned, then `this` must allocate the right amount.
  // If `data_` is not owned, then safety requires correctness from the caller
  // that constructed `this`.
...
void CJBig2_Image::Expand(int32_t h, bool v) {
  ...
  // NOTE: Must make a span() call here, as the span may have changed due to
  // memory (re-)allocation above.
