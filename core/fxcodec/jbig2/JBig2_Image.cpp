  ...
  switch (compose_to_op) {
    case ComposeToOp::kDestAlignedSrcAlignedSrcGreaterThanDest: {
      ...
      for (int32_t i = 0; i < h; ++i) {
        auto src = fxcrt::reinterpret_span<const uint32_t>(
            GetLine(src_start_line + i));
        auto dest = fxcrt::reinterpret_span<uint32_t>(
            pDst->GetLine(dest_start_line + i));
    ...
    case ComposeToOp::kDestAlignedSrcNotAligned: {
      ...
      for (int32_t i = 0; i < h; ++i) {
        ...
        uint32_t src_val1 = FromBE32(src.take_first_elem()) << shift1;
        uint32_t src_val2 = FromBE32(src.front()) >> shift2;
        uint32_t& dest_elem = dest.subspan(dest_offset).front();
        dest_elem = FromBE32(DoComposeWithMask(op, src_val1 | src_val2,
                                               FromBE32(dest_elem), maskM));
      }
      return true;
    }
    case ComposeToOp::kDestNotAlignedSrcGreaterThanDest: {
      ...
      for (int32_t i = 0; i < h; ++i) {
        ...
        if (d1 != 0) {
          auto src_val1 = FromBE32(src.take_first_elem()) << shift1;
          auto src_val2 = FromBE32(src.front()) >> shift2;
          uint32_t& dest_elem = dest.take_first<1u>().front();
          dest_elem = FromBE32(DoComposeWithMask(op, src_val1 | src_val2,
                                                 FromBE32(dest_elem), maskL));
        }
        for (int32_t xx = 0; xx < middleDwords; xx++) {
          ...
          uint32_t& dest_elem = dest.take_first<1u>().front();
          dest_elem =
              FromBE32(DoCompose(op, src_val1 | src_val2, FromBE32(dest_elem)));
        }
        if (d2 != 0) {
    ...
    case ComposeToOp::kDestNotAlignedSrcLessThanDest: {
      ...
      for (int32_t i = 0; i < h; ++i) {
        ...
        for (int32_t xx = 0; xx < middleDwords; xx++) {
          uint32_t src_val1 = FromBE32(src.take_first_elem()) << shift2;
          uint32_t src_val2 = FromBE32(src.front()) >> shift1;
          uint32_t& dest_elem = dest.take_first<1u>().front();
          dest_elem =
              FromBE32(DoCompose(op, src_val1 | src_val2, FromBE32(dest_elem)));
        }
