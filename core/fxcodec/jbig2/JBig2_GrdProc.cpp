  ...
  for (uint32_t h = 0; h < height; ++h) {
    ...
    if (h > 1) {
      pdfium::span<const uint8_t> row_prev2 =
          GBREG->GetLine(h - 2).subspan<1u>();
      pdfium::span<const uint8_t> row_prev1 =
          GBREG->GetLine(h - 1).subspan<1u>();
      uint32_t val_prev2 = row_prev2.front() << kOptConstant2[OPT];
      uint32_t val_prev1 = row_prev1.front();
      uint32_t CONTEXT =
          (val_prev2 & kOptConstant3[OPT]) |
          ((val_prev1 >> kOptConstant4[OPT]) & kOptConstant5[OPT]);
      for (int32_t cc = 0; cc < nLineBytes; ++cc) {
        row_prev2 = row_prev2.subspan<1u>();
        val_prev2 =
            (val_prev2 << 8) | (row_prev2.front() << kOptConstant2[OPT]);
        row_prev1 = row_prev1.subspan<1u>();
        val_prev1 = (val_prev1 << 8) | row_prev1.front();
        uint8_t cVal = 0;
    ...
    pdfium::span<const uint8_t> row_prev = GBREG->GetLine(h - 1).subspan<1u>();
    uint32_t line2 = (h & 1) ? row_prev.front() : 0;
    uint32_t CONTEXT = ((line2 >> kOptConstant4[OPT]) & kOptConstant5[OPT]);
    for (int32_t cc = 0; cc < nLineBytes; ++cc) {
      if (h & 1) {
        row_prev = row_prev.subspan<1u>();
        line2 = (line2 << 8) | row_prev.front();
      }
      uint8_t cVal = 0;
      for (int32_t k = 7; k >= 0; --k) {
        if (pArithDecoder->IsComplete()) {
...
std::unique_ptr<CJBig2_Image> CJBig2_GRDProc::DecodeArithTemplate3Opt3(
  ...
  for (uint32_t h = 0; h < GBH; h++) {
    ...
    if (h > 0) {
      pdfium::span<uint8_t> row_prev = GBREG->GetLine(h - 1).subspan<1u>();
      uint32_t val_prev = row_prev.front();
      uint32_t CONTEXT = (val_prev >> 1) & 0x03f0;
      for (int32_t cc = 0; cc < nLineBytes; cc++) {
        row_prev = row_prev.subspan<1u>();
        val_prev = (val_prev << 8) | row_prev.front();
        uint8_t cVal = 0;
        for (int32_t k = 7; k >= 0; k--) {
          if (pArithDecoder->IsComplete()) {
            return nullptr;
...
std::unique_ptr<CJBig2_Image> CJBig2_GRDProc::DecodeArithTemplate3Unopt(
  ...
  for (uint32_t h = 0; h < GBH; h++) {
    ...
    uint32_t prev_value = GBREG->GetPixel(1, row_prev);
    prev_value |= GBREG->GetPixel(0, row_prev) << 1;
    uint32_t val_current = 0;
    for (uint32_t w = 0; w < GBW; w++) {
      int bVal = 0;
      if (!USESKIP || !SKIP->GetPixel(w, row_skip)) {
      ...
      prev_value =
          ((prev_value << 1) | GBREG->GetPixel(w + 2, row_prev)) & 0x1f;
...
FXCODEC_STATUS CJBig2_GRDProc::ProgressiveDecodeArithTemplate3Unopt(
  ...
  for (; loop_index_ < GBH; loop_index_++) {
    ...
    if (ltp_) {
      ...
      uint32_t prev_value = pImage->GetPixel(1, row_prev);
      prev_value |= pImage->GetPixel(0, row_prev) << 1;
      uint32_t val_current = 0;
      for (uint32_t w = 0; w < GBW; w++) {
        int bVal = 0;
        if (!USESKIP || !SKIP->GetPixel(w, row_skip)) {
        ...
        prev_value =
            ((prev_value << 1) | pImage->GetPixel(w + 2, row_prev)) & 0x1f;
        val_current = ((val_current << 1) | bVal) & 0x0f;
      }
    }
    if (pState->pPause && pState->pPause->NeedToPauseNow()) {
