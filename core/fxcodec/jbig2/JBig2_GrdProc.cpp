  ...
  UNSAFE_TODO({
    for (; loop_index_ < height; loop_index_++) {
      ...
      if (ltp_) {
        ...
        if (loop_index_ > 1) {
          pdfium::raw_span<const uint8_t> line_prev2 = line_prev2_;
          pdfium::raw_span<const uint8_t> line_prev1 = line_prev1_;
          ...
          pdfium::raw_span<const uint8_t> line_prev = line_prev1_;
          uint32_t val_prev =
              (loop_index_ & 1) ? line_prev.take_first_elem() : 0;
...
FXCODEC_STATUS CJBig2_GRDProc::ProgressiveDecodeArithTemplate3Opt3(
  ...
  UNSAFE_TODO({
    for (; loop_index_ < GBH; loop_index_++) {
      ...
      line_prev2_ = line_prev1_;
      line_prev1_ = line_;
      line_ = line_.subspan(static_cast<size_t>(pImage->stride()));
      if (pState->pPause && pState->pPause->NeedToPauseNow()) {
        loop_index_++;
        progressive_status_ = FXCODEC_STATUS::kDecodeToBeContinued;
        return FXCODEC_STATUS::kDecodeToBeContinued;
...
void CJBig2_GRDProc::AdvanceLine(CJBig2_Image* image) {
  line_prev2_ = line_prev1_;
  line_prev1_ = line_;
  line_ = line_.subspan(static_cast<size_t>(image->stride()));
}
