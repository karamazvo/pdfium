...
class ProgressiveDecoder final :
 ...
 private:
  ...
  int32_t GetBitsPerPixel() const {
    return src_number_of_components_ * src_bits_per_component_;
  }
  ...
  int src_width_ = 0;
  int src_height_ = 0;
  int src_number_of_components_ = 0;  // (e.g. 4 for RGBA, 3 for RGB)
  int src_bits_per_component_ = 0;    // (e.g. bits per red or bits per alpha)
