...
void IccTransform::Translate(pdfium::span<const float> pSrcValues,
  ...
  // apply some member of htransform_ to the input. We need to go over all the
  ...
  if (lab_) {
    ...
    cmsDoTransform(htransform_, inputs.data(), output, 1);
  } else {
...
void IccTransform::TranslateScanline(pdfium::span<uint8_t> pDest,
                                     ...
                                     int32_t pixels) {
  cmsDoTransform(htransform_, pSrc.data(), pDest.data(), pixels);
}
