...
IccTransform::IccTransform(cmsHTRANSFORM hTransform,
                           ...
                           bool bIsLab,
                           bool bNormal)
    : h_transform_(hTransform),
      src_components_(srcComponents),
      lab_(bIsLab),
      normal_(bNormal) {}

IccTransform::~IccTransform() {
  cmsDeleteTransform(h_transform_);
}
...
void IccTransform::Translate(pdfium::span<const float> pSrcValues,
  ...
  // apply some member of h_transform_ to the input. We need to go over all the
  ...
  if (lab_) {
    ...
    cmsDoTransform(h_transform_, inputs.data(), output, 1);
  } else {
...
void IccTransform::TranslateScanline(pdfium::span<uint8_t> pDest,
                                     ...
                                     int32_t pixels) {
  cmsDoTransform(h_transform_, pSrc.data(), pDest.data(), pixels);
}
