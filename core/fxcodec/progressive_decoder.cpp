...
bool ProgressiveDecoder::PngAskScanlineBuf(int line, uint8_t** pSrcBuf) {
  ...
  const size_t byte_size = Fx2DSizeOrDie(m_SrcWidth, 4);
  fxcrt::spancpy(dest_span.first(byte_size), src_span.first(byte_size));
...
void ProgressiveDecoder::PngFillScanlineBufCompleted(int pass, int line) {
  ...
  fxcrt::spancpy(dest_span.first(m_SrcWidth), src_span.first(m_SrcWidth));
}
#endif  // PDF_ENABLE_XFA_PNG
