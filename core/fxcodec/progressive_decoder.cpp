...
bool ProgressiveDecoder::ReadMoreData(ProgressiveDecoderIface* module,
  ...
  FX_SAFE_SIZE_T avail_input = module->GetAvailInput(context);
  ...
  if (unconsumed_bytes == codec_memory_->GetSize()) {
    ...
    codec_memory_->Consume(already_read_bytes);
  ...
  return module->Input(context, codec_memory_);
}
