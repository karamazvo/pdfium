...
bool ProgressiveDecoder::PngReadHeader(int width,
  ...
  if (!device_bitmap_) {
    ...
    src_number_of_components_ = number_of_components;
    return false;
  }
...
bool ProgressiveDecoder::BmpDetectImageTypeInBuffer(
  ...
  BmpDecoder::Status read_result = BmpDecoder::ReadHeader(
      ...
      &src_number_of_components_, &palette, pAttribute);
  ...
  switch (src_number_of_components_) {
    case 1:
...
FXCODEC_STATUS ProgressiveDecoder::BmpStartDecode() {
  SetTransMethod();
  scanline_size_ = FxAlignToBoundary<4>(src_width_ * src_number_of_components_);
...
bool ProgressiveDecoder::JpegDetectImageTypeInBuffer(
  ...
  while (1) {
    int read_result = JpegProgressiveDecoder::ReadHeader(
        jpeg_context_.get(), &src_width_, &src_height_,
        &src_number_of_components_, pAttribute);
    switch (read_result) {
...
FXCODEC_STATUS ProgressiveDecoder::JpegStartDecode() {
  ...
  decode_buf_.resize(
      FxAlignToBoundary<4>(src_width_ * src_number_of_components_));
  FXDIB_ResampleOptions options;
  options.bInterpolateBilinear = true;
  weight_horz_.CalculateWeights(src_width_, 0, src_width_, src_width_, 0,
                                src_width_, options);
...
bool ProgressiveDecoder::TiffDetectImageTypeFromFile(
  ...
  int32_t dummy_bpc;
  bool ret = TiffDecoder::LoadFrameInfo(
      tiff_context_.get(), 0, &src_width_, &src_height_,
      &src_number_of_components_, &dummy_bpc, pAttribute);
  src_number_of_components_ = 4;
  if (!ret) {
    tiff_context_.reset();
    status_ = FXCODEC_STATUS::kError;
