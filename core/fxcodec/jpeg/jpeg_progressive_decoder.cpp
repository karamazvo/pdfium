...
class CJpegContext final : public ProgressiveDecoderIface::Context {
 public:
  ...
  jmp_buf& GetJumpMark() { return m_Common.m_JmpBuf; }
...
static void error_fatal(j_common_ptr cinfo) {
  ...
  longjmp(pContext->m_Common.m_JmpBuf, -1);
}
...
CJpegContext::CJpegContext() {
  m_Common.m_Cinfo.client_data = this;
...
JpegProgressiveDecoder::Start() {
  ...
  if (setjmp(pContext->m_Common.m_JmpBuf) == -1) {
    delete pContext;
    return nullptr;
  }

  jpeg_create_decompress(&pContext->m_Common.m_Cinfo);
  pContext->m_Common.m_Cinfo.src = &pContext->m_Common.m_SrcMgr;
...
int JpegProgressiveDecoder::ReadHeader(Context* pContext,
  ...
  int ret = jpeg_read_header(&ctx->m_Common.m_Cinfo, TRUE);
  if (ret == JPEG_SUSPENDED)
    return 2;
  if (ret != JPEG_HEADER_OK)
    return 1;

  *width = ctx->m_Common.m_Cinfo.image_width;
  *height = ctx->m_Common.m_Cinfo.image_height;
  *nComps = ctx->m_Common.m_Cinfo.num_components;
  JpegLoadAttribute(ctx->m_Common.m_Cinfo, pAttribute);
  return 0;
}
...
bool JpegProgressiveDecoder::StartScanline(Context* pContext) {
  auto* ctx = static_cast<CJpegContext*>(pContext);
  ctx->m_Common.m_Cinfo.scale_denom = 1;
  return !!jpeg_start_decompress(&ctx->m_Common.m_Cinfo);
}
...
bool JpegProgressiveDecoder::ReadScanline(Context* pContext,
  ...
  auto* ctx = static_cast<CJpegContext*>(pContext);
  unsigned int nlines =
      jpeg_read_scanlines(&ctx->m_Common.m_Cinfo, &dest_buf, 1);
  return nlines == 1;
}

FX_FILESIZE JpegProgressiveDecoder::GetAvailInput(Context* pContext) const {
  auto* ctx = static_cast<CJpegContext*>(pContext);
  return static_cast<FX_FILESIZE>(ctx->m_Common.m_SrcMgr.bytes_in_buffer);
}
...
bool JpegProgressiveDecoder::Input(Context* pContext,
  ...
  if (ctx->m_SkipSize) {
    if (ctx->m_SkipSize > src_buf.size()) {
      ctx->m_Common.m_SrcMgr.bytes_in_buffer = 0;
  ...
  ctx->m_Common.m_SrcMgr.next_input_byte = src_buf.data();
  ctx->m_Common.m_SrcMgr.bytes_in_buffer = src_buf.size();
  return true;
}
