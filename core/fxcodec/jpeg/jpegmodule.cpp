...
JpegDecoder::~JpegDecoder() {
  if (m_bInited)
    jpeg_destroy_decompress(&m_Common.m_Cinfo);
...
bool JpegDecoder::InitDecode(bool bAcceptKnownBadHeader) {
  ...
  jpeg_create_decompress(&m_Common.m_Cinfo);
  InitDecompressSrc();
  m_bInited = true;
  ...
  m_Common.m_Cinfo.image_width = m_OrigWidth;
  m_Common.m_Cinfo.image_height = m_OrigHeight;
  int ret = jpeg_read_header(&m_Common.m_Cinfo, TRUE);
  if (ret != JPEG_HEADER_OK)
    return false;

  if (m_Common.m_Cinfo.saw_Adobe_marker) {
    m_bJpegTransform = true;
  }

  if (m_Common.m_Cinfo.num_components == 3 && !m_bJpegTransform) {
    m_Common.m_Cinfo.out_color_space = m_Common.m_Cinfo.jpeg_color_space;
  }

  m_OrigWidth = m_Common.m_Cinfo.image_width;
  m_OrigHeight = m_Common.m_Cinfo.image_height;
  m_OutputWidth = m_OrigWidth;
  m_OutputHeight = m_OrigHeight;
  m_nDefaultScaleDenom = m_Common.m_Cinfo.scale_denom;
  return true;
}
...
bool JpegDecoder::Create(pdfium::span<const uint8_t> src_span,
  ...
  if (m_Common.m_Cinfo.num_components < nComps) {
    return false;
  }

  if (m_Common.m_Cinfo.image_width < width) {
    return false;
  }

  CalcPitch();
  m_ScanlineBuf = DataVector<uint8_t>(m_Pitch);
  m_nComps = m_Common.m_Cinfo.num_components;
  m_bpc = 8;
  m_bStarted = false;
  return true;
}
...
bool JpegDecoder::Rewind() {
  ...
  m_Common.m_Cinfo.scale_denom = m_nDefaultScaleDenom;
  m_OutputWidth = m_OrigWidth;
  m_OutputHeight = m_OrigHeight;
  if (!jpeg_start_decompress(&m_Common.m_Cinfo)) {
    jpeg_destroy_decompress(&m_Common.m_Cinfo);
    return false;
  }
  CHECK_LE(static_cast<int>(m_Common.m_Cinfo.output_width), m_OrigWidth);
  m_bStarted = true;
  return true;
}
...
pdfium::span<uint8_t> JpegDecoder::GetNextLine() {
  ...
  int nlines = jpeg_read_scanlines(&m_Common.m_Cinfo, row_array, 1);
  if (nlines <= 0)
    return pdfium::span<uint8_t>();
...
void JpegDecoder::CalcPitch() {
  m_Pitch = static_cast<uint32_t>(m_Common.m_Cinfo.image_width) *
            m_Common.m_Cinfo.num_components;
  m_Pitch += 3;
  m_Pitch /= 4;
  m_Pitch *= 4;
}
...
bool JpegDecoder::HasKnownBadHeaderWithInvalidHeight(
  ...
  bool bDimensionChecks =
      m_Common.m_Cinfo.err->msg_code == JERR_IMAGE_TOO_BIG &&
      m_Common.m_Cinfo.image_width < JPEG_MAX_DIMENSION &&
      m_Common.m_Cinfo.image_height == 0xffff && m_OrigWidth > 0 &&
