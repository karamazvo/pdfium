 ...
 public:
  class Delegate {
   public:
    ...
    // * `pass`: interlacing pass (TODO: provide better description, one that
    //   matches actual usage in `ProgressiveDecoder`)
    // * `src_color_type` is the color type the image has been encoded with
    //
    // Implemenation should:
    //
    // * Return `true` upon success (and `false` otherwise)
    // * Set `*dst_color_type` to the color type to decode into
    // * Set `*gamma` to the target gamma to decode for
    //   (TODO?: support full target color profile via `SkColorSpace`?)
