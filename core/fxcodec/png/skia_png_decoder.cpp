...
void CSkiaPngContext::HandleDecodedRow(int row_number) {
  ...
  if (intermediate_bgra_buffer_.has_value()) {
    ...
    CHECK_EQ(width * 4, skia_output_buffer_->rowBytes());
    ...
    while (!src_bgra.empty()) {
      dst_bgr[0] = src_bgra[0];
      dst_bgr[1] = src_bgra[1];
      dst_bgr[2] = src_bgra[2];
      // Intentionally ignoring/dropping `src_bgra[3]` - the alpha channel.

      src_bgra = src_bgra.subspan(4);
      dst_bgr = dst_bgr.subspan(3);
    }
