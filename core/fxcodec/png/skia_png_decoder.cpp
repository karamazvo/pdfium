...
int GetNumberOfComponents(const SkImageInfo& info) {
...
bool SkiaPngContext::CreateDecoder() {
  ...
  int number_of_components = GetNumberOfComponents(info);
  int bits_per_component = (info.bytesPerPixel() * 8) / number_of_components;
  ...
  if (!delegate_->PngReadHeader(info.width(), info.height(), bits_per_component,
                                number_of_components, kPass, &target_color_type,
                                &target_gamma)) {
    ResetAfterFatalDecodingFailure();
    return false;
  }
