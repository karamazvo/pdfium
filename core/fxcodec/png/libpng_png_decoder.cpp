...
void _png_get_row_func(png_structp png_ptr,
  ...
  if (((pass + 1) == pContext->number_of_passes_) &&
      ((row_num + 1) == pContext->height_)) {
...
bool LibpngPngDecoder::ContinueDecode(
  ...
  // `libpng` always consumes all the data from `GetUnconsumedSpan`, so
  // advance/seek `codec_memory` to the end of the buffer.
  codec_memory->Seek(codec_memory->GetSize());
  CHECK_EQ(codec_memory->GetUnconsumedSpan().size(), 0u);
