...
class CPngContext final : public ProgressiveDecoderIface::Context {
 public:
  ...
  char sz_last_error_[PNG_ERROR_SIZE] = {};
};
  ...
  if (pAttribute) {
    ...
    switch (unit_type) {
      case PNG_RESOLUTION_METER:
        pAttribute->wdpiunit_ = CFX_DIBAttribute::kResUnitMeter;
        break;
      default:
        pAttribute->wdpiunit_ = CFX_DIBAttribute::kResUnitNone;
    }
#endif
...
std::unique_ptr<ProgressiveDecoderIface::Context> PngDecoder::StartDecode(
  ...
  if (!_png_set_read_and_error_fns(p->png_, p.get(), p->sz_last_error_)) {
    return nullptr;
  }
  return p;
}
...
bool PngDecoder::ContinueDecode(ProgressiveDecoderIface::Context* pContext,
  ...
  if (!_png_continue_decode(ctx->png_, ctx->info_, src_buf.data(),
                            src_buf.size())) {
    if (pAttribute && UNSAFE_TODO(strcmp(ctx->sz_last_error_,
