...
TEST(fxcodec, RLEShortInput) {
  ...
  auto dest_buf = BasicModule::RunLengthEncode(src_buf);
  auto dest_span = dest_buf.span();
  ASSERT_EQ(3u, dest_span.size());
  EXPECT_EQ(0, dest_span[0]);
  EXPECT_EQ(1, dest_span[1]);
  EXPECT_EQ(128, dest_span[2]);
}
...
TEST(fxcodec, RLENormalInputs) {
  {
    ...
    auto dest_buf = BasicModule::RunLengthEncode(src_buf_1);
  ...
  }
  {
    ...
    auto dest_buf = BasicModule::RunLengthEncode(src_buf_2);
    DataAndBytesConsumed result = RunLengthDecode(dest_buf);
  ...
  }
  {
    ...
    auto dest_buf = BasicModule::RunLengthEncode(src_buf_3);
    DataAndBytesConsumed result = RunLengthDecode(dest_buf);
...
TEST(fxcodec, RLEFullLengthInputs) {
  ...
  }
  {
    ...
    auto dest_buf = BasicModule::RunLengthEncode(src_buf_4);
    DataAndBytesConsumed result = RunLengthDecode(dest_buf);
