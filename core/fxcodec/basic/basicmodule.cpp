...
bool RLScanlineDecoder::CheckDestSize() {
  ...
  if (((uint32_t)orig_width_ * comps_ * pc_ * orig_height_ + 7) / 8 >
      dest_size) {
    return false;
  }
  return true;
...
bool RLScanlineDecoder::Create(pdfium::span<const uint8_t> src_buf,
  ...
  dw_line_bytes_ = (static_cast<uint32_t>(width) * nComps * bpc + 7) / 8;
...
pdfium::span<uint8_t> RLScanlineDecoder::GetNextLine() {
  ...
  while (src_offset_ < src_buf_.size() && !eol) {
    if (operator_ < 128) {
      ...
      if (col_pos + copy_len >= dw_line_bytes_) {
        copy_len = pdfium::checked_cast<uint32_t>(dw_line_bytes_ - col_pos);
        eol = true;
      }
      ...
      if (col_pos + duplicate_len >= dw_line_bytes_) {
        duplicate_len =
            pdfium::checked_cast<uint32_t>(dw_line_bytes_ - col_pos);
        eol = true;
      }
