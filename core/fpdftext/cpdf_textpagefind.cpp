...
CPDF_TextPageFind::CPDF_TextPageFind(
    ...
    : text_page_(pTextPage),
      ...
      cs_find_what_array_(findwhat_array),
      options_(options) {
...
bool CPDF_TextPageFind::FindFirst() {
  return str_text_.IsEmpty() || !cs_find_what_array_.empty();
}
...
bool CPDF_TextPageFind::FindNext() {
  ...
  int nCount = fxcrt::CollectionSize<int>(cs_find_what_array_);
  std::optional<size_t> nResultPos = 0;
  bool bSpaceStart = false;
  for (int iWord = 0; iWord < nCount; iWord++) {
    WideString csWord = cs_find_what_array_[iWord];
    if (csWord.IsEmpty()) {
      if (iWord == nCount - 1) {
        if (nStartPos >= strLen) {
          return false;
    ...
    if (iWord != 0 && !bSpaceStart) {
      ...
      WideString lastWord = cs_find_what_array_[iWord - 1];
    ...
    if (bMatch) {
      ...
      nStartPos = res_start_ + cs_find_what_array_[index].GetLength();
    }
  }
  res_end_ = nResultPos.value() + cs_find_what_array_.back().GetLength() - 1;
...
bool CPDF_TextPageFind::FindPrev() {
  ...
  CPDF_TextPageFind find_engine(text_page_, cs_find_what_array_, options_, 0);
