...
class CPDF_FontSubsetter {
 public:
  ...
  // Given `new_obj_nums`, the collection of all new object numbers, returns a
  // map of object numbers to new PDF objects that should override the original
  // objects in order to subset any embedded fonts for new text.
  std::map<uint32_t, RetainPtr<const CPDF_Object>> GenerateObjectOverrides(
      pdfium::span<uint32_t> new_obj_nums);
