...
CPDF_Creator::CPDF_Creator(CPDF_Document* pDoc,
    ...
    : document_(pDoc),
      ...
      dw_last_obj_num_(document_->GetLastObjNum()),
...
bool CPDF_Creator::WriteOldObjs() {
  ...
  // If there are no new objects to write, then adjust `dw_last_obj_num_` if
  // needed to reflect the actual last object number.
  if (new_obj_num_array_.empty()) {
    dw_last_obj_num_ = last_object_number_written;
  }
  return true;
}
...
CPDF_Creator::Stage CPDF_Creator::WriteDoc_Stage2() {
  ...
  if (stage_ == Stage::kWriteEncryptDict27) {
    if (encrypt_dict_ && encrypt_dict_->IsInline()) {
      ...
      if (!WriteIndirectObj(dw_last_obj_num_, encrypt_dict_.Get())) {
        return Stage::kInvalid;
      }

      object_offsets_[dw_last_obj_num_] = saveOffset;
      if (is_incremental_) {
        new_obj_num_array_.push_back(dw_last_obj_num_);
      }
    }
    stage_ = Stage::kInitWriteXRefs80;
  }
...
CPDF_Creator::Stage CPDF_Creator::WriteDoc_Stage3() {
  ...
  uint32_t dwLastObjNum = dw_last_obj_num_;
...
CPDF_Creator::Stage CPDF_Creator::WriteDoc_Stage4() {
  ...
  if (!archive_->WriteString("/Size ") ||
      !archive_->WriteDWord(dw_last_obj_num_ + (bXRefStream ? 2 : 1))) {
    return Stage::kInvalid;
  }
  if (is_incremental_) {
  ...
  if (idarray_) {
    if (!archive_->WriteString(("/ID")) ||
        !idarray_->WriteTo(archive_.get(), nullptr)) {
      return Stage::kInvalid;
    }
  }
  if (!bXRefStream) {
    ...
    if (is_incremental_ && parser_ && parser_->GetLastXRefOffset() == 0) {
      ...
      if (!archive_->WriteString("]/Length ") ||
          !archive_->WriteDWord(dw_last_obj_num_ * 5) ||
...
void CPDF_Creator::InitID() {
  ...
  idarray_ = pdfium::MakeRetain<CPDF_Array>();
  ...
  if (pID1) {
    idarray_->Append(pID1->Clone());
  } else {
    std::array<uint32_t, 4> file_id =
        GenerateFileID((uint32_t)(uintptr_t)this, dw_last_obj_num_);
    idarray_->AppendNew<CPDF_String>(pdfium::as_byte_span(file_id),
  ...
  idarray_->Append(idarray_->GetObjectAt(0)->Clone());
  if (encrypt_dict_) {
    DCHECK(parser_);
    ...
    if ((revision == 2 || revision == 3) &&
      ...
      security_handler_->OnCreate(new_encrypt_dict_.Get(), idarray_.Get(),
