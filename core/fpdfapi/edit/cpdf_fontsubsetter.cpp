// Copyright 2026 The PDFium Authors
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

#include "core/fpdfapi/edit/cpdf_fontsubsetter.h"
#include <iostream>
#include "core/fpdfapi/font/cpdf_font.h"
#include "core/fpdfapi/page/cpdf_page.h"
#include "core/fpdfapi/page/cpdf_textobject.h"
#include "core/fpdfapi/parser/cpdf_array.h"
#include "core/fpdfapi/parser/cpdf_dictionary.h"
#include "core/fpdfapi/parser/cpdf_document.h"
#include "core/fpdfapi/parser/cpdf_name.h"
#include "core/fpdfapi/parser/cpdf_number.h"
#include "core/fpdfapi/parser/cpdf_string.h"
#include "hb-subset.h"
#include "hb.h"

namespace {

template <auto DestroyFunction>
struct HbDeleter {
  template <typename T>
  void operator()(T* ptr) const {
    DestroyFunction(ptr);
  }
};

using HBBlob = std::unique_ptr<hb_blob_t, HbDeleter<hb_blob_destroy>>;
using HBFace = std::unique_ptr<hb_face_t, HbDeleter<hb_face_destroy>>;
using HBSubsetInput =
    std::unique_ptr<hb_subset_input_t, HbDeleter<hb_subset_input_destroy>>;

std::vector<uint8_t> GenerateFontSubset(pdfium::span<const uint8_t> font_data,
                                        const std::set<uint32_t>& gids) {
  // Wrap the data.
  HBBlob blob(
      hb_blob_create_or_fail(reinterpret_cast<const char*>(font_data.data()),
                             static_cast<uint32_t>(font_data.size()),
                             HB_MEMORY_MODE_READONLY, nullptr, nullptr));
  if (!blob) {
    return {};
  }

  HBSubsetInput input(hb_subset_input_create_or_fail());
  if (!input) {
    return {};
  }

  hb_subset_input_set_flags(input.get(), HB_SUBSET_FLAGS_RETAIN_GIDS);

  hb_set_t* glyphs = hb_subset_input_glyph_set(input.get());
  for (uint32_t id : gids) {
    hb_set_add(glyphs, id);
  }

  HBFace face(hb_face_create(blob.get(), 0));
  HBFace subset_face(hb_subset_or_fail(face.get(), input.get()));
  if (!subset_face) {
    return {};
  }

  HBBlob subset_blob(hb_face_reference_blob(subset_face.get()));
  const char* out_data = hb_blob_get_data(subset_blob.get(), nullptr);
  unsigned int out_len = hb_blob_get_length(subset_blob.get());
  if (!out_data || out_len == 0) {
    return {};
  }

  // SAFETY: HarfBuzz guarantees the correct length from hb_blob_get_length.
  return std::vector<uint8_t>(out_data, UNSAFE_BUFFERS(out_data + out_len));
}

}  // namespace

CPDF_FontSubsetter::SubsetCandidate::SubsetCandidate() = default;

CPDF_FontSubsetter::SubsetCandidate::~SubsetCandidate() = default;

CPDF_FontSubsetter::CPDF_FontSubsetter(CPDF_Document* doc) : doc_(doc) {}

CPDF_FontSubsetter::~CPDF_FontSubsetter() = default;

std::map<uint32_t, RetainPtr<CPDF_Object>>
CPDF_FontSubsetter::GenerateFontSubsetObjectOverrides() {
  std::map<uint32_t, RetainPtr<CPDF_Object>> overrides;

  // Scan pages.
  for (int i = 0; i < doc_->GetPageCount(); ++i) {
    RetainPtr<CPDF_Dictionary> page_dict = doc_->GetMutablePageDictionary(i);
    if (!page_dict) {
      continue;
    }

    auto page = pdfium::MakeRetain<CPDF_Page>(doc_.get(), std::move(page_dict));
    page->ParseContent();

    AddTextAndFonts(page.Get());
  }

  std::cout << " " << __func__ << " candidates_.size()=" << candidates_.size()
            << std::endl;
  for (auto& [obj_num, candidate] : candidates_) {
    auto old_stream_acc =
        pdfium::MakeRetain<CPDF_StreamAcc>(std::move(candidate.stream));
    old_stream_acc->LoadAllDataFiltered();
    std::vector<uint8_t> new_font_data =
        GenerateFontSubset(old_stream_acc->GetSpan(), candidate.used_gids);
    std::cout << " size before=" << old_stream_acc->GetSpan().size()
              << " size after =" << new_font_data.size() << std::endl;
    if (new_font_data.empty()) {
      continue;
    }
    auto new_stream = pdfium::MakeRetain<CPDF_Stream>(new_font_data);
    RetainPtr<CPDF_Dictionary> stream_dict = new_stream->GetMutableDict();
    stream_dict->SetNewFor<CPDF_Name>("Filter", "FlateDecode");
    overrides[obj_num] = new_stream;

    if (candidate.descriptor) {
      RetainPtr<CPDF_Dictionary> new_descriptor =
          ToDictionary(candidate.descriptor->Clone());

      // Update the length metadata to match the new binary size.
      new_descriptor->SetNewFor<CPDF_Number>(
          "Length1", static_cast<int>(new_font_data.size()));
      new_descriptor->RemoveFor("Length2");
      new_descriptor->RemoveFor("Length3");
      overrides[candidate.descriptor->GetObjNum()] = new_descriptor;
    }
  }
  return overrides;
}

void CPDF_FontSubsetter::AddTextAndFonts(CPDF_Page* page) {
  // Scan page objects.
  for (const auto& page_obj : *page) {
    if (!page_obj->IsText()) {
      continue;
    }
    // Search for text and for font files.
    std::cout << " Text" << std::endl;
    const CPDF_TextObject* text = page_obj->AsText();
    RetainPtr<CPDF_Font> font = text->GetFont();
    std::cout << " FontName=" << font->GetBaseFontName().c_str() << std::endl;
    if (!font) {
      continue;
    }

    RetainPtr<CPDF_Dictionary> font_dict = font->GetMutableFontDict();
    if (!font_dict) {
      continue;
    }

    RetainPtr<const CPDF_Dictionary> descriptor;
    if (font->IsCIDFont()) {
      RetainPtr<const CPDF_Array> descendants =
          font_dict->GetArrayFor("DescendantFonts");
      if (!descendants || descendants->IsEmpty()) {
        continue;
      }
      RetainPtr<const CPDF_Dictionary> cid_font = descendants->GetDictAt(0);
      if (!cid_font) {
        continue;
      }
      descriptor = cid_font->GetDictFor("FontDescriptor");
    } else {
      descriptor = font_dict->GetDictFor("FontDescriptor");
    }
    if (!descriptor) {
      continue;
    }

    // Only TrueType is supported for now.
    RetainPtr<const CPDF_Stream> stream = descriptor->GetStreamFor("FontFile2");
    if (!stream) {
      continue;
    }

    font_dict->SetNewFor<CPDF_String>("CIDToGIDMap", "Identity");

    auto& candidate = candidates_[stream->GetObjNum()];
    if (!candidate.stream) {
      candidate.stream = std::move(stream);
      candidate.descriptor = std::move(descriptor);
    }

    const std::vector<uint32_t>& char_codes = text->GetCharCodes();
    std::set<uint32_t>& used_gids = candidate.used_gids;
    for (uint32_t char_code : char_codes) {
      int gid = font->GlyphFromCharCode(char_code, /*pVertGlyph=*/nullptr);
      if (gid != -1) {
        used_gids.insert(static_cast<uint32_t>(gid));
      }
    }
  }
}
