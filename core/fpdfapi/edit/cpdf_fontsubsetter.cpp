...
void AddTextAndFonts(CPDF_Page* page,
  ...
  for (const auto& page_obj : *page) {
    ...
    auto& candidate = candidates[stream->GetObjNum()];
    if (!candidate.stream) {
...
std::map<uint32_t, RetainPtr<CPDF_Object>> GenerateFontSubsetObjectOverrides(
  ...
  for (int i = 0; i < doc->GetPageCount(); ++i) {
    ...
    page->ParseContent();
    AddTextAndFonts(page.Get(), candidates);
  }

  std::cout << " " << __func__ << " candidates.size()=" << candidates.size()
            << std::endl;
  for (auto& [obj_num, candidate] : candidates) {
    auto old_stream_acc =
