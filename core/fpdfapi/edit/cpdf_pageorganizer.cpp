...
void CPDF_PageOrganizer::CopyOCProperties() {
  ...
  RetainPtr<const CPDF_Dictionary> src_root(src()->GetRoot());
  if (!src_root) {
    return;
  }

  RetainPtr<const CPDF_Dictionary> src_oc_props =
      src_root->GetDictFor("OCProperties");
  if (!src_oc_props) {
    return;
  }
  ...
  if (ocgs && !ocgs->IsEmpty()) {
    dest_root->SetNewFor<CPDF_Reference>(
        "OCProperties", dest(), dest()->AddIndirectObject(cloned_props));
  }
}
