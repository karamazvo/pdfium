...
TEST_F(CPDFFontSubsetterTest, MultipleFontMultipleText) {
  ...
  // The file size for `font_path1` is ~16.4 MB.
  ...
  // Subset size for `font_data1` is ~0.015% of the original file, i.e. ~2.8 KB.
  // Subset size for `font_data2` is ~3% of the original file, i.e. ~13.3 KB.
  EXPECT_THAT(overrides,
              AllOf(Contains(StreamSizeIsWithinRange(original_size1 * 0.0001,
                                                     original_size1 * 0.0002)),
                    Contains(StreamSizeIsWithinRange(original_size2 * 0.025,
