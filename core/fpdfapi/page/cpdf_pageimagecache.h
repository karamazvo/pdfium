 ...
 private:
  class Entry {
   public:
    ...
    void Reset();
    uint32_t EstimateSize() const { return dw_cache_size_; }
    uint32_t GetMatteColor() const { return matte_color_; }
    uint32_t GetTimeCount() const { return dw_time_count_; }
    void SetTimeCount(uint32_t count) { dw_time_count_ = count; }
   ...
   private:
    ...
    uint32_t dw_time_count_ = 0;
    uint32_t matte_color_ = 0;
    uint32_t dw_cache_size_ = 0;
