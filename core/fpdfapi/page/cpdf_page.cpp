...
std::optional<CFX_PointF> CPDF_Page::DeviceToPage(
  ...
  CFX_Matrix page2device = GetDisplayMatrixForRect(rect, rotate);
...
CFX_Matrix CPDF_Page::GetDisplayMatrixForRect(const FX_RECT& rect,
                                              int rotate) const {
  return GetDisplayMatrixForFloatRect(CFX_FloatRect(rect), rotate);
}
...
CFX_Matrix CPDF_Page::GetDisplayMatrixForFloatRect(const CFX_FloatRect& rect,
  ...
  // translated on x. On rotate = 0, y0 is rect.top and the translation to get
