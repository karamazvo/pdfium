...
class CPDF_TransferFuncDIB final : public CFX_DIBBase {
 ...
 private:
  static constexpr size_t kSampleCount = CPDF_TransferFunc::kChannelSampleSize;
  ...
  const pdfium::span<const uint8_t, kSampleCount> r_samples_;
  const pdfium::span<const uint8_t, kSampleCount> g_samples_;
  const pdfium::span<const uint8_t, kSampleCount> b_samples_;
