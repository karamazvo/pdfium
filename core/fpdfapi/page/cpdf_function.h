...
class CPDF_Function {
 ...
 protected:
  ...
  static std::unique_ptr<CPDF_Function> Load(
      RetainPtr<const CPDF_Object> pFuncObj,
      VisitedSet* pVisited);
  bool Init(const CPDF_Object* obj, VisitedSet* pVisited);
  // `obj` is guaranteed to be either a dictionary or a stream.
  virtual bool v_Init(const CPDF_Object* obj, VisitedSet* pVisited) = 0;
