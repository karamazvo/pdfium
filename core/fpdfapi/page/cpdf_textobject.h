...
class CPDF_TextObject final : public CPDF_PageObject {
 public:
  ...
  // using these setters.
  //
  // `char_positions` must be one less than the `char_codes_` size.
  bool SetCharPositions(pdfium::span<const float> char_positions);
 ...
 private:
  ...
  //   applied after the character at the same index.
  // - `char_positions_` has N - 1 elements, representing the horizontal or
  //   vertical offsets for all characters except the first, depending on text
  //   direction. The first character's offset is always 0.
