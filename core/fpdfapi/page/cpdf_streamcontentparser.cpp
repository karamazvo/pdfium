...
int CPDF_StreamContentParser::GetNextParamPos() {
  if (param_count_ == kParamBufSize) {
    ...
    if (param_buf_[param_start_pos_].type_ == ContentParam::Type::kObject) {
      param_buf_[param_start_pos_].object_.Reset();
    }
...
void CPDF_StreamContentParser::AddNameParam(ByteStringView bsName) {
  ContentParam& param = param_buf_[GetNextParamPos()];
  param.type_ = ContentParam::Type::kName;
  param.name_ = PDF_NameDecode(bsName);
}

void CPDF_StreamContentParser::AddNumberParam(ByteStringView str) {
  ContentParam& param = param_buf_[GetNextParamPos()];
  param.type_ = ContentParam::Type::kNumber;
  param.number_ = FX_Number(str);
}

void CPDF_StreamContentParser::AddObjectParam(RetainPtr<CPDF_Object> pObj) {
  ContentParam& param = param_buf_[GetNextParamPos()];
  param.type_ = ContentParam::Type::kObject;
  param.object_ = std::move(pObj);
}

void CPDF_StreamContentParser::ClearAllParams() {
  uint32_t index = param_start_pos_;
  for (uint32_t i = 0; i < param_count_; i++) {
    if (param_buf_[index].type_ == ContentParam::Type::kObject) {
      param_buf_[index].object_.Reset();
    }
    index++;
    if (index == kParamBufSize)
      index = 0;
...
RetainPtr<CPDF_Object> CPDF_StreamContentParser::GetObject(uint32_t index) {
  ...
  if (param.type_ == ContentParam::Type::kNumber) {
    param.type_ = ContentParam::Type::kObject;
    param.object_ =
        param.number_.IsInteger()
            ? pdfium::MakeRetain<CPDF_Number>(param.number_.GetSigned())
            : pdfium::MakeRetain<CPDF_Number>(param.number_.GetFloat());
    return param.object_;
  }
  if (param.type_ == ContentParam::Type::kName) {
    param.type_ = ContentParam::Type::kObject;
    param.object_ = document_->New<CPDF_Name>(param.name_);
    return param.object_;
  }
  if (param.type_ == ContentParam::Type::kObject) {
    return param.object_;
  }
...
ByteString CPDF_StreamContentParser::GetString(uint32_t index) const {
  ...
  if (param.type_ == ContentParam::Type::kObject && param.object_) {
    return param.object_->GetString();
  }
...
float CPDF_StreamContentParser::GetNumber(uint32_t index) const {
  ...
  const ContentParam& param = param_buf_[real_index];
  if (param.type_ == ContentParam::Type::kNumber) {
    return param.number_.GetFloat();
  }

  if (param.type_ == ContentParam::Type::kObject && param.object_) {
    return param.object_->GetNumber();
  }
