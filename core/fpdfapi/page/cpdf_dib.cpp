...
bool CPDF_DIB::ContinueInternal() {
  if (image_mask_) {
    ...
    const uint32_t bpp = m_bpc * components_;
    if (bpp == 0) {
      return false;
    }
...
CPDF_DIB::LoadState CPDF_DIB::ContinueLoadDIBBase(PauseIndicatorIface* pPause) {
  ...
  if (!m_pJbig2Context) {
    ...
    iDecodeStatus = Jbig2Decoder::StartDecode(
        m_pJbig2Context.get(), document_->GetOrCreateCodecContext(), GetWidth(),
...
bool CPDF_DIB::LoadColorInfo(const CPDF_Dictionary* pFormResources,
  ...
  m_bpc_orig = dict_->GetIntegerFor("BitsPerComponent");
  ...
  m_Family = color_space_->GetFamily();
...
bool CPDF_DIB::GetDecodeAndMaskArray() {
  ...
  if (pDecode) {
    ...
    for (uint32_t i = 0; i < components_; i++) {
      float def_value;
      color_space_->GetDefaultValue(i, &def_value, &m_CompData[i].m_DecodeMin,
...
CPDF_DIB::LoadState CPDF_DIB::CreateDecoder(uint8_t resolution_levels_to_skip) {
  ...
  const std::optional<uint32_t> requested_pitch =
      fxge::CalculatePitch8(m_bpc, components_, GetWidth());
...
bool CPDF_DIB::CreateDCTDecoder(pdfium::span<const uint8_t> src_span,
  ...
  if (color_space_) {
    ...
    if (m_Family == CPDF_ColorSpace::Family::kLab && components_ != 3) {
      return false;
    }
  }
...
bool CPDF_DIB::LoadInternal(const CPDF_Dictionary* pFormResources,
  ...
  if (do_bpc_check_ && (m_bpc == 0 || components_ == 0)) {
    return false;
  }
...
CPDF_DIB::LoadState CPDF_DIB::StartLoadMask() {
  ...
  if (pMatte && color_space_ && m_Family != CPDF_ColorSpace::Family::kPattern &&
...
void CPDF_DIB::LoadPalette() {
  if (!color_space_ || m_Family == CPDF_ColorSpace::Family::kPattern) {
    return;
  }
  ...
  if (bits == 1) {
    if (default_decode_ && (m_Family == CPDF_ColorSpace::Family::kDeviceGray ||
  ...
  for (int i = 0; i < palette_count; i++) {
    ...
    if (components_ == 1 && m_Family == CPDF_ColorSpace::Family::kICCBased &&
...
bool CPDF_DIB::TransMask() const {
  return load_mask_ && m_GroupFamily == CPDF_ColorSpace::Family::kDeviceCMYK &&
