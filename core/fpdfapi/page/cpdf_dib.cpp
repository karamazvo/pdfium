...
bool CPDF_DIB::ContinueInternal() {
  if (image_mask_) {
    ...
    const uint32_t bpp = pc_ * components_;
    if (bpp == 0) {
      return false;
    }
...
bool CPDF_DIB::LoadColorInfo(const CPDF_Dictionary* pFormResources,
  ...
  pc_orig_ = dict_->GetIntegerFor("BitsPerComponent");
  if (!IsMaybeValidBitsPerComponent(pc_orig_)) {
    return false;
  }
...
CPDF_DIB::LoadState CPDF_DIB::CreateDecoder(uint8_t resolution_levels_to_skip) {
  ...
  const std::optional<uint32_t> requested_pitch =
      fxge::CalculatePitch8(pc_, components_, GetWidth());
...
bool CPDF_DIB::LoadInternal(const CPDF_Dictionary* pFormResources,
  ...
  if (do_bpc_check_ && (pc_ == 0 || components_ == 0)) {
    return false;
  }
...
void CPDF_DIB::LoadPalette() {
  ...
  // Use FX_SAFE_UINT32 just to be on the safe side, in case |pc_| or
  // |components_| somehow gets a bad value.
  FX_SAFE_UINT32 safe_bits = pc_;
  ...
  for (int i = 0; i < palette_count; i++) {
    ...
    for (uint32_t j = 0; j < components_; j++) {
      int encoded_component = color_data % (1 << pc_);
      color_data /= 1 << pc_;
...
bool CPDF_DIB::ValidateDictParam(const ByteString& filter) {
  ...
  // Per spec, |pc_| should always be 8 for RunLengthDecode, but too many
  ...
  if (!IsAllowedBitsPerComponent(pc_)) {
    pc_ = 0;
    return false;
  }
  return true;
  ...
  for (int column = 0; column < GetWidth(); column++) {
    for (uint32_t color = 0; color < components_; color++) {
      if (bpp8) {
        ...
        unsigned int data = GetBits8(src_scan, src_bit_pos, pc_);
        color_values[color] = comp_data_[color].decode_min_ +
                              comp_data_[color].decode_step_ * data;
        src_bit_pos += pc_;
      }
...
bool CPDF_DIB::TranslateScanline24bppDefaultDecode(
  ...
  switch (pc_) {
    ...
    case 16:
      ...
      break;
    default:
      const unsigned int max_data = (1 << pc_) - 1;
      uint64_t src_bit_pos = 0;
      size_t dest_byte_pos = 0;
      for (int column = 0; column < GetWidth(); column++) {
        unsigned int R = GetBits8(src_scan, src_bit_pos, pc_);
        src_bit_pos += pc_;
        unsigned int G = GetBits8(src_scan, src_bit_pos, pc_);
        src_bit_pos += pc_;
        unsigned int B = GetBits8(src_scan, src_bit_pos, pc_);
        src_bit_pos += pc_;
  ...
  if (pc_ * components_ <= 8) {
    ...
    if (pc_ == 8) {
      ...
      for (int col = 0; col < GetWidth(); col++) {
        ...
        for (uint32_t color = 0; color < components_; color++) {
          unsigned int data = GetBits8(pSrcLine, src_bit_pos, pc_);
          color_index |= data << (color * pc_);
          src_bit_pos += pc_;
        }
