...
CPDF_DIB::LoadState CPDF_DIB::StartLoadMask() {
  ...
  if (pMatte && m_pColorSpace &&
    ...
    auto rgb = m_pColorSpace->GetRGB(colors).value_or(FX_RGB_STRUCT<float>{});
    m_MatteColor =
...
void CPDF_DIB::LoadPalette() {
  ...
  if (bits == 1) {
    ...
    auto rgb =
        m_pColorSpace->GetRGB(color_values).value_or(FX_RGB_STRUCT<float>{});
  ...
  for (int i = 0; i < palette_count; i++) {
    ...
    if (m_nComponents == 1 && m_Family == CPDF_ColorSpace::Family::kICCBased &&
      ...
      rgb = m_pColorSpace->GetRGB(temp_buf).value_or(FX_RGB_STRUCT<float>{});
    } else {
      rgb =
          m_pColorSpace->GetRGB(color_values).value_or(FX_RGB_STRUCT<float>{});
    }
