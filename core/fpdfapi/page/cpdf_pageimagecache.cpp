...
bool CPDF_PageImageCache::StartGetCachedBitmap(
  ...
  cur_find_cache_ = it != m_ImageCache.end();
  if (cur_find_cache_) {
  ...
  time_count_++;
  if (!cur_find_cache_) {
    m_ImageCache[pStream] = cur_image_cache_entry_.Release();
  }
...
bool CPDF_PageImageCache::Continue(PauseIndicatorIface* pPause) {
  ...
  time_count_++;
  if (!cur_find_cache_) {
    m_ImageCache[cur_image_cache_entry_->GetImage()->GetStream()] =
...
void CPDF_PageImageCache::Entry::ContinueGetCachedBitmap(
    CPDF_PageImageCache* pPageImageCache) {
  m_MatteColor = cur_bitmap_.AsRaw<CPDF_DIB>()->GetMatteColor();
...
void CPDF_PageImageCache::Entry::CalcSize() {
  m_dwCacheSize = 0;
  if (cached_bitmap_) {
    m_dwCacheSize += cached_bitmap_->GetEstimatedImageMemoryBurden();
  }
  if (cached_mask_) {
    m_dwCacheSize += cached_mask_->GetEstimatedImageMemoryBurden();
  }
}
