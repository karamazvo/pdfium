...
void CPDF_PageImageCache::Entry::ContinueGetCachedBitmap(
  ...
  dw_time_count_ = pPageImageCache->GetTimeCount();
...
void CPDF_PageImageCache::Entry::CalcSize() {
  dw_cache_size_ = 0;
  if (cached_bitmap_) {
    dw_cache_size_ += cached_bitmap_->GetEstimatedImageMemoryBurden();
  }
  if (cached_mask_) {
    dw_cache_size_ += cached_mask_->GetEstimatedImageMemoryBurden();
  }
}
