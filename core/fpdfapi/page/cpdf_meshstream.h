...
class CPDF_MeshStream {
 public:
  ...
  const CPDF_ColorSpace& BaseColorSpace() const { return *base_cs_; }
  uint32_t BaseComponents() const {
    if (cs_->AsIndexedCS()) {
      CHECK(funcs_.empty());
      return base_cs_->ComponentCount();
    }
    return Components();
  }
 ...
 private:
  ...
  RetainPtr<CPDF_ColorSpace> base_cs_;
