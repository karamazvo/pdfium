...
CPDF_Form::GetBitmapAndMatrixFromSoleImageOfForm() const {
  // Don't use GetActivePageCount() here, as it is more expensive and knowing
  // the exact number of active objects is overkill when just need to know if
  // there is only one object or not.  Rely on an exact IsActive() call instead
  // of GetActivePageCount().
