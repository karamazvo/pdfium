...
`fpdfapi` is the heart of PDFium's PDF engine. It is used by:

*   **`fpdfdoc/`**: To implement higher-level document logic like metadata,
    bookmarks, and forms.
*   **`fpdfsdk/`**: To bridge the internal C++ logic to the public C API.
*   **`fpdftext/`**: To perform text extraction and search.

Consumers typically interact with `fpdfapi` through `CPDF_Document` and
`CPDF_Page` classes to access document and page-level information.

## Lay of the Land

The `fpdfapi/` directory is organized into several subdirectories:
