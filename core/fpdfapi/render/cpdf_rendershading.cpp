...
CFX_PointF operator*(float f, CFX_PointF p) {
  return {f * p.x, f * p.y};
}
...
struct CubicBezierPatch {
  bool IsSmall() const {
    CFX_FloatRect bbox =
        CFX_FloatRect::GetBBox(pdfium::span(points[0].data(), 16u));
  ...
  void GetBoundary(pdfium::span<CFX_Path::Point> boundary) {
    ...
    // XXX make bezier patch stress test; this won't always work :/
  ...
  void SubdivideVertical(CubicBezierPatch& top, CubicBezierPatch& bottom) {
    for (int x = 0; x < 4; ++x) {
      std::array<CFX_PointF, 3> level1 = {
          (1.0f / 2.0f) * (points[x][0] + points[x][1]),
          (1.0f / 2.0f) * (points[x][1] + points[x][2]),
          (1.0f / 2.0f) * (points[x][2] + points[x][3]),
      };
      std::array<CFX_PointF, 2> level2 = {
          (1.0f / 2.0f) * (level1[0] + level1[1]),
          (1.0f / 2.0f) * (level1[1] + level1[2]),
      };
      CFX_PointF level3 = (1.0f / 2.0f) * (level2[0] + level2[1]);
  ...
  void SubdivideHorizontal(CubicBezierPatch& left, CubicBezierPatch& right) {
    for (int y = 0; y < 4; ++y) {
      std::array<CFX_PointF, 3> level1 = {
          (1.0f / 2.0f) * (points[0][y] + points[1][y]),
          (1.0f / 2.0f) * (points[1][y] + points[2][y]),
          (1.0f / 2.0f) * (points[2][y] + points[3][y]),
      };
      std::array<CFX_PointF, 2> level2 = {
          (1.0f / 2.0f) * (level1[0] + level1[1]),
          (1.0f / 2.0f) * (level1[1] + level1[2]),
...
int Interpolate(int p1, int p2, int delta1, int delta2, bool* overflow) {
  ...
  p *= delta1;
  if (delta2 > 1)
    //p = (p + (delta2 - 1) / 2)/ (delta2 - 1); // XXX truncates instead of rounding; also, needs - 1?
    p = p/ (delta2 - 1); // XXX truncates instead of rounding; also, needs - 1?
  p += p1;
  if (!p.IsValid()) {
    *overflow = true;
  }
  ...
  void Draw(int x_scale,
    ...
    if (depth > 1 || bSmall ||
      ...
      if (d_bottom < kCoonColorThreshold && d_top < kCoonColorThreshold) {
        ++depth;
        CubicBezierPatch top_patch, bottom_patch;
        ...
        ++depth;
        CubicBezierPatch left_patch, right_patch;
        ...
        ++depth;
        CubicBezierPatch top_left, bottom_left, top_right, bottom_right;
...
void DrawCoonPatchMeshes(
  ...
  while (!stream.IsEOF()) {
    ...
    if (type == kTensorProductPatchMeshShading) {
      ...
      CHECK(type == kCoonsPatchMeshShading);
      // From Type 7 Shadings (Tensor-Product Patch Meshes):
      patch.points[1][1] =
          (1.0f / 9.0f) *
          (-4 * patch.points[0][0] +
           6 * (patch.points[0][1] + patch.points[1][0]) -
           2 * (patch.points[0][3] + patch.points[3][0]) +
           3 * (patch.points[3][1] + patch.points[1][3]) - patch.points[3][3]);
      patch.points[1][2] =
          (1.0f / 9.0f) *
          (-4 * patch.points[0][3] +
           6 * (patch.points[0][2] + patch.points[1][3]) -
           2 * (patch.points[0][0] + patch.points[3][3]) +
           3 * (patch.points[3][2] + patch.points[1][0]) - patch.points[3][0]);
      patch.points[2][1] =
          (1.0f / 9.0f) *
          (-4 * patch.points[3][0] +
           6 * (patch.points[3][1] + patch.points[2][0]) -
           2 * (patch.points[3][3] + patch.points[0][0]) +
           3 * (patch.points[0][1] + patch.points[2][3]) - patch.points[0][3]);
      patch.points[2][2] =
          (1.0f / 9.0f) *
          (-4 * patch.points[3][3] +
           6 * (patch.points[3][2] + patch.points[2][3]) -
           2 * (patch.points[3][0] + patch.points[0][3]) +
           3 * (patch.points[0][2] + patch.points[2][0]) - patch.points[0][0]);
    }
