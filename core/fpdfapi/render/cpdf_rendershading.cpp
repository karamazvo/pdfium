    ...
    for (int x = start_x; x < end_x; x++) {
      ...
      for (const auto& func : funcs) {
        if (func && func->OutputCount() <= kMaxMeshColorComponents) {
          // XXX wrong for array of 1-elt funcs
          func->Call(pdfium::make_span(result_color).first<1u>(),
...
struct CoonsColor {
  ...
  float Distance(
    ...
    for (const auto& func : funcs) {
      if (func && func->OutputCount() <= kMaxMeshColorComponents) {
        // XXX wrong for array of 1-elt funcs
        func->Call(pdfium::make_span(comp).first<1u>(),
                   pdfium::make_span(result));
      }
    }
    ...
    for (const auto& func : funcs) {
      if (func && func->OutputCount() <= kMaxMeshColorComponents) {
        // XXX wrong for array of 1-elt funcs
        func->Call(pdfium::make_span(o.comp).first<1u>(),
                   pdfium::make_span(oresult));
      }
    }
  ...
  void Draw(int x_scale,
    ...
    if (bSmall ||
      ...
      for (const auto& func : funcs) {
        if (func && func->OutputCount() <= kMaxMeshColorComponents) {
          // XXX wrong for array of 1-elt funcs
          func->Call(pdfium::make_span(div_colors[0].comp).first<1u>(),
                     pdfium::make_span(result));
        }
      }
