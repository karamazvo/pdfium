...
struct CubicBezierPatch {
  bool IsSmall() const {
    // It's safe to treat a 4x4 2d array as a 16 element 1d array.
    auto element_span = UNSAFE_BUFFERS(pdfium::span(points[0].data(), 16u));
    CFX_FloatRect bbox = CFX_FloatRect::GetBBox(element_span);
