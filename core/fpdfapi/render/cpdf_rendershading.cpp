...
void DrawGouraud(const RetainPtr<CFX_DIBitmap>& pBitmap,
  ...
  for (int y = min_yi; y <= max_yi; y++) {
    ...
    for (int x = start_x; x < end_x; x++) {
      ...
      CPDF_MeshColor result{result_color};
      pdfium::span<float> result_span = pdfium::make_span(result);
      for (const auto& func : funcs) {
        if (func && func->OutputCount() <= kMaxMeshColorComponents) {
          std::optional<uint32_t> nresults = func->Call(
              pdfium::make_span(result_color).first<1u>(), result_span);
          if (nresults.has_value()) {
            result_span = result_span.subspan(nresults.value());
          }
        }
      ...
      FX_RGB_STRUCT<float> rgb = cs.GetRGBOrZerosOnError(result);
...
struct CoonsColor {
  ...
  float Distance(
      ...
      const std::vector<std::unique_ptr<CPDF_Function>>& funcs) const {
    // XXX converting here a bit wasteful
    // XXX have to look at func here too
    CPDF_MeshColor result{comp};
    pdfium::span<float> result_span = pdfium::make_span(result);
    for (const auto& func : funcs) {
      if (func && func->OutputCount() <= kMaxMeshColorComponents) {
        std::optional<uint32_t> nresults =
            func->Call(pdfium::make_span(comp).first<1u>(), result_span);
        if (nresults.has_value()) {
          result_span = result_span.subspan(nresults.value());
        }
      }
    ...
    pdfium::span<float> oresult_span = pdfium::make_span(oresult);
    for (const auto& func : funcs) {
      if (func && func->OutputCount() <= kMaxMeshColorComponents) {
        std::optional<uint32_t> nresults =
            func->Call(pdfium::make_span(o.comp).first<1u>(), oresult_span);
        if (nresults.has_value()) {
          oresult_span = result_span.subspan(nresults.value());
        }
      }
    }
    FX_RGB_STRUCT<float> rgb_right = color_space.GetRGBOrZerosOnError(oresult);
...
struct PatchDrawer {
  explicit PatchDrawer(const CPDF_ColorSpace& color_space,
                       const std::vector<std::unique_ptr<CPDF_Function>>& funcs)
      : color_space(color_space), funcs(funcs) {}
  ...
  void Draw(int x_scale,
    ...
    if (!bSmall) {
      ...
      d_bottom = div_colors[3].Distance(div_colors[0], color_space, funcs);
      d_left = div_colors[1].Distance(div_colors[0], color_space, funcs);
      d_top = div_colors[1].Distance(div_colors[2], color_space, funcs);
      d_right = div_colors[2].Distance(div_colors[3], color_space, funcs);
    }
    ...
    if (bSmall ||
      ...
      for (const auto& func : funcs) {
        if (func && func->OutputCount() <= kMaxMeshColorComponents) {
          std::optional<uint32_t> nresults = func->Call(
              pdfium::make_span(div_colors[0].comp).first<1u>(), result_span);
          if (nresults.has_value()) {
            result_span = result_span.subspan(nresults.value());
          }
        }
...
void DrawCoonPatchMeshes(
  ...
  PatchDrawer patch{stream.ColorSpace(), funcs};
  patch.alpha = alpha;
