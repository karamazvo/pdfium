...
bool GetShadingSteps(std::array<FX_ARGB, kShadingSteps>* output,
                     float t_min,
                     float t_max,
                     const std::vector<std::unique_ptr<CPDF_Function>>& funcs,
                     const RetainPtr<CPDF_ColorSpace>& pCS,
...
void DrawAxialShading(const RetainPtr<CFX_DIBitmap>& pBitmap,
  ...
  std::array<FX_ARGB, kShadingSteps> shading_steps;
  if (!GetShadingSteps(&shading_steps, t_min, t_max, funcs, pCS, alpha)) {
    return;
  }
...
void DrawFreeGouraudShading(
  ...
  std::array<FX_ARGB, kShadingSteps> shading_steps;
  if (!funcs.empty()) {
    if (!GetShadingSteps(&shading_steps, stream.component_min(0),
                         stream.component_max(0), funcs, pCS, alpha)) {
      return;
    }
  }
