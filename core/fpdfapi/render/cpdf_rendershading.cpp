      ...
      for (const auto& func : funcs) {
        if (func && func->OutputCount() <= kMaxMeshColorComponents) {
          std::optional<uint32_t> nresults = func->Call(
              pdfium::span(result_color).first<1u>(), result_span);
          if (nresults.has_value()) {
            result_span = result_span.subspan(nresults.value());
          }
        }
...
void DrawFreeGouraudShading(
  ...
  while (!stream.IsEOF()) {
    ...
    DrawGouraud(pBitmap, alpha, triangle, stream.BaseComponents(),
                stream.BaseColorSpace(), funcs);
  }
}
...
struct CoonsColor {
  ...
  float Distance(
    ...
    for (const auto& func : funcs) {
      if (func && func->OutputCount() <= kMaxMeshColorComponents) {
        std::optional<uint32_t> nresults =
            func->Call(pdfium::span(comp).first<1u>(), result_span);
        if (nresults.has_value()) {
          result_span = result_span.subspan(nresults.value());
        }
      }
    ...
    for (const auto& func : funcs) {
      if (func && func->OutputCount() <= kMaxMeshColorComponents) {
        std::optional<uint32_t> nresults =
            func->Call(pdfium::span(o.comp).first<1u>(), oresult_span);
        if (nresults.has_value()) {
  ...
  void Draw(int x_scale,
    ...
    if (bSmall ||
      ...
      for (const auto& func : funcs) {
        if (func && func->OutputCount() <= kMaxMeshColorComponents) {
          std::optional<uint32_t> nresults = func->Call(
              pdfium::span(div_colors[0].comp).first<1u>(), result_span);
          if (nresults.has_value()) {
            result_span = result_span.subspan(nresults.value());
          }
        }
...
void DrawCoonPatchMeshes(
  ...
  PatchDrawer patch_drawer{stream.BaseColorSpace(), funcs};
