...
struct CoonsColor {
  ...
  float Distance(const CoonsColor& o,
                 const CPDF_ColorSpace& color_space, const std::array<FX_ARGB, kShadingSteps>* shading_steps) const {
    ...
    if (shading_steps) {
      ...
      return std::max({abs(rgb_right.red - rgb_left.red),
                       ...
                       abs(rgb_right.blue - rgb_left.blue)}) / 255.0f;
    } else {
...
struct PatchDrawer {
  ...
  void Draw(int x_scale,
    ...
    if (!bSmall) {
      ...
      d_bottom = div_colors[3].Distance(div_colors[0], color_space, shading_steps);
      d_left = div_colors[1].Distance(div_colors[0], color_space, shading_steps);
      d_top = div_colors[1].Distance(div_colors[2], color_space, shading_steps);
      d_right = div_colors[2].Distance(div_colors[3], color_space, shading_steps);
    }
