...
void CPDF_RenderStatus::DrawObjWithBackground(CPDF_PageObject* pObj,
  ...
  const bool needs_buffer =
      !(device_->GetDeviceCaps(DeviceCapsId::kRenderCaps) &
        std::to_underlying(RenderCapsFlag::kGetBits));
  if (!needs_buffer) {
...
void CPDF_RenderStatus::ProcessClipPath(const CPDF_ClipPath& ClipPath,
  ...
  if (!IsPrint() && !(device_->GetDeviceCaps(DeviceCapsId::kRenderCaps) &
                      std::to_underlying(RenderCapsFlag::kSoftClip))) {
    return;
  }
...
bool CPDF_RenderStatus::ProcessTransparency(CPDF_PageObject* pPageObj,
  ...
  if (!transparency.IsIsolated() &&
      (device_->GetRenderCaps() & std::to_underlying(RenderCapsFlag::kGetBits))) {
...
RetainPtr<CFX_DIBitmap> CPDF_RenderStatus::GetBackdrop(
  ...
  const int cap_to_check = backdrop->IsAlphaFormat()
...
void CPDF_RenderStatus::CompositeDIBitmap(
  ...
  bool bGetBackGround = ((device_->GetRenderCaps() &
                          std::to_underlying(RenderCapsFlag::kAlphaOutput))) ||
                        (!(device_->GetRenderCaps() &
                           std::to_underlying(RenderCapsFlag::kAlphaOutput)) &&
                         (device_->GetRenderCaps() &
                          std::to_underlying(RenderCapsFlag::kGetBits)) &&
                         !bBackAlphaRequired);
...
FXDIB_Format CPDF_RenderStatus::GetCompatibleArgbFormat() const {
  ...
  if (device_->GetDeviceCaps(DeviceCapsId::kRenderCaps) &
      std::to_underlying(RenderCapsFlag::kPremultipliedAlpha)) {
