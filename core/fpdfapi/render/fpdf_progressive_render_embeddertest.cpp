...
TEST_F(FPDFProgressiveRenderEmbedderTest, RenderPathObjectUsability) {
  // Test rendering of paths with one of the page objects used vs. unused.
  ...
  // Check rendering result after a page object is to be unused.
  // Contents do not need to be regenerated to observe an effect.
  ASSERT_TRUE(FPDFPageObj_SetIsUsed(page_obj, /*used=*/false));
  {
    ScopedFPDFBitmap bitmap = RenderPage(page.get());
    CompareBitmap(bitmap.get(), 200, 300, one_rectangle_unused_checksum);
  }

  // Check rendering result after the same page object is used again.
  ASSERT_TRUE(FPDFPageObj_SetIsUsed(page_obj, /*used=*/true));
  {
    ScopedFPDFBitmap bitmap = RenderPage(page.get());
    CompareBitmap(bitmap.get(), 200, 300, all_rectangles_used_checksum);
  }
