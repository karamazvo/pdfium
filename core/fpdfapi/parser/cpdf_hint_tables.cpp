...
bool CPDF_HintTables::ReadPageHintTable(CFX_BitStream* hStream) {
  ...
  sz_first_page_obj_offset_ = szFirstObjLoc;
  ...
  DCHECK(sz_first_page_obj_offset_);
  page_infos_[nFirstPageNum].set_page_offset(sz_first_page_obj_offset_);
...
bool CPDF_HintTables::ReadSharedObjHintTable(CFX_BitStream* hStream,
  ...
  // group length.
  FX_SAFE_FILESIZE prev_shared_group_end_offset = sz_first_page_obj_offset_;
  for (uint32_t i = 0; i < dwSharedObjTotal; ++i) {
    ...
    shared_obj_group_infos_[i].dw_length_ = safeObjLen.ValueOrDie();
    shared_obj_group_infos_[i].sz_offset_ =
        prev_shared_group_end_offset.ValueOrDie();
    prev_shared_group_end_offset += shared_obj_group_infos_[i].dw_length_;
  ...
  for (uint32_t i = 0; i < dwSharedObjTotal; ++i) {
    ...
    shared_obj_group_infos_[i].dw_start_obj_num_ = obj_num;
    shared_obj_group_infos_[i].dw_objects_count_ = obj_count.ValueOrDie();
  }
...
CPDF_DataAvail::DocAvailStatus CPDF_HintTables::CheckPage(uint32_t index) {
  ...
  for (const uint32_t dwIndex : page_infos_[index].Identifiers()) {
    ...
    if (!shared_group_info.sz_offset_ || !shared_group_info.dw_length_) {
      return CPDF_DataAvail::kDataError;
    }

    if (!validator_->CheckDataRangeAndRequestIfUnavailable(
            shared_group_info.sz_offset_, shared_group_info.dw_length_)) {
      return CPDF_DataAvail::kDataNotAvailable;
    }
  }
  return CPDF_DataAvail::kDataAvailable;
