...
class CPDF_DataAvail final : public Observable::ObserverIface {
 ...
 private:
  ...
  InternalStatus nternal_status_ = InternalStatus::kHeader;
  std::unique_ptr<CPDF_CrossRefAvail> cross_ref_avail_;
  const FX_FILESIZE dw_file_len_;
  ...
  PageNode page_node_;
  std::set<uint32_t> age_map_check_state_;
  std::set<uint32_t> ages_load_state_;
