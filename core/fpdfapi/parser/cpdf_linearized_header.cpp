...
CPDF_LinearizedHeader::CPDF_LinearizedHeader(const CPDF_Dictionary* pDict,
    ...
    : sz_file_size_(pDict->GetIntegerFor("L")),
      dw_first_page_no_(pDict->GetIntegerFor("P")),
      mainxref_table_first_entry_offset_(pDict->GetIntegerFor("T")),
      page_count_(pDict->GetIntegerFor("N")),
      sz_first_page_end_offset_(pDict->GetIntegerFor("E")),
      first_page_obj_num_(pDict->GetIntegerFor("O")),
      sz_last_xref_offset_(szLastXRefOffset) {
  ...
  if (nHintStreamSize == 2 || nHintStreamSize == 4) {
    sz_hint_start_ = std::max(pHintStreamRange->GetIntegerAt(0), 0);
