...
RetainPtr<CPDF_ReadValidator> MakeValidatorFromFile(
  ...
  if (file_path.empty()) {
    return RetainPtr<CPDF_ReadValidator>();
  }
  auto stream = CFX_FileAccessStream::CreateFromFilename(file_path.c_str());
  if (!stream) {
    return RetainPtr<CPDF_ReadValidator>();
  }
  return pdfium::MakeRetain<CPDF_ReadValidator>(std::move(stream), nullptr);
}

std::unique_ptr<CPDF_DataAvail> MakeDataAvailFromFile(
    const std::string& file_name) {
  auto validator = MakeValidatorFromFile(file_name);
  if (!validator) {
    return std::unique_ptr<CPDF_DataAvail>();
  }
  return std::make_unique<CPDF_DataAvail>(nullptr, std::move(validator));
}
...
class TestLinearizedHeader final : public CPDF_LinearizedHeader {
 public:
  ...
  static std::unique_ptr<CPDF_LinearizedHeader> MakeHeader(
    ...
    CPDF_SyntaxParser parser(pdfium::MakeRetain<CFX_ReadOnlyByteStringStream>(
