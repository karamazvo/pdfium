...
class TestSyntaxParser : public CPDF_IndirectObjectHolder,
 ...
 public:
  explicit TestSyntaxParser(RetainPtr<IFX_SeekableReadStream> pFileAccess)
      : CPDF_SyntaxParser(pFileAccess, this) {}
  TestSyntaxParser(RetainPtr<CPDF_ReadValidator> pValidator,
                   FX_FILESIZE HeaderOffset)
      : CPDF_SyntaxParser(pValidator, HeaderOffset, this) {}
};
...
class TestLinearizedHeader final : public CPDF_LinearizedHeader {
 public:
  ...
  static std::unique_ptr<CPDF_LinearizedHeader> MakeHeader(
    ...
    TestSyntaxParser parser(
        pdfium::MakeRetain<CFX_ReadOnlyStringStream>(std::move(inline_data)));
    RetainPtr<CPDF_Dictionary> dict = ToDictionary(parser.GetObjectBody());
    DCHECK(dict);
...
TEST_F(HintTablesTest, FirstPageOffset) {
  ...
  TestSyntaxParser parser(validator, 0);
  RetainPtr<CPDF_Stream> stream = ToStream(parser.GetObjectBody());
  ASSERT_TRUE(stream);
