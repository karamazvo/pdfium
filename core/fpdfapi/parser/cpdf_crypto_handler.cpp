...
void* CPDF_CryptoHandler::DecryptStart(uint32_t objnum, uint32_t gennum) {
  ...
  if (cipher_ == Cipher::kAES) {
    return DecryptStartAes(objnum, gennum);
  }
  return DecryptStartArcFour(objnum, gennum);
}

void* CPDF_CryptoHandler::DecryptStartAes(uint32_t objnum, uint32_t gennum) {
...
void* CPDF_CryptoHandler::DecryptStartArcFour(uint32_t objnum,
                                              uint32_t gennum) {
