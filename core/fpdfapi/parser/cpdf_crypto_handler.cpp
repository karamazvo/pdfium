...
DataVector<uint8_t> CPDF_CryptoHandler::EncryptContent(
  ...
  if (m_Cipher == Cipher::kAES) {
    ...
    auto padding_span = pdfium::make_span(padding);
    fxcrt::Copy(source.subspan(source_data_size, source_padding_size),
                padding_span);
    fxcrt::Fill(padding_span.subspan(source_padding_size),
                16 - source_padding_size);
    CRYPT_AESEncrypt(m_pAESContext.get(), dest_padding_span, padding_span);
    return dest;
  }
