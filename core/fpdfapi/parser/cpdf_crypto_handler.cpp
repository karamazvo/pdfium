...
size_t CPDF_CryptoHandler::EncryptContent(uint32_t objnum,
  ...
  if (m_Cipher != Cipher::kAES || m_KeyLen != 32) {
    ...
    if (m_Cipher == Cipher::kAES) {
      fxcrt::spancpy(pdfium::make_span(key1).subspan(m_KeyLen + 5),
                     ByteStringView("sAlT").unsigned_span());
    }
  ...
  if (m_Cipher == Cipher::kAES) {
    ...
    fxcrt::spancpy(dest, pdfium::make_span(iv));
    int nblocks = source.size() / 16;
    CRYPT_AESEncrypt(m_pAESContext.get(), dest.subspan(16).data(),
                     source.data(), nblocks * 16);
    uint8_t padding[16];
    fxcrt::spancpy(pdfium::make_span(padding),
                   source.subspan(nblocks * 16, source.size() % 16));
    fxcrt::spanset(pdfium::make_span(padding).subspan(source.size() % 16),
                   16 - source.size() % 16);
