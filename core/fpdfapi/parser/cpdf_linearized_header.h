...
class CPDF_LinearizedHeader {
 public:
  ...
  FX_FILESIZE GetFileSize() const { return sz_file_size_; }
  uint32_t GetFirstPageNo() const { return dw_first_page_no_; }
  // Will only return values > 0.
  FX_FILESIZE GetMainXRefTableFirstEntryOffset() const {
    return mainxref_table_first_entry_offset_;
  }
  uint32_t GetPageCount() const { return page_count_; }
  // Will only return values > 0.
  FX_FILESIZE GetFirstPageEndOffset() const {
    return sz_first_page_end_offset_;
  }
  ...
  // Will only return values > 0.
  FX_FILESIZE GetLastXRefOffset() const { return sz_last_xref_offset_; }

  bool HasHintTable() const;
  // Will only return values > 0.
  FX_FILESIZE GetHintStart() const { return sz_hint_start_; }
 ...
 private:
  const FX_FILESIZE sz_file_size_;
  const uint32_t dw_first_page_no_;
  const FX_FILESIZE mainxref_table_first_entry_offset_;
  const uint32_t page_count_;
  const FX_FILESIZE sz_first_page_end_offset_;
  const uint32_t first_page_obj_num_;
  const FX_FILESIZE sz_last_xref_offset_;
  FX_FILESIZE sz_hint_start_ = 0;
