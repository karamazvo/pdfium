...
CPDF_DataAvail::CPDF_DataAvail(FileAvail* pFileAvail,
    ...
    : file_read_(pdfium::MakeRetain<CPDF_ReadValidator>(std::move(pFileRead),
                                                        pFileAvail)),
      dw_file_len_(file_read_->GetSize()) {}
...
bool CPDF_DataAvail::CheckDocStatus() {
  switch (nternal_status_) {
    ...
    case InternalStatus::kPage:
      ...
      nternal_status_ = InternalStatus::kPageLaterLoad;
      return true;
    case InternalStatus::kError:
      return LoadAllFile();
    case InternalStatus::kPageLaterLoad:
      nternal_status_ = InternalStatus::kPage;
      [[fallthrough]];
    default:
      doc_avail_ = true;
      return true;
...
bool CPDF_DataAvail::LoadAllFile() {
  if (GetValidator()->CheckWholeFileAndRequestIfUnavailable()) {
    nternal_status_ = InternalStatus::kDone;
    return true;
  }
  return false;
}
...
bool CPDF_DataAvail::CheckAndLoadAllXref() {
  if (!cross_ref_avail_) {
    ...
    const FX_FILESIZE last_xref_offset = arser_.ParseStartXRef();
    if (GetValidator()->has_read_problems())
      return false;

    if (last_xref_offset <= 0) {
      nternal_status_ = InternalStatus::kError;
      return false;
    }
  ...
  if (!arser_.LoadAllCrossRefTablesAndStreams(
          cross_ref_avail_->last_crossref_offset())) {
    nternal_status_ = InternalStatus::kLoadAllFile;
    return false;
  }

  nternal_status_ = InternalStatus::kRoot;
  return true;
}
...
RetainPtr<CPDF_Object> CPDF_DataAvail::GetObject(uint32_t objnum,
                                                 bool* pExistInFile) {
  *pExistInFile = false;
  CPDF_Parser* pParser = document_ ? document_->GetParser() : &arser_;
  if (!pParser)
    return nullptr;
...
bool CPDF_DataAvail::CheckInfo() {
  const uint32_t dwInfoObjNum = arser_.GetInfoObjNum();
  if (dwInfoObjNum == CPDF_Object::kInvalidObjNum) {
    nternal_status_ = InternalStatus::kPageTree;
    return true;
  }

  CPDF_ReadValidator::ScopedSession read_session(GetValidator());
  arser_.ParseIndirectObject(dwInfoObjNum);
  if (GetValidator()->has_read_problems())
    return false;
...
bool CPDF_DataAvail::CheckRoot() {
  const uint32_t dwRootObjNum = arser_.GetRootObjNum();
  ...
  CPDF_ReadValidator::ScopedSession read_session(GetValidator());
  root_ = ToDictionary(arser_.ParseIndirectObject(dwRootObjNum));
  if (GetValidator()->has_read_problems())
    return false;
  ...
  nternal_status_ = InternalStatus::kInfo;
  return true;
}
...
bool CPDF_DataAvail::IsFirstCheck(uint32_t dwPage) {
  return age_map_check_state_.insert(dwPage).second;
}

void CPDF_DataAvail::ResetFirstCheck(uint32_t dwPage) {
  age_map_check_state_.erase(dwPage);
}
...
bool CPDF_DataAvail::CheckPages() {
  ...
  if (!pPages) {
    if (nternal_status_ == InternalStatus::kError) {
      nternal_status_ = InternalStatus::kLoadAllFile;
      return true;
    }
    return false;
...
bool CPDF_DataAvail::CheckHeader() {
  switch (CheckHeaderAndLinearized()) {
    case kDataAvailable:
      nternal_status_ = linearized_ ? InternalStatus::kFirstPage
...
bool CPDF_DataAvail::CheckFirstPage() {
  ...
  dwEnd += 512;
  if ((FX_FILESIZE)dwEnd > dw_file_len_) {
    dwEnd = (uint32_t)dw_file_len_;
  }

  const FX_FILESIZE start_pos = dw_file_len_ > 1024 ? 1024 : dw_file_len_;
  ...
  nternal_status_ = InternalStatus::kHintTable;
  return true;
}
...
CPDF_DataAvail::DocAvailStatus CPDF_DataAvail::CheckHeaderAndLinearized() {
  ...
  arser_.syntax_ = std::make_unique<CPDF_SyntaxParser>(GetValidator(),
                                                       header_offset.value());
  linearized_ = arser_.ParseLinearizedHeader();
  if (GetValidator()->has_read_problems())
    return kDataNotAvailable;
...
bool CPDF_DataAvail::CheckArrayPageNode(uint32_t dwPageNo,
  ...
  for (size_t i = 0; i < pArray->size(); ++i) {
    ...
    pNode->dw_page_no_ = pKid->GetRefObjNum();
...
bool CPDF_DataAvail::CheckUnknownPageNode(uint32_t dwPageNo,
  ...
  if (pPage->IsArray()) {
    pPageNode->dw_page_no_ = dwPageNo;
...
bool CPDF_DataAvail::CheckPageNode(const CPDF_DataAvail::PageNode& pageNode,
  ...
  for (int32_t i = 0; i < iSize; ++i) {
    ...
    if (pNode->type_ == PageNode::Type::kUnknown) {
      ...
      if (!CheckUnknownPageNode(pNode->dw_page_no_, pNode)) {
        return false;
      }
    }
    if (pNode->type_ == PageNode::Type::kArray) {
      // Updates a more specific type for the array page node.
      if (!CheckArrayPageNode(pNode->dw_page_no_, pNode)) {
        return false;
      }
    }
    switch (pNode->type_) {
      case PageNode::Type::kPage:
        ...
        if (iPage == iCount && document_) {
          document_->SetPageObjNum(iPage, pNode->dw_page_no_);
        }
        break;
...
CPDF_DataAvail::DocAvailStatus CPDF_DataAvail::CheckLinearizedData() {
  ...
  if (!main_xref_load_tried_) {
    ...
    FX_SAFE_SIZE_T data_size = dw_file_len_;
...
CPDF_DataAvail::DocAvailStatus CPDF_DataAvail::IsPageAvail(
  ...
  if (pdfium::Contains(ages_load_state_, dwPage)) {
    return kDataAvailable;
  }
  ...
  if (linearized_) {
    ...
    if (hint_tables_) {
      ...
      if (GetPageDictionary(dwPage)) {
        ages_load_state_.insert(dwPage);
        return kDataAvailable;
      }
    }
