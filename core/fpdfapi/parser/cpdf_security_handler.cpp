  ...
  do {
    ...
    for (int j = 0; j < 64; ++j) {
      content.insert(
          ...
          UNSAFE_TODO(password.unsigned_str() + password.GetLength()));
      content.insert(std::end(content), input.first(block_size).begin(),
                     input.first(block_size).end());
      if (vector.has_value()) {
...
bool CPDF_SecurityHandler::AES256_CheckPassword(const ByteString& password,
  ...
  uint8_t digest[32];
  if (m_Revision >= 6) {
    Revision6_Hash(password, pkey.subspan(32, 8),
                   bOwner ? std::optional(ukey.unsigned_span()) : std::nullopt,
                   digest);
  } else {
    CRYPT_SHA256Start(&sha);
    CRYPT_SHA256Update(&sha, password.unsigned_span());
  ...
  if (m_Revision >= 6) {
    Revision6_Hash(password, pkey.subspan<40, 8>(),
                   bOwner ? std::optional(ukey.unsigned_span()) : std::nullopt,
                   digest);
  } else {
    CRYPT_SHA256Start(&sha);
