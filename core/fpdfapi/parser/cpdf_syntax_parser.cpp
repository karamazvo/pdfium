...
RetainPtr<CPDF_Object> CPDF_SyntaxParser::GetObjectBodyInternal(
  ...
  if (word_result.is_number) {
    ...
    return pdfium::MakeRetain<CPDF_Reference>(m_pObjList, refnum);
  }
  ...
  if (word == "(") {
    return pdfium::MakeRetain<CPDF_String>(m_pObjList->GetByteStringPool(),
                                           ReadString());
  }
  if (word == "<") {
    return pdfium::MakeRetain<CPDF_String>(m_pObjList->GetByteStringPool(),
  ...
  if (word[0] == '/') {
    ...
    return pdfium::MakeRetain<CPDF_Name>(
        m_pObjList->GetByteStringPool(),
        PDF_NameDecode(ByteStringView(word_span).Substr(1)));
  }
  if (word == "<<") {
    RetainPtr<CPDF_Dictionary> pDict =
        pdfium::MakeRetain<CPDF_Dictionary>(m_pObjList->GetByteStringPool());
    while (true) {
