...
RetainPtr<CPDF_Object> CPDF_Parser::ParseIndirectObject(uint32_t objnum) {
  ...
  const auto* info = m_CrossRefTable->GetObjectInfo(objnum);
  if (!info || info->type == ObjectType::kFree) {
    return nullptr;
  }

  if (info->type == ObjectType::kNormal) {
    if (info->pos <= 0) {
      return nullptr;
    }
  ...
  CHECK_EQ(info->type, ObjectType::kCompressed);
  const CPDF_ObjectStream* obj_stream = GetObjectStream(info->archive.obj_num);
  if (!obj_stream) {
    return nullptr;
  }
