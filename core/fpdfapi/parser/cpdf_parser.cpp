...
bool CPDF_Parser::LoadCrossRefTable(FX_FILESIZE pos, bool skip) {
  ...
  bool success = MergeCrossRefObjectsData(objects);
  if (!success){
    return false;
  }
  return true;
...
void CPDF_Parser::MergeCrossRefObjectsData(
  ...
  for (const auto& obj : objects) {
    switch (obj.info.type) {
      ...
      bool success = cross_ref_table_->AddNormal(obj.obj_num, obj.info.gennum,
  ...
  for (CPDF_SyntaxParser::WordResult result = syntax_->GetNextWord();
    ...
    if (word == "(") {
      ...
      if (obj_num <= kMaxObjectNumber) {
        bool success = cross_ref_table->AddNormal(obj_num, gen_num, /*is_object_stream=*/false,
                                   obj_pos);
        if (!success){
          return false;
        }
