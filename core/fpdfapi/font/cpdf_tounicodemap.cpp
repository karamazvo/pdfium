...
ByteStringView CPDF_ToUnicodeMap::HandleBeginBFChar(
  ...
  const int expected_count = StringToInt(previous_word);
  bool is_valid = expected_count >= 0 && expected_count <= 100;
  if (is_valid) {
  ...
  while (true) {
    ...
    // Since `is_valid` is true, `expected_count` is not negative.
    if (code_words.size() > static_cast<size_t>(expected_count)) {
      is_valid = false;
    }
  }

  // Since `is_valid` is true, `expected_count` is not negative.
  if (is_valid && code_words.size() == static_cast<size_t>(expected_count)) {
...
ByteStringView CPDF_ToUnicodeMap::HandleBeginBFRange(
  ...
  while (true) {
    ...
    if (start == "[") {
      ...
      // Since `is_valid` is true, `expected_count` is not negative.
      if (ranges.size() > static_cast<size_t>(expected_count)) {
        is_valid = false;
        continue;
      }
  ...
  // Since `is_valid` is true, `expected_count` is not negative.
  if (is_valid && ranges.size() == static_cast<size_t>(expected_count)) {
