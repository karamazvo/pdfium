...
const std::array<uint8_t, 508> kCmap03Ttf = {
    {0x00, 0x01, 0x00, 0x00, 0x00, 0x08, 0x00, 0x80, 0x00, 0x03, 0x00, 0x00,
     ...
     0x00, 0x4d, 0x00, 0x43}};
const std::array<uint8_t, 508> kCmap31Ttf = {
    {0x00, 0x01, 0x00, 0x00, 0x00, 0x08, 0x00, 0x80, 0x00, 0x03, 0x00, 0x00,
...
TEST_F(CPDFTrueTypeFontTest, AllUnicodeCmapsTreatedEqually) {
  auto make_font =
      [](CPDF_TestDocument& doc,
         std::span<const uint8_t> font_data) -> RetainPtr<CPDF_Font> {
    ...
    if (font_file_stream_object_number <= 0u) {
      return nullptr;
    }
...
CMapName currentdict /CMap defineresource pop
    ...
    auto to_unicode_stream = doc.NewIndirect<CPDF_Stream>(
        DataVector<uint8_t>(std::begin(to_unicode), std::end(to_unicode)),
        pdfium::MakeRetain<CPDF_Dictionary>());
    if (!to_unicode_stream) {
      return nullptr;
    }
    const uint32_t to_unicode_object_number = to_unicode_stream->GetObjNum();
    if (to_unicode_object_number <= 0u) {
      return nullptr;
    }
    auto to_unicode_ref =
  ...
  CPDF_TestDocument doc;
  auto cmap31 = make_font(doc, std::span(kCmap31Ttf));
  auto cmap03 = make_font(doc, std::span(kCmap03Ttf));
  ASSERT_TRUE(cmap31);
  ASSERT_TRUE(cmap03);

  uint32_t period = 0x2E;
  bool isVert;
  int cmap31Glyph = cmap31->GlyphFromCharCode(period, &isVert);
  int cmap03Glyph = cmap03->GlyphFromCharCode(period, &isVert);

  EXPECT_EQ(cmap31Glyph, 1);
  EXPECT_EQ(cmap31Glyph, cmap03Glyph);
}
