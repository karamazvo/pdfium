...
void CPDF_Font::LoadFontDescriptor(const CPDF_Dictionary* pFontDesc) {
  m_Flags = pFontDesc->GetIntegerFor("Flags", pdfium::FontStyle::kNonSymbolic);
  int ItalicAngle = 0;
  ...
  if (ItalicAngle < 0) {
    m_Flags |= pdfium::FontStyle::kItalic;
...
uint32_t CPDF_Font::FallbackFontFromCharcode(uint32_t charcode) {
  if (m_FontFallbacks.empty()) {
    ...
    m_FontFallbacks[0]->LoadSubst(
        ...
        safeWeight.ValueOrDefault(pdfium::FontWeight::kNormal), m_ItalicAngle,
