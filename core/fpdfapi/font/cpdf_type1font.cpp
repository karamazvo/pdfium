...
bool CPDF_Type1Font::Load() {
  base_14font_ = CFX_FontMapper::GetStandardFontName(&base_font_name_);
  if (!IsBase14Font()) {
    return LoadCommon();
  }
  ...
  if (base_14font_ == CFX_FontMapper::kSymbol) {
    base_encoding_ = FontEncoding::kAdobeSymbol;
  } else if (base_14font_ == CFX_FontMapper::kDingbats) {
...
bool CPDF_Type1Font::IsSymbolicFont() const {
  return base_14font_.has_value() &&
         CFX_FontMapper::IsSymbolicFont(base_14font_.value());
}

bool CPDF_Type1Font::IsFixedFont() const {
  return base_14font_.has_value() &&
         CFX_FontMapper::IsFixedFont(base_14font_.value());
}
