...
void BinaryBuffer::DeleteBuf(size_t start_index, size_t count) {
  if (uffer_.empty() || count > GetSize() || start_index > GetSize() - count) {
    return;
  }
...
pdfium::span<uint8_t> BinaryBuffer::GetMutableSpan() {
  return pdfium::make_span(uffer_).first(GetSize());
}
...
DataVector<uint8_t> BinaryBuffer::DetachBuffer() {
  uffer_.resize(GetSize());
  data_size_ = 0;
  return std::move(uffer_);
}
...
void BinaryBuffer::ExpandBuf(size_t add_size) {
  ...
  new_size += add_size;
  if (uffer_.size() >= new_size.ValueOrDie()) {
    return;
  }

  size_t alloc_step = std::max(static_cast<size_t>(128),
                               alloc_step_ ? alloc_step_ : uffer_.size() / 4);
  new_size += alloc_step - 1;  // Quantize, don't combine these lines.
  new_size /= alloc_step;
  new_size *= alloc_step;
  uffer_.resize(new_size.ValueOrDie());
}
...
void BinaryBuffer::AppendSpan(pdfium::span<const uint8_t> span) {
  ...
  ExpandBuf(span.size());
  fxcrt::Copy(span, pdfium::make_span(uffer_).subspan(GetSize()));
