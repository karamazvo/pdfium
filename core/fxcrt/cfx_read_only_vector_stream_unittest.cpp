...
TEST(CFX_ReadOnlyVectorStreamTest, FromDataVector) {
  DataVector<uint8_t> data = {'a', 'b', 'c', 'd'};
  auto stream = pdfium::MakeRetain<CFX_ReadOnlyVectorStream>(std::move(data));
...
TEST(CFX_ReadOnlyVectorStreamTest, FromFixedSizeDataVector) {
...
TEST(CFX_ReadOnlyVectorStreamTest, ReadOverBoundary) {
...
#if BUILDFLAG(IS_POSIX)
TEST(CFX_ReadOnlyVectorStreamTest, FromFile) {
  char temp_name[] = "/tmp/pdfium_stream_XXXXXX";
  ...
  auto stream =
      pdfium::MakeRetain<CFX_ReadOnlyVectorStream>(ByteString(temp_name));
  ASSERT_TRUE(stream);
