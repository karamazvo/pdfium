...
FX_Random::FX_Random(uint32_t seed) : context_(FX_Alloc(MTContext, 1)) {
  context_->mt[0] = seed;
...
void FX_Random::Fill(pdfium::span<uint32_t> buffer) {
  ...
  for (size_t i = 0; i < buffer.size(); ++i) {
    buffer[i] = next_generator.Generate();
  }
}
...
void FX_Random::MTContextDeleter::operator()(MTContext* context) const {
  FX_Free(context);
}
