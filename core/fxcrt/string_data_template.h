...
class StringDataTemplate {
 public:
  ...
  pdfium::span<CharType> span() {
    // SAFETY: data_length_ is within m_String.
    return UNSAFE_BUFFERS(pdfium::make_span(m_String, data_length_));
  }
  ...
  pdfium::span<const CharType> span_with_terminator() const {
    // SAFETY: data_length_ is within m_String and there is always a
    // terminator character following it.
    return UNSAFE_BUFFERS(pdfium::make_span(m_String, data_length_ + 1));
  }

  pdfium::span<CharType> alloc_span() {
    // SAFETY: alloc_length_ is within m_String.
    return UNSAFE_BUFFERS(pdfium::make_span(m_String, alloc_length_));
  }
  ...
  pdfium::span<CharType> capacity_span() {
    // SAFETY: alloc_length_ + 1 is within m_String.
    return UNSAFE_BUFFERS(pdfium::make_span(m_String, alloc_length_ + 1));
  }
