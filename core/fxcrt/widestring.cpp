...
bool operator==(const WideString& lhs, WideStringView rhs) {
  if (!lhs.data_) {
    return rhs.IsEmpty();
  }

  // SAFTEY: Comparison ensure there are `data_length_` wchars in `rhs`
  // and `data_length_ is within `string_`.
  return lhs.data_->data_length_ == rhs.GetLength() &&
         UNSAFE_BUFFERS(FXSYS_wmemcmp(
             lhs.data_->string_, rhs.unterminated_c_str(), rhs.GetLength())) ==
             0;
}

bool operator==(const WideString& lhs, const WideString& rhs) {
  ...
  return lhs.data_->data_length_ == rhs.data_->data_length_ &&
         UNSAFE_BUFFERS(FXSYS_wmemcmp(lhs.data_->string_, rhs.data_->string_,
                                      rhs.data_->data_length_)) == 0;
}
