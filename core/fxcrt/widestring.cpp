...
WideString& WideString::operator+=(const WideString& str) {
  if (str.data_) {
    Concat(str.data_->m_String, str.data_->data_length_);
  }
...
bool WideString::operator==(const wchar_t* ptr) const {
  ...
  // and `data_length_` is within `m_String`.
  return UNSAFE_BUFFERS(wcslen(ptr)) == data_->data_length_ &&
         UNSAFE_BUFFERS(
             FXSYS_wmemcmp(ptr, data_->m_String, data_->data_length_)) == 0;
}
...
bool WideString::operator==(WideStringView str) const {
  ...
  return data_->data_length_ == str.GetLength() &&
         UNSAFE_BUFFERS(FXSYS_wmemcmp(data_->m_String, str.unterminated_c_str(),
                                      str.GetLength())) == 0;
}
...
bool WideString::operator==(const WideString& other) const {
  ...
  // SAFETY: data_length_ bytes available at m_String.
  return other.data_->data_length_ == data_->data_length_ &&
         UNSAFE_BUFFERS(FXSYS_wmemcmp(other.data_->m_String, data_->m_String,
...
int WideString::Compare(const wchar_t* str) const {
  if (data_) {
    // SAFETY: required from caller.
    return str ? UNSAFE_BUFFERS(wcscmp(data_->m_String, str)) : 1;
  }
...
int WideString::Compare(const WideString& str) const {
  ...
  int result = UNSAFE_BUFFERS(
      FXSYS_wmemcmp(data_->m_String, str.data_->m_String, min_len));
  if (result != 0)
    return result;
...
int WideString::CompareNoCase(const wchar_t* str) const {
  if (data_) {
    return str ? FXSYS_wcsicmp(data_->m_String, str) : 1;
  }
  return (!str || str[0] == 0) ? 0 : -1;
}
...
int WideString::GetInteger() const {
  return data_ ? StringToInt(data_->m_String) : 0;
}
