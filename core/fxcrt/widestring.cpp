...
std::optional<size_t> GuessSizeForVSWPrintf(const wchar_t* pFormat,
  ...
  for (WideStringView view(pFormat); !view.IsEmpty(); view = view.Substr(1u)) {
    ...
    WSTRModifiers modifier;
    wchar_t c = view.Front();
    if (view.First(3u) == L"I64") {
      view = view.Substr(3u);
      modifier.force_int64 = true;
    } else {
      if (c == 'h' || c == 'l' || c == 'F' || c == 'N' || c == 'L') {
        if (c == 'h') {
          modifier.force_ansi = true;
        } else if (c == 'l') {
          modifier.force_unicode = true;
        }
        view = view.Substr(1u);
      }
    }
    ...
    if (c == 'c' || c == 'C') {
      ...
      bool use_wide = false;
      if (modifier.force_unicode) {
        use_wide = true;
      } else if (modifier.force_ansi) {
        use_wide = false;
      } else {
        use_wide = (c == 's');
      }
