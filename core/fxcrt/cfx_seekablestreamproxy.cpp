...
CFX_SeekableStreamProxy::CFX_SeekableStreamProxy(
  ...
  if (bom == BOM_UTF8) {
    wbomlength_ = 3;
    wcode_page_ = FX_CodePage::kUTF8;
  } else {
    bom &= BOM_UTF16_MASK;
    if (bom == BOM_UTF16_BE) {
      wbomlength_ = 2;
      wcode_page_ = FX_CodePage::kUTF16BE;
    } else if (bom == BOM_UTF16_LE) {
      wbomlength_ = 2;
      wcode_page_ = FX_CodePage::kUTF16LE;
    } else {
      wbomlength_ = 0;
      wcode_page_ = FX_GetACP();
    }
  }

  Seek(From::Begin, static_cast<FX_FILESIZE>(wbomlength_));
}
...
size_t CFX_SeekableStreamProxy::ReadBlock(pdfium::span<wchar_t> buffer) {
  ...
  if (wcode_page_ == FX_CodePage::kUTF16LE ||
      wcode_page_ == FX_CodePage::kUTF16BE) {
    ...
    if (wcode_page_ == FX_CodePage::kUTF16BE) {
  ...
  if (wcode_page_ != FX_CodePage::kUTF8) {
    return 0;
  }
