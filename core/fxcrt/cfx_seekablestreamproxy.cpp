...
CFX_SeekableStreamProxy::CFX_SeekableStreamProxy(
  ...
  if (bom == BOM_UTF8) {
    w_bomlength_ = 3;
    w_code_page_ = FX_CodePage::kUTF8;
  } else {
    bom &= BOM_UTF16_MASK;
    if (bom == BOM_UTF16_BE) {
      w_bomlength_ = 2;
      w_code_page_ = FX_CodePage::kUTF16BE;
    } else if (bom == BOM_UTF16_LE) {
      w_bomlength_ = 2;
      w_code_page_ = FX_CodePage::kUTF16LE;
    } else {
      w_bomlength_ = 0;
      w_code_page_ = FX_GetACP();
    }
  }

  Seek(From::Begin, static_cast<FX_FILESIZE>(w_bomlength_));
}
...
size_t CFX_SeekableStreamProxy::ReadBlock(pdfium::span<wchar_t> buffer) {
  ...
  if (w_code_page_ == FX_CodePage::kUTF16LE ||
      w_code_page_ == FX_CodePage::kUTF16BE) {
    ...
    if (w_code_page_ == FX_CodePage::kUTF16BE) {
  ...
  if (w_code_page_ != FX_CodePage::kUTF8) {
    return 0;
  }
