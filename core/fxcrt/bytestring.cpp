...
bool ByteString::operator==(const char* ptr) const {
  ...
  // SAFETY: `data_length_` is within `m_String`, and the strlen() call
  ...
  return UNSAFE_BUFFERS(strlen(ptr)) == data_->data_length_ &&
         UNSAFE_BUFFERS(
             FXSYS_memcmp(ptr, data_->m_String, data_->data_length_)) == 0;
}
...
bool ByteString::operator==(ByteStringView str) const {
  ...
  // `data_length_` is within `m_String`, and equality comparison.
  return data_->data_length_ == str.GetLength() &&
         UNSAFE_BUFFERS(FXSYS_memcmp(data_->m_String, str.unterminated_c_str(),
                                     str.GetLength())) == 0;
}
...
bool ByteString::operator==(const ByteString& other) const {
  ...
  // SAFETY: data_length_ describes the length of m_String.
  return other.data_->data_length_ == data_->data_length_ &&
         UNSAFE_BUFFERS(memcmp(other.data_->m_String, data_->m_String,
...
int ByteString::Compare(ByteStringView str) const {
  ...
  // SAFETY: Comparison limited to minimum valid length of either argument.
  int result = UNSAFE_BUFFERS(
      FXSYS_memcmp(data_->m_String, str.unterminated_c_str(), min_len));
  if (result != 0)
    return result;
