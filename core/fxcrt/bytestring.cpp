...
bool operator==(const ByteString& lhs, ByteStringView rhs) {
  if (!lhs.data_) {
    return rhs.IsEmpty();
  }

  // SAFETY: `rhs` has `GetLength()` valid bytes in `unterminated_c_str()`,
...
bool operator==(const ByteString& lhs, const ByteString& rhs) {
  ...
  return lhs.data_->data_length_ == rhs.data_->data_length_ &&
         UNSAFE_BUFFERS(memcmp(lhs.data_->string_, rhs.data_->string_,
