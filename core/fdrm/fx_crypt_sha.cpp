...
void SHA1Transform(pdfium::span<uint32_t, 5> digest,
  ...
  uint32_t round_logic;
  uint32_t round_const;

  for (size_t t = 0; t < 80; ++t) {
    if (t < 20) {
      round_logic = (dig[1] & dig[2]) | (~dig[1] & dig[3]);
      round_const = 0x5a827999;
    } else if (t < 40) {
      round_logic = dig[1] ^ dig[2] ^ dig[3];
      round_const = 0x6ed9eba1;
    } else if (t < 60) {
      round_logic = (dig[1] & dig[2]) | (dig[1] & dig[3]) | (dig[2] & dig[3]);
    ...
    const uint32_t tmp =
        sha256::Rol(dig[0], 5) + round_logic + dig[4] + w[t] + round_const;
    dig[4] = dig[3];
    dig[3] = dig[2];
...
void CRYPT_SHA1Finish(CRYPT_sha1_context* context,
  ...
  uint8_t pad = (context->blkused < 56 ? 56 : 120) - context->blkused;
