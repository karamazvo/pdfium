...
<<<<<<< PATCH SET (6a2146740c45959b4f517f376152bde6ff9fa9ae Clean up SHA-256 and SHA-384 implementations)
namespace sha384 {
||||||| BASE      (dfca4b449c987dccd92251d92f15d7127af45515 Replace SHA macros with helper functions in fx_crypt_sha.cpp)
=======
<<<<<<< PATCH SET (dfca4b449c987dccd92251d92f15d7127af45515 Replace SHA macros with helper functions in fx_crypt_sha.cpp)
||||||| BASE      (f14af1cc27a3f6165624125a830dec4eabe00d53 Remove UNSAFE_BUFFERS in fx_crypt_sha.cpp)
...
>>>>>>> BASE      (ba6803772a31775884cfd7750af2dafcbaedb1d2 Remove UNSAFE_BUFFERS in fx_crypt_sha.cpp)
>>>>>>> BASE      (7b1e5df7c1d86c7b42af8ebb19c58703d0a1bfcc Replace SHA macros with helper functions in fx_crypt_sha.cpp)
...
void SHA_Core_Init(pdfium::span<uint32_t, 5> h) {
  fxcrt::Copy({0x67452301, 0xefcdab89, 0x98badcfe, 0x10325476, 0xc3d2e1f0}, h);
}
...
void sha256_process(CRYPT_sha2_context* ctx,
  ...
  for (size_t i = 0; i < 8; i++) {
    state[i] = static_cast<uint32_t>(ctx->state[i]);
  }
  for (size_t i = 0; i < 64; ++i) {
    ...
    uint32_t temp1 = state[(15 - i) % 8] +
                     sha256::UpperSigma1(state[(12 - i) % 8]) +
                     sha256::Ch(state[(12 - i) % 8], state[(13 - i) % 8],
                                state[(14 - i) % 8]) +
    ...
    uint32_t temp2 = sha256::UpperSigma0(state[(8 - i) % 8]) +
                     sha256::Maj(state[(8 - i) % 8], state[(9 - i) % 8],
                                 state[(10 - i) % 8]);

    state[(11 - i) % 8] += temp1;
    state[(15 - i) % 8] = temp1 + temp2;
  }
  for (size_t i = 0; i < 8; ++i) {
    ctx->state[i] += state[i];
  }
...
void sha384_process(CRYPT_sha2_context* ctx,
  ...
  for (size_t i = 0; i < 80; ++i) {
    ...
    uint64_t temp1 = state[(15 - i) % 8] +
                     sha384::UpperSigma1(state[(12 - i) % 8]) +
                     sha384::Ch(state[(12 - i) % 8], state[(13 - i) % 8],
                                state[(14 - i) % 8]) +
                     word + constants[i];

    uint64_t temp2 = sha384::UpperSigma0(state[(8 - i) % 8]) +
                     sha384::Maj(state[(8 - i) % 8], state[(9 - i) % 8],
                                 state[(10 - i) % 8]);
...
void CRYPT_SHA1Finish(CRYPT_sha1_context* context,
  ...
  uint8_t pad = (context->blkused >= 56 ? 120 : 56) - context->blkused;
  std::ranges::fill(pdfium::span(data).first(pad), 0);
  data[0] = 0x80;
  CRYPT_SHA1Update(context, pdfium::span(data).first(pad));
  for (size_t i = 0; i < 8; ++i) {
    data[i] = (total_bits >> (56 - 8 * i)) & 0xFF;
  }
  CRYPT_SHA1Update(context, pdfium::span(data).first<8u>());
  for (int i = 0; i < 5; i++) {
...
void CRYPT_SHA384Start(CRYPT_sha2_context* context) {
  context->total_bytes = 0;
  context->state = {0xcbbb9d5dc1059ed8ULL, 0x629a292a367cd507ULL,
...
void CRYPT_SHA512Start(CRYPT_sha2_context* context) {
  context->total_bytes = 0;
  context->state = {0x6a09e667f3bcc908ULL, 0xbb67ae8584caa73bULL,
