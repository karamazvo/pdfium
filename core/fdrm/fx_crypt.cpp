...
void md5_process(CRYPT_md5_context* ctx, pdfium::span<const uint8_t, 64> data) {
  ...
  for (size_t i = 0; i < 16; ++i) {
    data_32[i] = fxcrt::GetUInt32LSBFirst(data.subspan<4 * i, 4>());
  }
  ...
  constexpr uint32_t kShifts[4][4] = {
      {7, 12, 17, 22},
      {5, 9, 14, 20},
      {4, 11, 16, 23},
      {6, 10, 15, 21},
  ...
  for (size_t i = 0; i < 64; ++i) {
    state[(16 - i) % 4] += F(state[(17 - i) % 4], state[(18 - i) % 4],
                             state[(19 - i) % 4], i / 16) +
                           data_32[GetK(i)] + kMd5Constants[i];
    state[(16 - i) % 4] =
        S(state[(16 - i) % 4], kShifts[i / 16][i % 4]) + state[(17 - i) % 4];
  }
...
void CRYPT_MD5Finish(CRYPT_md5_context* context,
  ...
  for (size_t i = 0; i < 4; ++i) {
    fxcrt::PutUInt32LSBFirst(context->state[i], digest.subspan<4 * i, 4>());
  }
}
