...
void CRYPT_AESSetKey(CRYPT_aes_context* ctx, pdfium::span<const uint8_t> key) {
  ...
  ctx->Nb_ = 4;
  ctx->Nr_ = 6 + (ctx->Nb_ > Nk ? ctx->Nb_ : Nk);
  int rconst = 1;
  for (size_t i = 0; i < (ctx->Nr_ + 1) * ctx->Nb_; i++) {
    if (i < Nk) {
      ctx->keysched[i] =
  ...
  for (size_t i = 0; i <= ctx->Nr_; i++) {
    for (size_t j = 0; j < ctx->Nb_; j++) {
      uint32_t temp = ctx->keysched[(ctx->Nr_ - i) * ctx->Nb_ + j];
      if (i != 0 && i != ctx->Nr_) {
        int a = (temp >> 24) & 0xFF;
        int b = (temp >> 16) & 0xFF;
        int c = (temp >> 8) & 0xFF;
      ...
      ctx->invkeysched[i * ctx->Nb_ + j] = temp;
    }
  }
}
  ...
  UNSAFE_TODO({
    ...
    while (src.size() != 0) {
      for (size_t i = 0; i < 4; i++) {
        x[i] = ct[i] =
            fxcrt::GetUInt32MSBFirst(src.subspan(4 * i, 4u).first<4u>());
      }
      aes_decrypt_nb_4(ctx, x);
      for (size_t i = 0; i < 4; i++) {
        fxcrt::PutUInt32MSBFirst(iv[i] ^ x[i], dest.subspan(4 * i).first<4u>());
        iv[i] = ct[i];
      }
      dest = dest.subspan<16u>();
      src = src.subspan<16u>();
    }
