...
void CFWL_ScrollBar::DrawUpperTrack(CFGAS_GEGraphics* pGraphics,
  ...
  CFWL_ThemeBackground param(CFWL_ThemePart::Part::kUpperTrack, this,
                             pGraphics);
  param.dw_states_ = (properties_.dw_states_ & FWL_STATE_WGT_Disabled)
...
void CFWL_ScrollBar::CalcButtonLen() {
  fbutton_len_ = IsVertical() ? client_rect_.width : client_rect_.height;
  float fLength = IsVertical() ? client_rect_.height : client_rect_.width;
  if (fLength < fbutton_len_ * 2) {
    fbutton_len_ = fLength / 2;
    min_size_ = true;
  } else {
    min_size_ = false;
  }
...
CFX_RectF CFWL_ScrollBar::CalcMinButtonRect() {
  if (IsVertical())
    return CFX_RectF(client_rect_.TopLeft(), client_rect_.width, fbutton_len_);
  return CFX_RectF(client_rect_.TopLeft(), fbutton_len_, client_rect_.height);
}

CFX_RectF CFWL_ScrollBar::CalcMaxButtonRect() {
  if (IsVertical()) {
    return CFX_RectF(client_rect_.left, client_rect_.bottom() - fbutton_len_,
                     client_rect_.width, fbutton_len_);
  }
  return CFX_RectF(client_rect_.right() - fbutton_len_, client_rect_.top,
                   fbutton_len_, client_rect_.height);
}
...
CFX_RectF CFWL_ScrollBar::CalcThumbButtonRect(const CFX_RectF& rtThumb) {
  ...
  float fRange = frange_max_ - frange_min_;
  if (fRange < 0) {
    if (IsVertical()) {
  ...
  float fTrackPos = std::clamp(ftrack_pos_, frange_min_, frange_max_);
  if (!fRange)
    return rect;

  float iPos = fSize + fDiff * (fTrackPos - frange_min_) / fRange;
...
float CFWL_ScrollBar::GetTrackPointPos(const CFX_PointF& point) {
  CFX_PointF diff = point - cp_track_point_;
  float fRange = frange_max_ - frange_min_;
  float fPos;
  ...
  fPos += flast_track_pos_;
  return std::clamp(fPos, frange_min_, frange_max_);
}

bool CFWL_ScrollBar::SendEvent() {
  if (min_button_state_ == CFWL_PartState::kPressed) {
    DoScroll(CFWL_EventScroll::Code::StepBackward, ftrack_pos_);
    return false;
  }
  if (max_button_state_ == CFWL_PartState::kPressed) {
    DoScroll(CFWL_EventScroll::Code::StepForward, ftrack_pos_);
    return false;
  }
  if (min_track_state_ == CFWL_PartState::kPressed) {
    DoScroll(CFWL_EventScroll::Code::PageBackward, ftrack_pos_);
    return thumb_rect_.Contains(cp_track_point_);
  }
  if (max_track_state_ == CFWL_PartState::kPressed) {
    DoScroll(CFWL_EventScroll::Code::PageForward, ftrack_pos_);
    return thumb_rect_.Contains(cp_track_point_);
  }
  if (mouse_wheel_) {
...
void CFWL_ScrollBar::OnLButtonDown(const CFX_PointF& point) {
  ...
  cp_track_point_ = point;
  flast_track_pos_ = ftrack_pos_;
...
void CFWL_ScrollBar::DoMouseUp(int32_t iItem,
  ...
  *pState = iNewState;
  RepaintRect(rtItem);
  OnScroll(CFWL_EventScroll::Code::EndScroll, ftrack_pos_);
}
...
void CFWL_ScrollBar::DoMouseMove(int32_t iItem,
  ...
  if (!mouse_down_) {
    ...
    *pState = iNewState;
    ...
    ftrack_pos_ = GetTrackPointPos(point);
    OnScroll(CFWL_EventScroll::Code::TrackPos, ftrack_pos_);
  }
}
