...
class CFWL_Widget : public cppgc::GarbageCollected<CFWL_Widget>,
 ...
 public:
  ...
  class Properties {
   public:
    uint32_t dw_styles_ = FWL_STYLE_WGT_Child;  // Mask of FWL_STYLE_*_*.
    uint32_t dw_style_exts_ = 0;                // Mask of FWL_STYLEEXT_*_*.
    uint32_t dw_states_ = 0;                    // Mask of FWL_STATE_*_*.
  };
  ...
  uint32_t GetStyleExts() const { return properties_.dw_style_exts_; }
  uint32_t GetStates() const { return properties_.dw_states_; }
