...
void CFWL_MonthCalendar::DrawLButton(CFGAS_GEGraphics* pGraphics,
  ...
  params.dw_states_ = lbtn_part_states_;
...
void CFWL_MonthCalendar::DrawRButton(CFGAS_GEGraphics* pGraphics,
  ...
  params.dw_states_ = rbtn_part_states_;
...
void CFWL_MonthCalendar::DrawCaption(CFGAS_GEGraphics* pGraphics,
  ...
  textParam.ws_text_ = GetHeadText(cur_year_, cur_month_);
  head_size_ = CalcTextSize(textParam.ws_text_, false);
  CalcHeadSize();
  textParam.part_rect_ = head_text_rect_;
  textParam.dw_ttostyles_.single_line_ = true;
  textParam.ttoalign_ = FDE_TextAlignment::kCenter;
...
void CFWL_MonthCalendar::DrawDatesInBK(CFGAS_GEGraphics* pGraphics,
  ...
  for (int32_t j = 0; j < iCount; j++) {
    ...
    if (pDataInfo->bSelected) {
      params.dw_states_ |= CFWL_PartState::kSelected;
      if (pDataInfo->bFlagged) {
        params.dw_states_ |= CFWL_PartState::kFlagged;
      }
    } else if (j == hovered_ - 1) {
      params.dw_states_ |= CFWL_PartState::kHovered;
    ...
    pTheme->DrawBackground(params);
    params.dw_states_ = CFWL_PartState::kNormal;
  }
}
...
void CFWL_MonthCalendar::DrawWeek(CFGAS_GEGraphics* pGraphics,
  ...
  params.ttoalign_ = FDE_TextAlignment::kCenter;
  params.dw_ttostyles_.single_line_ = true;
  params.matrix_ = mtMatrix;
  ...
  for (int32_t i = 0; i < 7; ++i) {
    ...
    params.ws_text_ = GetAbbreviatedDayOfWeek(i);
...
void CFWL_MonthCalendar::DrawToday(CFGAS_GEGraphics* pGraphics,
  ...
  params.ttoalign_ = FDE_TextAlignment::kCenterLeft;
  params.ws_text_ = GetTodayText(year_, month_, day_);
  today_size_ = CalcTextSize(params.ws_text_, false);
  CalcTodaySize();
...
void CFWL_MonthCalendar::DrawDatesIn(CFGAS_GEGraphics* pGraphics,
  ...
  for (int32_t j = 0; j < iCount; j++) {
    DATEINFO* pDataInfo = date_array_[j].get();
    params.ws_text_ = pDataInfo->wsDay;
    params.part_rect_ = pDataInfo->rect;
    params.dw_states_ = pDataInfo->AsPartStateMask();
    if (j + 1 == hovered_) {
      params.dw_states_ |= CFWL_PartState::kHovered;
    }
...
CFX_SizeF CFWL_MonthCalendar::CalcSize() {
  ...
  ws_today_ = GetTodayText(year_, month_, day_);
  today_size_ = CalcTextSize(ws_today_, false);
...
void CFWL_MonthCalendar::InitDate() {
  ...
  ws_today_ = GetTodayText(year_, month_, day_);
  ws_head_ = GetHeadText(cur_year_, cur_month_);
...
void CFWL_MonthCalendar::OnLButtonUp(CFWL_MessageMouse* pMsg) {
  ...
  if (rbtn_rect_.Contains(pMsg->os_)) {
    rbtn_part_states_ = CFWL_PartState::kNormal;
    RepaintRect(rbtn_rect_);
    return;
  }
  if (today_rect_.Contains(pMsg->os_)) {
    return;
  }
  ...
  int32_t iCurSel = GetDayAtPoint(pMsg->os_);
  if (iCurSel > 0) {
...
void CFWL_MonthCalendar::OnMouseMove(CFWL_MessageMouse* pMsg) {
  ...
  if (dates_rect_.Contains(pMsg->os_)) {
    int32_t iHover = GetDayAtPoint(pMsg->os_);
