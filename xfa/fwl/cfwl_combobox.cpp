...
void CFWL_ComboBox::ModifyStyleExts(uint32_t dwStyleExtsAdded,
  ...
  properties_.dw_style_exts_ |= FWL_STYLEEXT_CMB_DropDown;
  if (bAddDropDown)
...
void CFWL_ComboBox::ResetEditAlignment() {
  ...
  uint32_t dwAdd = 0;
  switch (properties_.dw_style_exts_ & FWL_STYLEEXT_CMB_EditHAlignMask) {
  ...
  switch (properties_.dw_style_exts_ & FWL_STYLEEXT_CMB_EditVAlignMask) {
  ...
  if (properties_.dw_style_exts_ & FWL_STYLEEXT_CMB_EditJustified) {
...
void CFWL_ComboBox::ResetListItemAlignment() {
  ...
  uint32_t dwAdd = 0;
  switch (properties_.dw_style_exts_ & FWL_STYLEEXT_CMB_ListItemAlignMask) {
...
void CFWL_ComboBox::OnProcessMessage(CFWL_Message* pMessage) {
  ...
  switch (pMessage->GetType()) {
    ...
    case CFWL_Message::Type::kKey: {
      ...
      if (IsDropListVisible() &&
          pKey->dw_cmd_ == CFWL_MessageKey::KeyCommand::kKeyDown) {
        bool bListKey = pKey->dw_key_code_or_char_ == XFA_FWL_VKEY_Up ||
                        pKey->dw_key_code_or_char_ == XFA_FWL_VKEY_Down ||
                        pKey->dw_key_code_or_char_ == XFA_FWL_VKEY_Return ||
                        pKey->dw_key_code_or_char_ == XFA_FWL_VKEY_Escape;
        if (bListKey) {
...
void CFWL_ComboBox::OnLButtonUp(CFWL_MessageMouse* pMsg) {
  if (btn_rect_.Contains(pMsg->os_)) {
...
void CFWL_ComboBox::OnKey(CFWL_MessageKey* pMsg) {
  uint32_t dwKeyCode = pMsg->dw_key_code_or_char_;
