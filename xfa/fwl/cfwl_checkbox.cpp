...
void CFWL_CheckBox::Layout() {
  ...
  float fTextLeft = client_rect_.left + fbox_height_;
  box_rect_ = CFX_RectF(client_rect_.TopLeft(), fbox_height_, fbox_height_);
  caption_rect_ =
  ...
  CalcTextRect(WideString::FromASCII("Check box"), ttostyles_, ttoalign_,
               &rtFocus);
...
Mask<CFWL_PartState> CFWL_CheckBox::GetPartStates() const {
  ...
  if ((properties_.dw_states_ & FWL_STATE_CKB_CheckMask) ==
      FWL_STATE_CKB_Neutral) {
    dwStates = CFWL_PartState::kNeutral;
  } else if ((properties_.dw_states_ & FWL_STATE_CKB_CheckMask) ==
  ...
  } else if (properties_.dw_states_ & FWL_STATE_CKB_Hovered) {
    dwStates |= CFWL_PartState::kHovered;
  } else if (properties_.dw_states_ & FWL_STATE_CKB_Pressed) {
...
void CFWL_CheckBox::UpdateTextOutStyles() {
  ttoalign_ = FDE_TextAlignment::kTopLeft;
  ttostyles_.Reset();
  ttostyles_.single_line_ = true;
}

void CFWL_CheckBox::NextStates() {
  uint32_t dwFirststate = properties_.dw_states_;
  if (properties_.dw_style_exts_ & FWL_STYLEEXT_CKB_RadioButton) {
    if ((properties_.dw_states_ & FWL_STATE_CKB_CheckMask) ==
  ...
  RepaintRect(client_rect_);
  if (dwFirststate == properties_.dw_states_) {
    return;
  }
...
void CFWL_CheckBox::OnLButtonDown() {
  ...
  btn_down_ = true;
  properties_.dw_states_ &= ~FWL_STATE_CKB_Hovered;
  properties_.dw_states_ |= FWL_STATE_CKB_Pressed;
  RepaintRect(client_rect_);
}
...
void CFWL_CheckBox::OnLButtonUp(CFWL_MessageMouse* pMsg) {
  ...
  btn_down_ = false;
  if (!client_rect_.Contains(pMsg->os_)) {
    return;
  }
...
void CFWL_CheckBox::OnMouseMove(CFWL_MessageMouse* pMsg) {
  ...
  if (btn_down_) {
    if (client_rect_.Contains(pMsg->os_)) {
      if ((properties_.dw_states_ & FWL_STATE_CKB_Pressed) == 0) {
        bRepaint = true;
        properties_.dw_states_ |= FWL_STATE_CKB_Pressed;
      }
      if ((properties_.dw_states_ & FWL_STATE_CKB_Hovered)) {
        bRepaint = true;
        properties_.dw_states_ &= ~FWL_STATE_CKB_Hovered;
      }
    } else {
      if (properties_.dw_states_ & FWL_STATE_CKB_Pressed) {
        bRepaint = true;
        properties_.dw_states_ &= ~FWL_STATE_CKB_Pressed;
      }
      if ((properties_.dw_states_ & FWL_STATE_CKB_Hovered) == 0) {
        bRepaint = true;
        properties_.dw_states_ |= FWL_STATE_CKB_Hovered;
      }
    }
...
void CFWL_CheckBox::OnKeyDown(CFWL_MessageKey* pMsg) {
  if (pMsg->dw_key_code_or_char_ == XFA_FWL_VKEY_Tab) {
    return;
  }
  if (pMsg->dw_key_code_or_char_ == XFA_FWL_VKEY_Return ||
      pMsg->dw_key_code_or_char_ == XFA_FWL_VKEY_Space) {
    NextStates();
  }
}
