...
void CFWL_Barcode::GenerateBarcodeImageCache() {
  ...
  if (c_start_char_.has_value()) {
    barcode_engine_->SetStartChar(c_start_char_.value());
  }
  if (c_end_char_.has_value()) {
    barcode_engine_->SetEndChar(c_end_char_.value());
  }
