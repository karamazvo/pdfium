...
void CFWL_Barcode::GenerateBarcodeImageCache() {
  ...
  if (cstart_char_.has_value()) {
    barcode_engine_->SetStartChar(cstart_char_.value());
  }
  if (cend_char_.has_value()) {
    barcode_engine_->SetEndChar(cend_char_.value());
  }
