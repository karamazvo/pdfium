...
void CFWL_Barcode::GenerateBarcodeImageCache() {
  ...
  if (m_cStartChar.has_value())
    barcode_engine_->SetStartChar(m_cStartChar.value());
  if (m_cEndChar.has_value())
    barcode_engine_->SetEndChar(m_cEndChar.value());
  if (m_nECLevel.has_value())
    barcode_engine_->SetErrorCorrectionLevel(m_nECLevel.value());
...
void CFWL_Barcode::CreateBarcodeEngine() {
  if (barcode_engine_ || m_type == BC_TYPE::kUnknown) {
    return;
  }

  barcode_engine_ = CFX_Barcode::Create(m_type);
}
