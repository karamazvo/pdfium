...
class CFWL_ListBox : public CFWL_Widget {
 public:
  class Item {
   public:
    ...
    WideString GetText() const { return ws_text_; }

   private:
    bool is_selected_ = false;
    bool is_focused_ = false;
    CFX_RectF item_rect_;
    const WideString ws_text_;
  };
 ...
 private:
  ...
  FDE_TextStyle ttostyles_;
  FDE_TextAlignment ttoaligns_ = FDE_TextAlignment::kTopLeft;
  bool lbutton_down_ = false;
  float fitem_height_ = 0.0f;
  float fscorll_bar_width_ = 0.0f;
  std::vector<std::unique_ptr<Item>> item_array_;  // Must outlive `hanchor_`.
  UnownedPtr<Item> hanchor_;
};
