...
Mask<CFWL_PartState> CFWL_PushButton::GetPartStates() {
  ...
  } else if (properties_.dw_states_ & FWL_STATE_PSB_Pressed) {
    dwStates |= CFWL_PartState::kPressed;
  } else if (properties_.dw_states_ & FWL_STATE_PSB_Hovered) {
...
void CFWL_PushButton::OnLButtonDown(CFWL_MessageMouse* pMsg) {
  btn_down_ = true;
  properties_.dw_states_ |= FWL_STATE_PSB_Hovered;
  properties_.dw_states_ |= FWL_STATE_PSB_Pressed;
  RepaintRect(client_rect_);
}
...
void CFWL_PushButton::OnMouseMove(CFWL_MessageMouse* pMsg) {
  ...
  if (btn_down_) {
    if (client_rect_.Contains(pMsg->os_)) {
      if ((properties_.dw_states_ & FWL_STATE_PSB_Pressed) == 0) {
        properties_.dw_states_ |= FWL_STATE_PSB_Pressed;
        bRepaint = true;
      }
      if (properties_.dw_states_ & FWL_STATE_PSB_Hovered) {
        properties_.dw_states_ &= ~FWL_STATE_PSB_Hovered;
        bRepaint = true;
      }
    } else {
      if (properties_.dw_states_ & FWL_STATE_PSB_Pressed) {
        properties_.dw_states_ &= ~FWL_STATE_PSB_Pressed;
        bRepaint = true;
      }
      if ((properties_.dw_states_ & FWL_STATE_PSB_Hovered) == 0) {
        properties_.dw_states_ |= FWL_STATE_PSB_Hovered;
        bRepaint = true;
      }
    }
