...
class CFWL_ScrollBar final : public CFWL_Widget,
 ...
 public:
  ...
  float GetPageSize() const { return fpage_size_; }
  void SetPageSize(float fPageSize) { fpage_size_ = fPageSize; }
  float GetStepSize() const { return fstep_size_; }
  void SetStepSize(float fStepSize) { fstep_size_ = fStepSize; }
  float GetPos() const { return fpos_; }
  void SetPos(float fPos) { fpos_ = fPos; }
 ...
 private:
  ...
  float frange_min_ = 0.0f;
  float frange_max_ = -1.0f;
  float fpage_size_ = 0.0f;
  float fstep_size_ = 0.0f;
  float fpos_ = 0.0f;
  float ftrack_pos_ = 0.0f;
  ...
  float flast_track_pos_ = 0.0f;
  CFX_PointF cp_track_point_;
  int32_t mouse_wheel_ = 0;
  float fbutton_len_ = 0.0f;
