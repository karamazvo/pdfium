    ...
    case CFWL_ThemePart::Part::kBackground: {
      ...
      if (pParams.rt_data_) {
        FillSolidRect(pParams.GetGraphics(), FWLTHEME_COLOR_Background,
                      *pParams.rt_data_, pParams.matrix_);
      }
      break;
    }
    case CFWL_ThemePart::Part::kListItem: {
      DrawListBoxItem(pParams.GetGraphics(), pParams.dw_states_,
                      pParams.part_rect_, pParams.rt_data_, pParams.matrix_);
      break;
    }
    case CFWL_ThemePart::Part::kCheck: {
      uint32_t color = 0xFF000000;
      if (pParams.dw_states_ == CFWL_PartState::kChecked) {
        color = 0xFFFF0000;
      } else if (pParams.dw_states_ == CFWL_PartState::kNormal) {
        color = 0xFF0000FF;
      }
