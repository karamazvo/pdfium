...
void CFWL_CheckBoxTP::DrawText(const CFWL_ThemeText& pParams) {
  ...
  text_out_->SetTextColor(pParams.dw_states_ & CFWL_PartState::kDisabled
...
void CFWL_CheckBoxTP::DrawBackground(const CFWL_ThemeBackground& pParams) {
  ...
  if ((pParams.dw_states_ & CFWL_PartState::kChecked) ||
      (pParams.dw_states_ & CFWL_PartState::kNeutral)) {
    DrawCheckSign(pParams.GetWidget(), pParams.GetGraphics(),
                  pParams.part_rect_, pParams.dw_states_, pParams.matrix_);
  }
}
