...
void CFWL_NoteDriver::Trace(cppgc::Visitor* visitor) const {
  ...
  visitor->Trace(grab_);
  ContainerTrace(visitor, vent_targets_);
}

void CFWL_NoteDriver::SendEvent(CFWL_Event* pNote) {
  for (const auto& pair : vent_targets_) {
...
void CFWL_NoteDriver::RegisterEventTarget(CFWL_Widget* pListener,
  ...
  if (!vent_targets_[key]) {
    vent_targets_[key] = cppgc::MakeGarbageCollected<Target>(
        app_->GetHeap()->GetAllocationHandle(), pListener);
  }
  vent_targets_[key]->SetEventSource(pEventSource);
}
...
void CFWL_NoteDriver::UnregisterEventTarget(CFWL_Widget* pListener) {
  ...
  auto it = vent_targets_.find(key);
  if (it != vent_targets_.end()) {
...
bool CFWL_NoteDriver::DoKey(CFWL_Message* pMessage, CFWL_Widget* pMessageForm) {
  ...
  if (pMsg->dw_cmd_ == CFWL_MessageKey::KeyCommand::kKeyDown &&
      pMsg->dw_key_code_or_char_ == XFA_FWL_VKEY_Tab) {
    return true;
  }
#endif
  ...
  if (pMsg->dw_cmd_ == CFWL_MessageKey::KeyCommand::kKeyDown &&
      pMsg->dw_key_code_or_char_ == XFA_FWL_VKEY_Return) {
...
bool CFWL_NoteDriver::DoMouse(CFWL_Message* pMessage,
  ...
  if (pMsg->dw_cmd_ == CFWL_MessageMouse::MouseCommand::kLeave ||
      pMsg->dw_cmd_ == CFWL_MessageMouse::MouseCommand::kHover ||
      pMsg->dw_cmd_ == CFWL_MessageMouse::MouseCommand::kEnter) {
    return !!pMsg->GetDstTarget();
  }
  if (pMsg->GetDstTarget() != pMessageForm)
    pMsg->os_ = pMsg->GetDstTarget()->TransformTo(pMessageForm, pMsg->os_);
...
bool CFWL_NoteDriver::DoMouseEx(CFWL_Message* pMessage,
  ...
  CFWL_MessageMouse* pMsg = static_cast<CFWL_MessageMouse*>(pMessage);
  if (!pTarget)
    pTarget = pWidgetMgr->GetWidgetAtPoint(pMessageForm, pMsg->os_);
  if (!pTarget)
    return false;
  if (pTarget && pMessageForm != pTarget)
    pMsg->os_ = pMessageForm->TransformTo(pTarget, pMsg->os_);
...
void CFWL_NoteDriver::MouseSecondary(CFWL_Message* pMessage) {
  ...
  if (hover_) {
    CFWL_MessageMouse msLeave(
        hover_.Get(), CFWL_MessageMouse::MouseCommand::kLeave,
        Mask<XFA_FWL_KeyFlag>(), pTarget->TransformTo(hover_.Get(), pMsg->os_));
