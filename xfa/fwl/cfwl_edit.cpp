...
void CFWL_Edit::PreFinalize() {
  ...
  if (properties_.dw_states_ & FWL_STATE_WGT_Focused) {
    HideCaret(nullptr);
  }
...
CFX_RectF CFWL_Edit::GetWidgetRect() {
  ...
  if (properties_.dw_style_exts_ & FWL_STYLEEXT_EDT_OuterScrollbar) {
...
CFX_RectF CFWL_Edit::GetAutosizedWidgetRect() {
  ...
  if (edit_engine_->GetLength() > 0) {
    CFX_SizeF size = CalcTextSize(
        edit_engine_->GetText(),
        !!(properties_.dw_style_exts_ & FWL_STYLEEXT_EDT_MultiLine));
...
void CFWL_Edit::SetStates(uint32_t dwStates) {
  if ((properties_.dw_states_ & FWL_STATE_WGT_Invisible) ||
      (properties_.dw_states_ & FWL_STATE_WGT_Disabled)) {
    HideCaret(nullptr);
  }
...
void CFWL_Edit::OnCaretChanged() {
  ...
  if ((properties_.dw_states_ & FWL_STATE_WGT_Focused) == 0) {
    return;
  }
  ...
  if (properties_.dw_style_exts_ & FWL_STYLEEXT_EDT_MultiLine) {
...
void CFWL_Edit::OnTextChanged() {
  if (properties_.dw_style_exts_ & FWL_STYLEEXT_EDT_VAlignMask) {
    UpdateVAlignment();
  }
...
void CFWL_Edit::DrawContentNonComb(CFGAS_GEGraphics* pGraphics,
  ...
  float fOffSetX = engine_rect_.left - fscroll_offset_x_;
  float fOffSetY = engine_rect_.top - fscroll_offset_y_ + fvalign_offset_;
  CFX_Matrix mt(1, 0, 0, 1, fOffSetX, fOffSetY);
  rtClip = mtMatrix.TransformRect(rtClip);
  mt.Concat(mtMatrix);

  bool bShowSel = !!(properties_.dw_states_ & FWL_STATE_WGT_Focused);
...
void CFWL_Edit::UpdateEditParams() {
  ...
  edit_engine_->SetCombText(
      !!(properties_.dw_style_exts_ & FWL_STYLEEXT_EDT_CombText));

  edit_engine_->EnableValidation(
      !!(properties_.dw_style_exts_ & FWL_STYLEEXT_EDT_Validate));
  edit_engine_->EnablePasswordMode(
      !!(properties_.dw_style_exts_ & FWL_STYLEEXT_EDT_Password));

  uint32_t alignment = 0;
  switch (properties_.dw_style_exts_ & FWL_STYLEEXT_EDT_HAlignMask) {
  ...
  switch (properties_.dw_style_exts_ & FWL_STYLEEXT_EDT_HAlignModeMask) {
  ...
  bool auto_hscroll =
      !!(properties_.dw_style_exts_ & FWL_STYLEEXT_EDT_AutoHScroll);
  if (properties_.dw_style_exts_ & FWL_STYLEEXT_EDT_MultiLine) {
    edit_engine_->EnableMultiLine(true);
    edit_engine_->EnableLineWrap(!auto_hscroll);
    edit_engine_->LimitVerticalScroll(
        (properties_.dw_styles_ & FWL_STYLE_WGT_VScroll) == 0 &&
        (properties_.dw_style_exts_ & FWL_STYLEEXT_EDT_AutoVScroll) == 0);
  } else {
  ...
  ffont_size_ = theme->GetFontSize(part);
  ...
  edit_engine_->SetFontSize(ffont_size_);
  edit_engine_->SetLineSpace(theme->GetLineHeight(part));
  edit_engine_->SetTabWidth(ffont_size_);
...
bool CFWL_Edit::UpdateOffset() {
  ...
  if (edit_bounds.Contains(rtCaret)) {
    ...
    if (contents_bounds.right() < edit_bounds.right() &&
        fscroll_offset_x_ > 0) {
      fscroll_offset_x_ += contents_bounds.right() - edit_bounds.right();
      fscroll_offset_x_ = std::max(fscroll_offset_x_, 0.0f);
    }
    if (contents_bounds.bottom() < edit_bounds.bottom() &&
        fscroll_offset_y_ > 0) {
      fscroll_offset_y_ += contents_bounds.bottom() - edit_bounds.bottom();
      fscroll_offset_y_ = std::max(fscroll_offset_y_, 0.0f);
    }
    return false;
  }
  ...
  fscroll_offset_x_ += offsetX;
  fscroll_offset_y_ += offsetY;
  if (ffont_size_ > engine_rect_.height) {
    fscroll_offset_y_ = 0;
  }
...
void CFWL_Edit::UpdateVAlignment() {
  ...
  float fOffsetY = 0.0f;
  CFX_RectF contents_bounds = edit_engine_->GetContentsBoundingBox();
  if (properties_.dw_style_exts_ & FWL_STYLEEXT_EDT_VCenter) {
  ...
  } else if (properties_.dw_style_exts_ & FWL_STYLEEXT_EDT_VFar) {
    fOffsetY = (engine_rect_.height - contents_bounds.height);
    fOffsetY -= fSpaceBelow;
  } else {
    fOffsetY += fSpaceAbove;
  }
  fvalign_offset_ = std::max(fOffsetY, 0.0f);
}

void CFWL_Edit::UpdateCaret() {
  CFX_RectF rtCaret = caret_rect_;
  rtCaret.Offset(engine_rect_.left - fscroll_offset_x_,
                 engine_rect_.top - fscroll_offset_y_ + fvalign_offset_);
  ...
  if (properties_.dw_states_ & FWL_STATE_WGT_Focused && !rtCaret.IsEmpty()) {
    ShowCaret(&rtCaret);
  } else
    HideCaret(&rtCaret);
}
...
CFWL_ScrollBar* CFWL_Edit::UpdateScroll() {
  ...
  if (rtScroll.height < contents_bounds.height) {
    ...
    float fPos = std::clamp(fscroll_offset_y_, 0.0f, fRange);
...
bool CFWL_Edit::IsShowVertScrollBar() const {
  const bool bShow =
      !(properties_.dw_style_exts_ & FWL_STYLEEXT_EDT_ShowScrollbarFocus) ||
      (properties_.dw_states_ & FWL_STATE_WGT_Focused);
  return bShow && (properties_.dw_styles_ & FWL_STYLE_WGT_VScroll) &&
         (properties_.dw_style_exts_ & FWL_STYLEEXT_EDT_MultiLine) &&
...
void CFWL_Edit::LayoutScrollBar() {
  if (!(properties_.dw_style_exts_ & FWL_STYLEEXT_EDT_ShowScrollbarFocus)) {
    return;
  }
...
CFX_PointF CFWL_Edit::DeviceToEngine(const CFX_PointF& pt) {
  return pt +
         CFX_PointF(fscroll_offset_x_ - engine_rect_.left,
                    fscroll_offset_y_ - engine_rect_.top - fvalign_offset_);
}
...
void CFWL_Edit::InitCaret() {
  ...
  caret_ = cppgc::MakeGarbageCollected<CFWL_Caret>(
      ...
      this);
  caret_->SetStates(properties_.dw_states_);
  UpdateCursorRect();
}
...
void CFWL_Edit::OnProcessMessage(CFWL_Message* pMessage) {
  switch (pMessage->GetType()) {
    ...
    case CFWL_Message::Type::kKey: {
      ...
      if (pKey->dw_cmd_ == CFWL_MessageKey::KeyCommand::kKeyDown) {
        OnKeyDown(pKey);
      } else if (pKey->dw_cmd_ == CFWL_MessageKey::KeyCommand::kChar) {
        OnChar(pKey);
      }
      break;
    }
...
void CFWL_Edit::DoRButtonDown(CFWL_MessageMouse* pMsg) {
  SetCursorPosition(edit_engine_->GetIndexForPoint(DeviceToEngine(pMsg->os_)));
}
...
void CFWL_Edit::OnLButtonDown(CFWL_MessageMouse* pMsg) {
  if (properties_.dw_states_ & FWL_STATE_WGT_Disabled) {
    return;
  }
  ...
  size_t index_at_click =
      edit_engine_->GetIndexForPoint(DeviceToEngine(pMsg->os_));

  if (index_at_click != cursor_position_ &&
      !!(pMsg->dw_flags_ & XFA_FWL_KeyFlag::kShift)) {
...
void CFWL_Edit::OnButtonDoubleClick(CFWL_MessageMouse* pMsg) {
  size_t click_idx = edit_engine_->GetIndexForPoint(DeviceToEngine(pMsg->os_));
...
void CFWL_Edit::OnMouseMove(CFWL_MessageMouse* pMsg) {
  bool shift = !!(pMsg->dw_flags_ & XFA_FWL_KeyFlag::kShift);
...
void CFWL_Edit::OnKeyDown(CFWL_MessageKey* pMsg) {
  bool bShift = !!(pMsg->dw_flags_ & XFA_FWL_KeyFlag::kShift);
  bool bCtrl = !!(pMsg->dw_flags_ & XFA_FWL_KeyFlag::kCtrl);
  ...
  switch (pMsg->dw_key_code_or_char_) {
    ...
    case XFA_FWL_VKEY_Delete: {
      if ((properties_.dw_style_exts_ & FWL_STYLEEXT_EDT_ReadOnly) ||
          (properties_.dw_states_ & FWL_STATE_WGT_Disabled)) {
        break;
      }
...
void CFWL_Edit::OnChar(CFWL_MessageKey* pMsg) {
  ...
  wchar_t c = static_cast<wchar_t>(pMsg->dw_key_code_or_char_);
  switch (c) {
    case L'\b':
    ...
    case L'\t':
      ...
      break;
    case L'\r':
      if (properties_.dw_style_exts_ & FWL_STYLEEXT_EDT_WantReturn) {
        edit_engine_->Insert(cursor_position_, L"\n");
        SetCursorPosition(cursor_position_ + 1);
      }
      break;
    default: {
      if (pMsg->dw_flags_ & kEditingModifier) {
        break;
      }
