...
void CFWL_ComboList::OnProcessMessage(CFWL_Message* pMessage) {
  ...
  if (type == CFWL_Message::Type::kSetFocus ||
    ...
    if (IsShowVertScrollBar() && vertSB) {
      ...
      if (rect.Contains(pMsg->os_)) {
        pMsg->os_ -= rect.TopLeft();
...
void CFWL_ComboList::OnDropListMouseMove(CFWL_MessageMouse* pMsg) {
  if (GetRTClient().Contains(pMsg->os_)) {
    if (notify_owner_) {
      notify_owner_ = false;
    }
    ...
    CFWL_ListBox::Item* hItem = GetItemAtPoint(pMsg->os_);
    if (!hItem)
      return;

    ChangeSelected(GetItemIndex(this, hItem));
  } else if (notify_owner_) {
    pMsg->os_ = ClientToOuter(pMsg->os_);
...
bool CFWL_ComboList::OnDropListKey(CFWL_MessageKey* pKey) {
  ...
  if (pKey->dw_cmd_ == CFWL_MessageKey::KeyCommand::kKeyDown) {
    uint32_t dwKeyCode = pKey->dw_key_code_or_char_;
    switch (dwKeyCode) {
...
void CFWL_ComboList::OnDropListKeyDown(CFWL_MessageKey* pKey) {
  auto dwKeyCode = static_cast<XFA_FWL_VKEYCODE>(pKey->dw_key_code_or_char_);
  switch (dwKeyCode) {
    case XFA_FWL_VKEY_Up:
    case XFA_FWL_VKEY_Down:
