...
void CFWL_DateTimePicker::Update() {
  ...
  fbtn_ = GetThemeProvider()->GetScrollBarWidth();
...
WideString CFWL_DateTimePicker::FormatDateString(int32_t iYear,
                                                 int32_t iMonth,
                                                 int32_t iDay) {
  if (properties_.dw_style_exts_ & FWL_STYLEEXT_DTP_ShortDateFormat) {
...
void CFWL_DateTimePicker::ResetEditAlignment() {
  ...
  uint32_t dwAdd = 0;
  switch (properties_.dw_style_exts_ & FWL_STYLEEXT_DTP_EditHAlignMask) {
  ...
  switch (properties_.dw_style_exts_ & FWL_STYLEEXT_DTP_EditVAlignMask) {
  ...
  if (properties_.dw_style_exts_ & FWL_STYLEEXT_DTP_EditJustified) {
...
bool CFWL_DateTimePicker::NeedsToShowButton() const {
  return properties_.dw_states_ & FWL_STATE_WGT_Focused ||
...
void CFWL_DateTimePicker::OnFocusGained(CFWL_Message* pMsg) {
  ...
  if (edit_ && !(edit_->GetStyleExts() & FWL_STYLEEXT_EDT_ReadOnly)) {
    btn_rect_ =
        CFX_RectF(widget_rect_.width, 0, fbtn_, widget_rect_.height - 1);
  }
