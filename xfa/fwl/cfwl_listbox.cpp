...
void CFWL_ListBox::Update() {
  ...
  switch (properties_.dw_style_exts_ & FWL_STYLEEXT_LTB_AlignMask) {
  ...
  ttostyles_.single_line_ = true;
  fscorll_bar_width_ = GetScrollWidth();
  CalcSize();
}
...
void CFWL_ListBox::DrawWidget(CFGAS_GEGraphics* pGraphics,
  ...
  if (IsShowHorzScrollBar())
    rtClip.height -= fscorll_bar_width_;
  if (IsShowVertScrollBar())
    rtClip.width -= fscorll_bar_width_;

  pGraphics->SetClipRect(matrix.TransformRect(rtClip));
  if ((properties_.dw_styles_ & FWL_STYLE_WGT_NoBackground) == 0) {
...
bool CFWL_ListBox::IsMultiSelection() const {
  return properties_.dw_style_exts_ & FWL_STYLEEXT_LTB_MultiSelection;
}
...
void CFWL_ListBox::DrawBkground(CFGAS_GEGraphics* pGraphics,
  ...
  if (IsShowHorzScrollBar() && IsShowVertScrollBar())
    param.rt_data_ = &static_rect_;
  if (!IsEnabled())
    param.dw_states_ = CFWL_PartState::kDisabled;
...
void CFWL_ListBox::DrawItems(CFGAS_GEGraphics* pGraphics,
  ...
  if (horz_scroll_bar_) {
    rtView.height -= fscorll_bar_width_;
  }
  if (vert_scroll_bar_) {
    rtView.width -= fscorll_bar_width_;
  }
...
void CFWL_ListBox::DrawItem(CFGAS_GEGraphics* pGraphics,
  ...
  if ((properties_.dw_states_ & FWL_STATE_WGT_Focused) && pItem &&
      pItem->IsFocused()) {
  ...
  CFWL_ThemeBackground bg_param(CFWL_ThemePart::Part::kListItem, this,
                                pGraphics);
  bg_param.dw_states_ = dwPartStates;
  bg_param.matrix_ = mtMatrix;
  bg_param.part_rect_ = rtItem;
  bg_param.maximize_ = true;
  bg_param.rt_data_ = &rtFocus;
  if (vert_scroll_bar_ && !horz_scroll_bar_ &&
      (dwPartStates & CFWL_PartState::kFocused)) {
    bg_param.part_rect_.left += 1;
    bg_param.part_rect_.width -= (fscorll_bar_width_ + 1);
    rtFocus.Deflate(0.5, 0.5, 1 + fscorll_bar_width_, 1);
  }
  ...
  textParam.dw_states_ = dwPartStates;
  textParam.matrix_ = mtMatrix;
  textParam.part_rect_ = rtText;
  textParam.ws_text_ = std::move(wsText);
  textParam.dw_ttostyles_ = ttostyles_;
  textParam.ttoalign_ = ttoaligns_;
...
CFX_SizeF CFWL_ListBox::CalcSize() {
  ...
  fitem_height_ = CalcItemHeight();

  int32_t iCount = CountItems(this);
  CFX_SizeF fs;
  for (int32_t i = 0; i < iCount; i++) {
    Item* htem = GetItem(this, i);
    UpdateItemSize(htem, fs, fWidth, fitem_height_);
  }

  float iHeight = client_rect_.height;
  bool bShowVertScr = false;
  bool bShowHorzScr = false;
  if (!bShowVertScr && (properties_.dw_styles_ & FWL_STYLE_WGT_VScroll)) {
  ...
  if (bShowVertScr) {
    ...
    CFX_RectF rtScrollBar(client_rect_.right() - fscorll_bar_width_,
                          client_rect_.top, fscorll_bar_width_,
                          client_rect_.height - 1);
    if (bShowHorzScr)
      rtScrollBar.height -= fscorll_bar_width_;

    vert_scroll_bar_->SetWidgetRect(rtScrollBar);
    fMax = std::max(fs.height - content_rect_.height, fitem_height_);

    vert_scroll_bar_->SetRange(0.0f, fMax);
    vert_scroll_bar_->SetPageSize(rtScrollBar.height * 9 / 10);
    vert_scroll_bar_->SetStepSize(fitem_height_);

    float fPos = std::clamp(vert_scroll_bar_->GetPos(), 0.0f, fMax);
    vert_scroll_bar_->SetPos(fPos);
    vert_scroll_bar_->SetTrackPos(fPos);
    if ((properties_.dw_style_exts_ & FWL_STYLEEXT_LTB_ShowScrollBarFocus) ==
            0 ||
  ...
  if (bShowHorzScr) {
    ...
    CFX_RectF rtScrollBar(client_rect_.left,
                          client_rect_.bottom() - fscorll_bar_width_,
                          client_rect_.width, fscorll_bar_width_);
    if (bShowVertScr)
      rtScrollBar.width -= fscorll_bar_width_;
  ...
  if (bShowVertScr && bShowHorzScr) {
    static_rect_ = CFX_RectF(client_rect_.right() - fscorll_bar_width_,
                             client_rect_.bottom() - fscorll_bar_width_,
                             fscorll_bar_width_, fscorll_bar_width_);
  }
  return fs;
}
...
bool CFWL_ListBox::ScrollBarPropertiesPresent() const {
  return !(properties_.dw_style_exts_ & FWL_STYLEEXT_LTB_ShowScrollBarFocus) ||
         (properties_.dw_states_ & FWL_STATE_WGT_Focused);
}
...
void CFWL_ListBox::OnProcessMessage(CFWL_Message* pMessage) {
  ...
  switch (pMessage->GetType()) {
    ...
    case CFWL_Message::Type::kKey: {
      ...
      if (pMsg->dw_cmd_ == CFWL_MessageKey::KeyCommand::kKeyDown) {
        OnKeyDown(pMsg);
      }
      break;
    }
...
void CFWL_ListBox::OnLButtonDown(CFWL_MessageMouse* pMsg) {
  ...
  Item* pItem = GetItemAtPoint(pMsg->os_);
  if (!pItem)
    return;

  if (IsMultiSelection()) {
    if (pMsg->dw_flags_ & XFA_FWL_KeyFlag::kCtrl) {
      pItem->SetSelected(!pItem->IsSelected());
      hanchor_ = pItem;
    } else if (pMsg->dw_flags_ & XFA_FWL_KeyFlag::kShift) {
      if (hanchor_) {
        SetSelection(hanchor_, pItem, true);
      } else
...
void CFWL_ListBox::OnKeyDown(CFWL_MessageKey* pMsg) {
  auto dwKeyCode = static_cast<XFA_FWL_VKEYCODE>(pMsg->dw_key_code_or_char_);
  switch (dwKeyCode) {
...
CFWL_ListBox::Item::Item(const WideString& text) : ws_text_(text) {}
