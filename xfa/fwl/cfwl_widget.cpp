...
void CFWL_Widget::ModifyStyles(uint32_t dwStylesAdded,
  ...
  properties_.dw_styles_ &= ~dwStylesRemoved;
  properties_.dw_styles_ |= dwStylesAdded;
}

void CFWL_Widget::ModifyStyleExts(uint32_t dwStyleExtsAdded,
                                  uint32_t dwStyleExtsRemoved) {
  properties_.dw_style_exts_ &= ~dwStyleExtsRemoved;
  properties_.dw_style_exts_ |= dwStyleExtsAdded;
}
...
void CFWL_Widget::SetStates(uint32_t dwStates) {
  properties_.dw_states_ |= dwStates;
  if (IsVisible())
    return;
...
bool CFWL_Widget::IsEnabled() const {
  return (properties_.dw_states_ & FWL_STATE_WGT_Disabled) == 0;
}

bool CFWL_Widget::HasBorder() const {
  return !!(properties_.dw_styles_ & FWL_STYLE_WGT_Border);
}

bool CFWL_Widget::IsVisible() const {
  return !(properties_.dw_states_ & FWL_STATE_WGT_Invisible);
}

bool CFWL_Widget::IsOverLapper() const {
  return (properties_.dw_styles_ & FWL_STYLE_WGT_WindowTypeMask) ==
         FWL_STYLE_WGT_OverLapper;
}

bool CFWL_Widget::IsPopup() const {
  return !!(properties_.dw_styles_ & FWL_STYLE_WGT_Popup);
}

bool CFWL_Widget::IsChild() const {
  return !!(properties_.dw_styles_ & FWL_STYLE_WGT_Child);
}
...
CFX_SizeF CFWL_Widget::CalcTextSize(const WideString& wsText, bool bMultiLine) {
  ...
  calPart.ws_text_ = wsText;
  if (bMultiLine)
    calPart.dw_ttostyles_.line_wrap_ = true;
  else
    calPart.dw_ttostyles_.single_line_ = true;

  calPart.ttoalign_ = FDE_TextAlignment::kTopLeft;
...
void CFWL_Widget::CalcTextRect(const WideString& wsText,
  ...
  CFWL_ThemeText calPart(CFWL_ThemePart::Part::kNone, this, nullptr);
  calPart.ws_text_ = wsText;
  calPart.dw_ttostyles_ = dwTTOStyles;
  calPart.ttoalign_ = iTTOAlign;
...
void CFWL_Widget::OnProcessMessage(CFWL_Message* pMessage) {
  ...
  switch (pMessage->GetType()) {
    case CFWL_Message::Type::kMouse: {
      ...
      CFWL_EventMouse evt(pWidget, pWidget, pMsgMouse->dw_cmd_);
