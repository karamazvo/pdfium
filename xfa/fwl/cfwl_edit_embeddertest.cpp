...
TEST_F(CFWLEditEmbedderTest, FillWithNewLineWithMultiline) {
  ...
  // fghij|
  {
    constexpr char kFilledMultilineBasename[] = "filled_multiline";
    ScopedFPDFBitmap page_bitmap =
        RenderLoadedPageWithFlags(page.get(), FPDF_ANNOT);
  ...
  // abcde|ghij
  {
    const char* kMultilineBackspaceBasename = "multiline_backspace";
    if (CFX_DefaultRenderDevice::UseSkiaRenderer()) {
#if BUILDFLAG(IS_APPLE) && !defined(ARCH_CPU_ARM64)
      kMultilineBackspaceBasename = "multiline_backspace_x86";
#endif
    }
    ScopedFPDFBitmap page_bitmap =
        RenderLoadedPageWithFlags(page.get(), FPDF_ANNOT);
