...
TEST_F(CFWLEditEmbedderTest, FillWithNewLineWithMultiline) {
  ...
  // fghij|
  {
// TODO: Replace these checksums once we get the images.
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)
    const char kFilledMultilineMD5[] = "Need image from gerrit";
#else
    const char kFilledMultilineMD5[] = "a5654e027d8b1667c20f3b86d1918003";
#endif  // BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)
    ScopedFPDFBitmap page_bitmap =
        RenderLoadedPageWithFlags(page.get(), FPDF_ANNOT);
    // Will fail by default but useful to get the base64 images.
    CompareBitmapToPng(page_bitmap.get(), kFilledMultilineMD5);
  }
...
// TODO: Replace these checksums once we get the images.
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)
    const char kMultilineBackspaceMD5[] = "Need image from gerrit";
#else
    ...
    ScopedFPDFBitmap page_bitmap =
        RenderLoadedPageWithFlags(page.get(), FPDF_ANNOT);
    // Will fail by default but useful to get the base64 images.
    CompareBitmapToPng(page_bitmap.get(), kMultilineBackspaceMD5);
  }
}
...
TEST_F(CFWLEditEmbedderTest, ImageEditTest) {
  ...
  ScopedPage page = LoadScopedPage(0);
  FORM_OnLButtonDown(form_handle(), page.get(), 0, 115, 58);
  const char* kFilledBasename = "xfa_image_edit";
  ScopedFPDFBitmap page_bitmap =
      RenderLoadedPageWithFlags(page.get(), FPDF_ANNOT);
