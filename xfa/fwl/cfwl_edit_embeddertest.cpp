...
TEST_F(CFWLEditEmbedderTest, FillWithNewLineWithMultiline) {
  ...
  // fghij|
  {
    const char* kFilledMultilineBasename = "xfa_multiline_textfield_filled";
    if (CFX_DefaultRenderDevice::UseSkiaRenderer()) {
#if BUILDFLAG(IS_APPLE) && !defined(ARCH_CPU_ARM64)
      kFilledMultilineBasename = "xfa_multiline_textfield_filled_x86";
#endif
    }
    ScopedFPDFBitmap page_bitmap =
        RenderLoadedPageWithFlags(page.get(), FPDF_ANNOT);
  ...
  // abcde|ghij
  {
    const char* kMultilineBackspaceBasename =
        "xfa_multiline_textfield_backspace";
    if (CFX_DefaultRenderDevice::UseSkiaRenderer()) {
#if BUILDFLAG(IS_APPLE) && !defined(ARCH_CPU_ARM64)
      kMultilineBackspaceBasename = "xfa_multiline_textfield_backspace_x86";
#endif
    }
    ScopedFPDFBitmap page_bitmap =
        RenderLoadedPageWithFlags(page.get(), FPDF_ANNOT);
