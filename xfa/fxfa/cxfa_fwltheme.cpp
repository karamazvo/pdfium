...
void CXFA_FWLTheme::DrawText(const CFWL_ThemeText& pParams) {
  ...
  if (pParams.GetWidget()->GetClassID() == FWL_Type::MonthCalendar) {
    ...
    text_out_->SetStyles(pParams.dw_ttostyles_);
    text_out_->SetAlignment(pParams.ttoalign_);
    ...
    if ((pParams.GetPart() == CFWL_ThemePart::Part::kDatesIn) &&
        !(pParams.dw_states_ & CFWL_PartState::kFlagged) &&
        (pParams.dw_states_ &
    ...
    text_out_->DrawLogicText(pRenderDevice, pParams.ws_text_,
                             pParams.part_rect_);
    return;
  }
  CXFA_FFWidget* pWidget = GetOutmostFFWidget(pParams.GetWidget());
  ...
  text_out_->SetMatrix(mtPart);
  text_out_->DrawLogicText(pRenderDevice, pParams.ws_text_, pParams.part_rect_);
}
...
void CXFA_FWLTheme::CalcTextRect(const CFWL_ThemeText& pParams,
  ...
  if (pParams.GetWidget()->GetClassID() == FWL_Type::MonthCalendar) {
    ...
    text_out_->SetAlignment(pParams.ttoalign_);
    text_out_->SetStyles(pParams.dw_ttostyles_);
    text_out_->CalcLogicSize(pParams.ws_text_.AsStringView(), pRect);
    return;
  }
