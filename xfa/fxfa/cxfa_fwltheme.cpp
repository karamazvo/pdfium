...
void CXFA_FWLTheme::DrawText(const CFWL_ThemeText& pParams) {
  ...
  if (pParams.GetWidget()->GetClassID() == FWL_Type::MonthCalendar) {
    ...
    text_out_->SetStyles(pParams.m_dwTTOStyles);
    text_out_->SetAlignment(pParams.m_iTTOAlign);
    ...
    text_out_->DrawLogicText(pRenderDevice, pParams.text_, pParams.m_PartRect);
    return;
  }
  CXFA_FFWidget* pWidget = GetOutmostFFWidget(pParams.GetWidget());
  if (!pWidget)
