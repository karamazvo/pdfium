...
struct XFA_FMSOMMethod {
  const wchar_t* ws_som_method_name_;  // Ok, POD struct.
  uint32_t dparameters_;
};
...
CXFA_FMNumberExpression::CXFA_FMNumberExpression(WideString wsNumber)
    : CXFA_FMSimpleExpression(TOKnumber), ws_number_(std::move(wsNumber)) {}
...
CXFA_FMStringExpression::CXFA_FMStringExpression(WideString wsString)
    : CXFA_FMSimpleExpression(TOKstring), ws_string_(std::move(wsString)) {}
...
CXFA_FMIdentifierExpression::CXFA_FMIdentifierExpression(
    ...
    : CXFA_FMSimpleExpression(TOKidentifier),
      ws_identifier_(std::move(wsIdentifier)) {}
...
bool CXFA_FMIdentifierExpression::ToJavaScript(WideTextBuffer* js,
  ...
  if (ws_identifier_.EqualsASCII("$")) {
    *js << "this";
  } else if (ws_identifier_.EqualsASCII("!")) {
    *js << "xfa.datasets";
  } else if (ws_identifier_.EqualsASCII("$data")) {
    *js << "xfa.datasets.data";
  } else if (ws_identifier_.EqualsASCII("$event")) {
    *js << "xfa.event";
  } else if (ws_identifier_.EqualsASCII("$form")) {
    *js << "xfa.form";
  } else if (ws_identifier_.EqualsASCII("$host")) {
    *js << "xfa.host";
  } else if (ws_identifier_.EqualsASCII("$layout")) {
    *js << "xfa.layout";
  } else if (ws_identifier_.EqualsASCII("$template")) {
    *js << "xfa.template";
  } else if (ws_identifier_[0] == L'!') {
    *js << "pfm__excl__" << ws_identifier_.Last(ws_identifier_.GetLength() - 1);
  } else
    *js << ws_identifier_;
...
uint32_t CXFA_FMCallExpression::IsMethodWithObjParam(
  ...
  const XFA_FMSOMMethod* result = std::lower_bound(
      ...
      [](const XFA_FMSOMMethod iter, const WideString& val) {
        return val.Compare(iter.ws_som_method_name_) > 0;
      });
  if (result != std::end(kFMSomMethods) &&
      methodName == result->ws_som_method_name_) {
    return result->dparameters_;
  }
  return 0;
}
...
CXFA_FMFunctionDefinition::CXFA_FMFunctionDefinition(
    ...
    : ws_name_(std::move(wsName)),
      arguments_(std::move(arguments)),
      expressions_(std::move(expressions)) {
  DCHECK(!ws_name_.IsEmpty());
}
...
bool CXFA_FMFunctionDefinition::ToJavaScript(WideTextBuffer* js,
  ...
  *js << "function " << IdentifierToName(ws_name_) << "(";
...
CXFA_FMVarExpression::CXFA_FMVarExpression(WideString wsName,
    ...
    : ws_name_(std::move(wsName)), init_(pInit) {}
...
bool CXFA_FMVarExpression::ToJavaScript(WideTextBuffer* js,
  ...
  WideString tempName = IdentifierToName(ws_name_);
  *js << "var " << tempName << " = ";
  if (init_) {
...
CXFA_FMForExpression::CXFA_FMForExpression(WideString wsVariant,
    ...
    : ws_variant_(std::move(wsVariant)),
...
bool CXFA_FMForExpression::ToJavaScript(WideTextBuffer* js,
  ...
  *js << "{\n";
  ...
  WideString tmpName = IdentifierToName(ws_variant_);
  *js << "var " << tmpName << " = null;\n";
...
bool CXFA_FMForeachExpression::ToJavaScript(WideTextBuffer* js,
  ...
  *js << "{\n";
  ...
  WideString tmpName = IdentifierToName(ws_identifier_);
  *js << "var " << tmpName << " = null;\n";
