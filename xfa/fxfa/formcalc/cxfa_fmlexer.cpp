...
CXFA_FMLexer::Token CXFA_FMLexer::NextToken() {
  ...
  while (!IsComplete() && sp_input_[cursor_]) {
    if (!IsFormCalcCharacter(sp_input_[cursor_])) {
      RaiseError();
      return Token();
    }

    switch (sp_input_[cursor_]) {
      case '\n':
        ++cursor_;
        break;
      case '\r':
      ...
      case '.':
        ...
        if (FXSYS_IsDecimalDigit(sp_input_[cursor_])) {
          --cursor_;
          return AdvanceForNumber();
        }
        return Token(TOKdot);
      default:
        if (IsWhitespaceCharacter(sp_input_[cursor_])) {
          ++cursor_;
          break;
        }
        if (!IsInitialIdentifierCharacter(sp_input_[cursor_])) {
          RaiseError();
          return Token();
        }
...
CXFA_FMLexer::Token CXFA_FMLexer::AdvanceForNumber() {
  ...
  if (cursor_ < sp_input_.size()) {
    FXSYS_wcstof(WideStringView(sp_input_.subspan(cursor_)), &used_length);
  }
  size_t end = cursor_ + used_length;
  if (used_length == 0 ||
      (end < sp_input_.size() && FXSYS_iswalpha(sp_input_[end]))) {
    RaiseError();
    return Token();
  }
  WideStringView str(sp_input_.subspan(cursor_, end - cursor_));
  cursor_ = end;
    ...
    if (sp_input_[cursor_] == '"') {
      ...
      if (cursor_ >= sp_input_.size()) {
        return Token(TOKstring,
                     WideStringView(sp_input_.subspan(start, cursor_ - start)));
      }
...
CXFA_FMLexer::Token CXFA_FMLexer::AdvanceForIdentifier() {
  ...
  while (!IsComplete() && sp_input_[cursor_]) {
    ...
    if (!IsIdentifierCharacter(sp_input_[cursor_])) {
      break;
    }
  ...
  WideStringView str(sp_input_.subspan(start, cursor_ - start));
...
void CXFA_FMLexer::AdvanceForComment() {
  ...
  while (!IsComplete() && sp_input_[cursor_]) {
    ...
    if (sp_input_[cursor_] == L'\r') {
      ++cursor_;
      return;
    }
    if (sp_input_[cursor_] == L'\n') {
      ++cursor_;
      return;
    }
    ++cursor_;
