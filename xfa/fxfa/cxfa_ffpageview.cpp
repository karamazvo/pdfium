...
CXFA_FFPageWidgetIterator::CXFA_FFPageWidgetIterator(
    ...
    Mask<XFA_WidgetStatus> dwFilter)
    : siterator_(pPageView->GetLayoutItem()),
      dw_filter_(dwFilter),
...
CXFA_FFWidget* CXFA_FFPageWidgetIterator::MoveToFirst() {
  siterator_.Reset();
  for (CXFA_LayoutItem* pLayoutItem = siterator_.GetCurrent(); pLayoutItem;
       pLayoutItem = siterator_.MoveToNext()) {
    CXFA_FFWidget* hWidget = FilteredLoadedWidgetFromLayoutItem(
        pLayoutItem, dw_filter_, ignore_relevant_);
    if (hWidget)
      return hWidget;
  }
  return nullptr;
...
CXFA_FFWidget* CXFA_FFPageWidgetIterator::MoveToLast() {
  siterator_.SetCurrent(nullptr);
  return MoveToPrevious();
}

CXFA_FFWidget* CXFA_FFPageWidgetIterator::MoveToNext() {
  for (CXFA_LayoutItem* pLayoutItem = siterator_.MoveToNext(); pLayoutItem;
       pLayoutItem = siterator_.MoveToNext()) {
    CXFA_FFWidget* hWidget = FilteredLoadedWidgetFromLayoutItem(
        pLayoutItem, dw_filter_, ignore_relevant_);
    if (hWidget)
...
CXFA_FFWidget* CXFA_FFPageWidgetIterator::MoveToPrevious() {
  for (CXFA_LayoutItem* pLayoutItem = siterator_.MoveToPrev(); pLayoutItem;
       pLayoutItem = siterator_.MoveToPrev()) {
    CXFA_FFWidget* hWidget = FilteredLoadedWidgetFromLayoutItem(
        pLayoutItem, dw_filter_, ignore_relevant_);
    if (hWidget)
      return hWidget;
...
CXFA_FFWidget* CXFA_FFPageWidgetIterator::GetCurrentWidget() {
  CXFA_LayoutItem* pLayoutItem = siterator_.GetCurrent();
...
bool CXFA_FFPageWidgetIterator::SetCurrentWidget(CXFA_FFWidget* pWidget) {
  return pWidget && siterator_.SetCurrent(pWidget->GetLayoutItem());
}
...
CXFA_FFWidget* CXFA_FFTabOrderPageWidgetIterator::MoveToFirst() {
  for (int32_t i = 0;
    ...
    if (PageWidgetFilter(tab_order_widget_array_[i]->GetFFWidget(), dw_filter_,
