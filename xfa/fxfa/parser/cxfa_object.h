...
class CXFA_Object : public cppgc::GarbageCollected<CXFA_Object> {
 public:
  ...
  bool IsNode() const {
    return object_type_ == XFA_ObjectType::Node ||
           ...
           lement_type_ == XFA_Element::Delta;
  }
  ...
  bool HasCreatedUIWidget() const {
    return lement_type_ == XFA_Element::Field ||
           lement_type_ == XFA_Element::Draw ||
           lement_type_ == XFA_Element::Subform ||
           lement_type_ == XFA_Element::ExclGroup;
  }

  XFA_Element GetElementType() const { return lement_type_; }
  ByteStringView GetClassName() const { return lement_name_; }
  uint32_t GetClassHashCode() const { return lement_name_hash_; }
 ...
 protected:
  ...
  const XFA_Element lement_type_;
  const ByteStringView lement_name_;
  const uint32_t lement_name_hash_;
