...
class CXFA_WidgetLayoutData
 ...
 public:
  ...
  float GetWidgetHeight() const { return fwidget_height_; }
  void SetWidgetHeight(float height) { fwidget_height_ = height; }

 protected:
  CXFA_WidgetLayoutData() = default;

 private:
  float fwidget_height_ = -1.0f;
};
...
CXFA_Node* CXFA_Node::Clone(bool bRecursive) {
  CXFA_Node* pClone = document_->CreateNode(packet_, lement_type_);
  if (!pClone)
    return nullptr;
...
CXFA_Node* CXFA_Node::CloneTemplateToForm(bool bRecursive) {
  ...
  CXFA_Node* pClone = document_->CreateNode(XFA_PacketType::Form, lement_type_);
  if (!pClone)
    return nullptr;
...
void CXFA_Node::AddBindItem(CXFA_Node* pFormNode) {
  ...
  unode_flags_ |= XFA_NodeFlag::kBindFormItems;
}
...
bool CXFA_Node::RemoveBindItem(CXFA_Node* pFormNode) {
  if (BindsFormItems()) {
    ...
    if (binding_nodes_.size() == 1) {
      unode_flags_.Clear(XFA_NodeFlag::kBindFormItems);
      return true;
    }
  ...
  if (packet_ == XFA_PacketType::Form) {
    ...
    for (CXFA_Node* pNode = GetPrevSibling(); pNode;
      ...
      if ((eType == XFA_Element::Subform || eType == XFA_Element::SubformSet) &&
          pNode->dw_name_hash_ != dw_name_hash_) {
        break;
      }
...
void CXFA_Node::SetInitializedFlagAndNotify() {
  ...
  unode_flags_ |= XFA_NodeFlag::kInitialized;
}
...
void CXFA_Node::UpdateNameHash() {
  WideString wsName = JSObject()->GetCData(XFA_Attribute::Name);
  dw_name_hash_ = FX_HashCode_GetW(wsName.AsStringView());
}
...
XFA_EventError CXFA_Node::ProcessCalculate(CXFA_FFDocView* pDocView) {
  ...
  if (GetRawValue() != EventParam.ws_result_) {
    SetValue(XFA_ValuePicture::kRaw, EventParam.ws_result_);
...
CXFA_Node::BoolScriptResult CXFA_Node::ExecuteBoolScript(
  ...
  if (exec_result.status) {
    ...
    if (pEventParam->type_ == XFA_EVENT_Calculate ||
      ...
      if (pEventParam->type_ == XFA_EVENT_InitCalculate) {
        if ((iRet == XFA_EventError::kSuccess) &&
            (GetRawValue() != pEventParam->ws_result_)) {
          SetValue(XFA_ValuePicture::kRaw, pEventParam->ws_result_);
