...
class CXFA_ContentLayoutItem final : public CXFA_LayoutItem {
 public:
  ...
  void SetStatusBits(Mask<XFA_WidgetStatus> val) { dw_status_ |= val; }
  void ClearStatusBits(Mask<XFA_WidgetStatus> val) { dw_status_ &= ~val; }

  // TRUE if all (not any) bits set in |val| are set in |dw_status_|.
  bool TestStatusBits(Mask<XFA_WidgetStatus> val) const {
    return dw_status_.TestAll(val);
  }
 ...
 private:
  ...
  mutable Mask<XFA_WidgetStatus> dw_status_;
