...
WideString CXFA_TextLayout::GetLinkURLAtPoint(const CFX_PointF& point) {
  for (const auto& pPieceLine : iece_lines_) {
...
CFX_SizeF CXFA_TextLayout::CalcSize(const CFX_SizeF& minSize,
  ...
  return CFX_SizeF(fmax_width_, fLinePos);
}
...
bool CXFA_TextLayout::DrawString(CFX_RenderDevice* pFxDevice,
  ...
  size_t szPieceLines = iece_lines_.size();
  if (!blocks_.empty()) {
    if (szBlockIndex < blocks_.size()) {
  ...
  for (size_t i = 0; i < szPieceLines; ++i) {
    if (i + szLineStart >= iece_lines_.size()) {
      break;
    }

    PieceLine* pPieceLine = iece_lines_[i + szLineStart].get();
...
void CXFA_TextLayout::DoTabstops(CFX_CSSComputedStyle* pStyle,
  ...
  if (iCount > 0) {
    ...
    if (tabstop_context_->has_tabstops_) {
      ...
      tabstop_context_->fleft_ = std::min(fLeft, tabstop_context_->ftab_width_);
      tabstop_context_->has_tabstops_ = false;
      tabstop_context_->ftab_width_ = 0;
    }
    pPiece->rtPiece.left -= tabstop_context_->fleft_;
  }
}
...
void CXFA_TextLayout::AppendTextLine(CFGAS_Char::BreakType dwStatus,
  ...
  if (bSavePieces) {
    ...
    iece_lines_.push_back(std::move(pNew));
    if (tabstop_context_) {
      tabstop_context_->Reset();
    }
    ...
    *pLinePos += fLineStep;
    fmax_width_ = std::max(fmax_width_, fLineWidth);
