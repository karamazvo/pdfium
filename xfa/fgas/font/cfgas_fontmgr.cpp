...
CFX_Face::FTStreamRecAndFace LoadFace(
    const RetainPtr<IFX_SeekableReadStream>& font_stream,
    int32_t iFaceIndex) {
  CFX_FontMgr* font_mgr = CFX_GEModule::Get()->GetFontMgr();
  FXFT_LibraryRec* library = font_mgr->GetFTLibrary();
  CFX_Face::FTStreamRecAndFace StreamRecAndFace =
      CFX_Face::OpenFromStream(library, font_stream, iFaceIndex);
  return StreamRecAndFace;
}
...
bool VerifyUnicodeForFontDescriptor(CFGAS_FontDescriptor* pDesc,
  ...
  if (!pDesc->face_) {
    ...
    CFX_Face::FTStreamRecAndFace ft_stream_and_face =
