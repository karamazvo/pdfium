...
struct FTStreamAndFace {
  std::unique_ptr<FXFT_StreamRec> ft_stream;  // Must outlive `face`.
  RetainPtr<CFX_Face> face;
};

FTStreamAndFace LoadFace(const RetainPtr<IFX_SeekableReadStream>& font_stream,
                         int32_t iFaceIndex) {
  if (!font_stream) {
...
bool VerifyUnicodeForFontDescriptor(CFGAS_FontDescriptor* pDesc,
  ...
  if (!pDesc->face_) {
    ...
    FTStreamAndFace ft_stream_and_face =
...
void CFGAS_FontMgr::RegisterFaces(
  ...
  int32_t num_faces = 0;
  do {
    FTStreamAndFace ft_stream_and_face = LoadFace(font_stream, index++);
