...
bool CFGAS_FontDescriptor::VerifyUnicode(wchar_t unicode) {
  if (!face_) {
    ...
    RetainPtr<CFX_Face> ft_face = CFX_Face::New(
        CFX_GEModule::Get()->GetFontMgr(), nullptr, file_read, face_index_);
    if (!ft_face) {
      return false;
    }
...
void CFGAS_FontMgr::RegisterFaces(
  ...
  int num_faces = 0;
  do {
    RetainPtr<CFX_Face> face = CFX_Face::New(CFX_GEModule::Get()->GetFontMgr(),
                                             nullptr, font_stream, index);
    if (!face) {
      ++index;
      continue;
    }
