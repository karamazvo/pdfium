...
bool VerifyUnicodeForFontDescriptor(CFGAS_FontDescriptor* pDesc,
  ...
  if (!pDesc->face_) {
    RetainPtr<IFX_SeekableReadStream> pFileRead =
        CreateFontStream(pDesc->ws_face_name_.ToUTF8());
    if (!pFileRead) {
      return false;
    }
...
int32_t CalcPenalty(CFGAS_FontDescriptor* pInstalled,
  ...
  if (FontName.GetLength() != 0) {
    if (FontName != pInstalled->ws_face_name_) {
      size_t i;
      for (i = 0; i < pInstalled->ws_family_names_.size(); ++i) {
        if (pInstalled->ws_family_names_[i] == FontName) {
          break;
        }
      }
      if (i == pInstalled->ws_family_names_.size()) {
        nPenalty += 0xFFFF;
      } else
        nPenalty -= 28000;
    } else {
    ...
    if (nPenalty == 30000 && !IsPartName(pInstalled->ws_face_name_, FontName)) {
      size_t i;
      for (i = 0; i < pInstalled->ws_family_names_.size(); i++) {
        if (IsPartName(pInstalled->ws_family_names_[i], FontName)) {
          break;
        }
      }
      if (i == pInstalled->ws_family_names_.size()) {
  ...
  uint32_t dwStyleMask = pInstalled->dw_font_styles_ ^ dwFontStyles;
  ...
  if (wBit != static_cast<uint16_t>(-1)) {
    DCHECK(wBit < 64);
    if ((pInstalled->dw_csb_[wBit / 32] & (1 << (wBit % 32))) == 0) {
      nPenalty += 0xFFFF;
    } else
      nPenalty -= 60000;
  }
  ...
  if (wBit != FGAS_FONTUSB::kNoBitField) {
    DCHECK(wBit < 128);
    if ((pInstalled->dw_usb_[wBit / 32] & (1 << (wBit % 32))) == 0) {
      nPenalty += 0xFFFF;
    } else
      nPenalty -= 60000;
  }
...
RetainPtr<CFGAS_GEFont> CFGAS_FontMgr::GetFontByUnicodeImpl(
  ...
  for (const auto& info : hash_2candidate_list_[dwHash]) {
    ...
    if (!VerifyUnicodeForFontDescriptor(pDesc, wUnicode))
      continue;
    RetainPtr<CFGAS_GEFont> pFont =
        LoadFontInternal(pDesc->ws_face_name_, pDesc->face_index_);
    if (!pFont)
      continue;
    pFont->SetLogicalFontStyle(dwFontStyles);
    hash_2fonts_[dwHash].push_back(pFont);
...
void CFGAS_FontMgr::RegisterFace(RetainPtr<CFX_Face> pFace,
  ...
  pFont->dw_font_styles_ |= GetFlags(pFace);
  ...
  if (unicode_range.has_value()) {
    fxcrt::Copy(unicode_range.value(), pFont->dw_usb_);
  } else {
    fxcrt::Fill(pFont->dw_usb_, 0);
  }

  std::optional<std::array<uint32_t, 2>> code_page_range =
      pFace->GetOs2CodePageRange();
  if (code_page_range.has_value()) {
    fxcrt::Copy(code_page_range.value(), pFont->dw_csb_);
  } else {
    fxcrt::Fill(pFont->dw_csb_, 0);
  }
  ...
  pFont->ws_family_names_ = GetNames(table);
  pFont->ws_family_names_.push_back(
      WideString::FromUTF8(pFace->GetRec()->family_name));
  pFont->ws_face_name_ = wsFaceName;
  pFont->face_index_ =
