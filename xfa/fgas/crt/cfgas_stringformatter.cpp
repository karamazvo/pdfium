...
CFGAS_StringFormatter::CFGAS_StringFormatter(const WideString& wsPattern)
    : ws_pattern_(wsPattern), sp_pattern_(ws_pattern_.span()) {}
...
CFGAS_StringFormatter::Category CFGAS_StringFormatter::GetCategory() const {
  ...
  while (ccf < sp_pattern_.size()) {
    if (sp_pattern_[ccf] == '\'') {
      GetLiteralText(sp_pattern_, &ccf);
      continue;
    }
    if (!bBraceOpen && !pdfium::Contains(kConstChars, sp_pattern_[ccf])) {
      WideString wsCategory(sp_pattern_[ccf]);
      ccf++;
      while (true) {
        if (ccf >= sp_pattern_.size()) {
          return eCategory;
        }
        if (sp_pattern_[ccf] == '.' || sp_pattern_[ccf] == '(') {
          break;
        }
        if (sp_pattern_[ccf] == '{') {
          bBraceOpen = true;
...
WideString CFGAS_StringFormatter::GetTextFormat(
  ...
  while (ccf < sp_pattern_.size()) {
    if (sp_pattern_[ccf] == '\'') {
      ...
      GetLiteralText(sp_pattern_, &ccf);
      wsPurgePattern +=
          WideStringView(sp_pattern_.subspan(iCurChar, ccf - iCurChar));
      continue;
    }
    if (!bBrackOpen && !pdfium::Contains(kConstChars, sp_pattern_[ccf])) {
      WideString wsSearchCategory(sp_pattern_[ccf]);
      ccf++;
      while (ccf < sp_pattern_.size() && sp_pattern_[ccf] != '{' &&
             sp_pattern_[ccf] != '.' && sp_pattern_[ccf] != '(') {
        wsSearchCategory += sp_pattern_[ccf];
        ccf++;
      }
      ...
      wsPurgePattern += sp_pattern_[ccf];
    }
    ccf++;
  }
  if (!bBrackOpen)
    wsPurgePattern = ws_pattern_;
...
LocaleIface* CFGAS_StringFormatter::GetNumericFormat(
  ...
  *dwStyle = 0;
  ...
  while (ccf < sp_pattern_.size()) {
    ...
    if (!bFindDot && ccf < sp_pattern_.size() &&
        (sp_pattern_[ccf] == '.' || sp_pattern_[ccf] == 'V' ||
         sp_pattern_[ccf] == 'v')) {
      bFindDot = true;
...
bool CFGAS_StringFormatter::ParseText(const WideString& wsSrcText,
  ...
  wsValue->clear();
  if (wsSrcText.IsEmpty() || sp_pattern_.empty()) {
    return false;
  }
...
bool CFGAS_StringFormatter::ParseNum(LocaleMgrIface* pLocaleMgr,
                                     ...
                                     WideString* wsValue) const {
  wsValue->clear();
  if (wsSrcNum.IsEmpty() || sp_pattern_.empty()) {
    return false;
  }

  size_t dot_index_f = sp_pattern_.size();
  ...
  *pLocale = nullptr;
  ...
  while (ccf < sp_pattern_.size()) {
    ...
    if (!bBraceOpen && eDateTimeType != DateTimeType::kDateTime &&
        !pdfium::Contains(kConstChars, sp_pattern_[ccf])) {
      WideString wsCategory(sp_pattern_[ccf]);
      ccf++;
      while (ccf < sp_pattern_.size() && sp_pattern_[ccf] != '{' &&
             sp_pattern_[ccf] != '.' && sp_pattern_[ccf] != '(') {
        if (sp_pattern_[ccf] == 'T') {
          *wsDatePattern = ws_pattern_.First(ccf);
          *wsTimePattern = ws_pattern_.Last(ws_pattern_.GetLength() - ccf);
...
bool CFGAS_StringFormatter::ParseDateTime(LocaleMgrIface* pLocaleMgr,
  ...
  dtValue->Reset();
  if (wsSrcDateTime.IsEmpty() || sp_pattern_.empty()) {
    return false;
  }
...
bool CFGAS_StringFormatter::FormatNum(LocaleMgrIface* pLocaleMgr,
                                      ...
                                      WideString* wsOutput) const {
  if (wsInputNum.IsEmpty() || sp_pattern_.empty()) {
    return false;
  }
