  ...
  for (; index < spFormatString.size(); ++index) {
    if (spFormatString[index] == '\'') {
      ...
      wsPatterns.push_back(
          WideString(spFormatString.subspan(token, index - token)));
      token = index + 1;
    }
  }
  wsPatterns.push_back(
    ...
    switch (spNumFormat[ccf]) {
      case '\'': {
        ...
        if (iLiteralLen > 0) {
          ...
          const size_t start = cc + 1 - iLiteralLen;
          if (WideStringView(spSrcNum.subspan(start, iLiteralLen)) !=
              wsLiteral) {
            return false;
          }
          cc = start;
      ...
      case 's':
        if (spSrcNum[cc] == '+' ||
          ...
          const size_t start = cc + 1 - iMinusLen;
          if (WideStringView(spSrcNum.subspan(start, iMinusLen)) != wsMinus) {
            return false;
          }
      ...
      case 'E': {
        ...
        while (cc < spSrcNum.size()) {
          ...
          if (iMinusLen > 0 && cc + 1 >= iMinusLen &&
              WideStringView(spSrcNum.subspan(cc + 1 - iMinusLen, iMinusLen)) ==
                  wsMinus) {
            bExpSign = true;
            cc -= iMinusLen;
            continue;
      ...
      case '$': {
        ...
        const size_t start = cc + 1 - iSymbolLen;
        if (WideStringView(spSrcNum.subspan(start, iSymbolLen)) != wsSymbol) {
          return false;
        }
      ...
      case ',': {
        if (cc < spSrcNum.size()) {
          if (iGroupLen > 0 && cc + 1 >= iGroupLen &&
              WideStringView(spSrcNum.subspan(cc + 1 - iGroupLen, iGroupLen)) ==
                  wsGroupSymbol) {
            cc -= (iGroupLen - 1);
          }
        }
...
bool CFGAS_StringFormatter::ParseZero(const WideString& wsSrcText) const {
  ...
  while (iPattern < spTextFormat.size() && iText < spSrcText.size()) {
    if (spTextFormat[iPattern] == '\'') {
      ...
      size_t iLiteralLen = wsLiteral.GetLength();
      if (iText + iLiteralLen > spSrcText.size() ||
          WideStringView(spSrcText.subspan(iText, iLiteralLen)) != wsLiteral) {
        return false;
      }
      iText += iLiteralLen;
      continue;
