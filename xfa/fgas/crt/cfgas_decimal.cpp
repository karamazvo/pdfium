...
CFGAS_Decimal::CFGAS_Decimal(uint64_t val)
    : u_mid_(static_cast<uint32_t>(FXMATH_DECIMAL_RSHIFT32BIT(val))),
      u_lo_(static_cast<uint32_t>(val)) {}
...
CFGAS_Decimal::CFGAS_Decimal(float val, uint8_t scale) {
  ...
  u_hi_ = static_cast<uint32_t>(phi);
  u_mid_ = static_cast<uint32_t>(pmid);
  u_lo_ = static_cast<uint32_t>(plo);
...
CFGAS_Decimal::CFGAS_Decimal(WideStringView str) {
  ...
  while (scale < FXMATH_DECIMAL_SCALELIMIT) {
    ...
    u_hi_ = u_hi_ * 0xA + FXMATH_DECIMAL_RSHIFT32BIT((uint64_t)u_mid_ * 0xA);
    u_mid_ = u_mid_ * 0xA + FXMATH_DECIMAL_RSHIFT32BIT((uint64_t)u_lo_ * 0xA);
    u_lo_ = u_lo_ * 0xA + (str.Front() - '0');
    if (pointmet) {
      scale++;
    }
    str = str.Substr(1);
...
double CFGAS_Decimal::ToDouble() const {
  ...
  double base = static_cast<double>(u_hi_) * pow * pow +
                static_cast<double>(u_mid_) * pow + static_cast<double>(u_lo_);
...
CFGAS_Decimal CFGAS_Decimal::operator*(const CFGAS_Decimal& val) const {
  uint64_t a[3] = {u_lo_, u_mid_, u_hi_},
           b[3] = {val.u_lo_, val.u_mid_, val.u_hi_};
  uint64_t c[6];
...
CFGAS_Decimal CFGAS_Decimal::operator/(const CFGAS_Decimal& val) const {
  ...
  bool neg = neg_ ^ val.neg_;
  uint64_t a[7] = {u_lo_, u_mid_, u_hi_},
           b[3] = {val.u_lo_, val.u_mid_, val.u_hi_}, c[7] = {0};
  uint8_t scale = 0;
