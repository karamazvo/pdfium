...
CFGAS_Decimal::CFGAS_Decimal(uint64_t val)
    : umid_(static_cast<uint32_t>(FXMATH_DECIMAL_RSHIFT32BIT(val))),
      ulo_(static_cast<uint32_t>(val)) {}

CFGAS_Decimal::CFGAS_Decimal(uint32_t val) : ulo_(static_cast<uint32_t>(val)) {}
...
CFGAS_Decimal::CFGAS_Decimal(uint32_t lo,
    ...
    : uhi_(hi),
      ...
      ulo_(lo),
      neg_(neg && IsNotZero()),
      uscale_(scale > FXMATH_DECIMAL_SCALELIMIT ? 0 : scale) {}
...
CFGAS_Decimal::CFGAS_Decimal(float val, uint8_t scale) {
  ...
  uhi_ = static_cast<uint32_t>(phi);
  umid_ = static_cast<uint32_t>(pmid);
  ulo_ = static_cast<uint32_t>(plo);
...
CFGAS_Decimal::CFGAS_Decimal(WideStringView str) {
  ...
  while (scale < FXMATH_DECIMAL_SCALELIMIT) {
    ...
    uhi_ = uhi_ * 0xA + FXMATH_DECIMAL_RSHIFT32BIT((uint64_t)umid_ * 0xA);
    umid_ = umid_ * 0xA + FXMATH_DECIMAL_RSHIFT32BIT((uint64_t)ulo_ * 0xA);
    ulo_ = ulo_ * 0xA + (str.Front() - '0');
    if (pointmet) {
      scale++;
    }
    str = str.Substr(1);
...
double CFGAS_Decimal::ToDouble() const {
  ...
  double base = static_cast<double>(uhi_) * pow * pow +
                static_cast<double>(umid_) * pow + static_cast<double>(ulo_);
  return (neg_ ? -1 : 1) * base * powf(10.0f, -uscale_);
}

void CFGAS_Decimal::SetScale(uint8_t newscale) {
  uint8_t oldscale = uscale_;
...
CFGAS_Decimal CFGAS_Decimal::operator*(const CFGAS_Decimal& val) const {
  uint64_t a[3] = {ulo_, umid_, uhi_}, b[3] = {val.ulo_, val.umid_, val.uhi_};
  uint64_t c[6];
  decimal_helper_raw_mul(a, 3, b, 3, c, 6);
  bool neg = neg_ ^ val.neg_;
  uint8_t scale = uscale_ + val.uscale_;
...
CFGAS_Decimal CFGAS_Decimal::operator/(const CFGAS_Decimal& val) const {
  ...
  bool neg = neg_ ^ val.neg_;
  uint64_t a[7] = {ulo_, umid_, uhi_}, b[3] = {val.ulo_, val.umid_, val.uhi_},
           c[7] = {0};
  uint8_t scale = 0;
  if (uscale_ < val.uscale_) {
    for (int i = val.uscale_ - uscale_; i > 0; i--) {
      decimal_helper_mul10_any(a, 7);
    }
  } else {
    scale = uscale_ - val.uscale_;
  }
