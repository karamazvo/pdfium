...
CFGAS_Break::CFGAS_Break(Mask<LayoutStyle> dwLayoutStyles)
    : dw_layout_styles_(dwLayoutStyles), cur_line_(&lines_[0]) {}
...
void CFGAS_Break::SetLayoutStyles(Mask<LayoutStyle> dwLayoutStyles) {
  dw_layout_styles_ = dwLayoutStyles;
  single_line_ = !!(dw_layout_styles_ & LayoutStyle::kSingleLine);
  comb_text_ = !!(dw_layout_styles_ & LayoutStyle::kCombText);
}
...
void CFGAS_Break::SetBreakStatus() {
  ...
  if (tc && tc->dw_status_ == CFGAS_Char::BreakType::kNone) {
    tc->dw_status_ = CFGAS_Char::BreakType::kPiece;
  }
}
