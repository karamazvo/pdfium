...
void CFGAS_TxtBreak::AppendChar_Combination(CFGAS_Char* pCurChar) {
  ...
  if (!comb_text_) {
    ...
    if (pLastChar &&
        (pLastChar->dw_char_styles_ & FX_TXTCHARSTYLE_ArabicShadda) == 0) {
      ...
      if (maybe_shadda.has_value()) {
        ...
        pCurChar->dw_char_styles_ |= FX_TXTCHARSTYLE_ArabicShadda;
        pLastChar->dw_char_styles_ |= FX_TXTCHARSTYLE_ArabicShadda;
...
CFGAS_Char::BreakType CFGAS_TxtBreak::AppendChar(wchar_t wch) {
  ...
  pCurChar->dw_char_styles_ = alignment_ | (1 << 8);
...
std::deque<CFGAS_Break::TPO> CFGAS_TxtBreak::EndBreakBidiLine(
  ...
  if (!cur_line_->HasArabicChar()) {
    ...
    pTC = &chars[0];
    tp.SetCharStyles(pTC->dw_char_styles_);
  ...
  while (i < iCount) {
    ...
    if (iBidiLevel != pTC->bidi_level_ ||
        pTC->dw_status_ != CFGAS_Char::BreakType::kNone) {
      if (iBidiLevel == pTC->bidi_level_) {
...
CFGAS_Char::BreakType CFGAS_TxtBreak::EndBreak(CFGAS_Char::BreakType dwStatus) {
  ...
  cur_line_->line_chars_.back().dw_status_ = dwStatus;
  if (dwStatus == CFGAS_Char::BreakType::kPiece)
    return dwStatus;
