  ...
  for (int32_t i = 0; i <= iLength; i++) {
    ...
    for (int32_t j = 0; j < iForms; j++) {
      ...
      if (!bEmptyChar || (bEmptyChar && !bSkipSpace)) {
        pCharPos.front().m_GlyphIndex = pFont->GetGlyphIndex(wForm);
#if BUILDFLAG(IS_APPLE)
        pCharPos.front().m_ExtGID = pCharPos.front().m_GlyphIndex;
#endif
        pCharPos.front().m_FontCharWidth = iCharWidth;
      }
      ...
      if (!bEmptyChar || (bEmptyChar && !bSkipSpace)) {
        pCharPos.front().m_Origin = CFX_PointF(fX, fY);
        ...
        if (!!(dwStyles & LayoutStyle::kCombText)) {
          ...
          pCharPos.front().m_Origin.x += fOffset;
        }
        if (chartype == FX_CHARTYPE::kCombination) {
          std::optional<FX_RECT> rtBBox = pFont->GetCharBBox(wForm);
          if (rtBBox.has_value()) {
            pCharPos.front().m_Origin.y =
                fYBase + fFontSize -
      ...
      if (!bEmptyChar || (bEmptyChar && !bSkipSpace)) {
        pCharPos.front().m_bGlyphAdjust = true;
        pCharPos.front().m_AdjustMatrix[0] = -1;
        pCharPos.front().m_AdjustMatrix[1] = 0;
        pCharPos.front().m_AdjustMatrix[2] = 0;
        pCharPos.front().m_AdjustMatrix[3] = 1;

        if (iHorScale != 100 || iVerScale != 100) {
          pCharPos.front().m_AdjustMatrix[0] =
              pCharPos.front().m_AdjustMatrix[0] * iHorScale / 100.0f;
          pCharPos.front().m_AdjustMatrix[1] =
              pCharPos.front().m_AdjustMatrix[1] * iHorScale / 100.0f;
          pCharPos.front().m_AdjustMatrix[2] =
              pCharPos.front().m_AdjustMatrix[2] * iVerScale / 100.0f;
          pCharPos.front().m_AdjustMatrix[3] =
              pCharPos.front().m_AdjustMatrix[3] * iVerScale / 100.0f;
        }
