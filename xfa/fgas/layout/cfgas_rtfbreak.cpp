...
CFGAS_RTFBreak::CFGAS_RTFBreak(Mask<LayoutStyle> dwLayoutStyles)
  ...
  SetBreakStatus();
  pagination_ = !!(m_dwLayoutStyles & LayoutStyle::kPagination);
}
...
CFGAS_Char::BreakType CFGAS_RTFBreak::AppendChar(wchar_t wch) {
  ...
  cur_line_->m_LineChars.emplace_back(wch, m_iHorizontalScale,
                                      m_iVerticalScale);
  CFGAS_Char* pCurChar = &cur_line_->m_LineChars.back();
  ...
  if (chartype != FX_CHARTYPE::kCombination &&
    ...
    if (!cur_line_->m_LineChars.empty()) {
      pCurChar = &cur_line_->m_LineChars.back();
    }
  }
...
void CFGAS_RTFBreak::AppendChar_Combination(CFGAS_Char* pCurChar) {
  ...
  if (iCharWidthValid > 0) {
    FX_SAFE_INT32 checked_width = cur_line_->m_iWidth;
    checked_width += iCharWidthValid;
    if (!checked_width.IsValid())
      return;

    cur_line_->m_iWidth = checked_width.ValueOrDie();
  }
}
...
void CFGAS_RTFBreak::AppendChar_Tab(CFGAS_Char* pCurChar) {
  ...
  int32_t& iLineWidth = cur_line_->m_iWidth;
...
CFGAS_Char::BreakType CFGAS_RTFBreak::AppendChar_Arabic(CFGAS_Char* pCurChar) {
  ...
  if (char_type_ >= FX_CHARTYPE::kArabicAlef &&
    ...
    if (pLastChar) {
      cur_line_->m_iWidth -= pLastChar->m_iCharWidth;
...
CFGAS_Char::BreakType CFGAS_RTFBreak::EndBreak(CFGAS_Char::BreakType dwStatus) {
  ...
  ++m_dwIdentity;
  if (!cur_line_->m_LinePieces.empty()) {
    if (dwStatus != CFGAS_Char::BreakType::kPiece)
      cur_line_->m_LinePieces.back().SetStatus(dwStatus);
    return cur_line_->m_LinePieces.back().GetStatus();
  }
  ...
  m_iReadyLineIndex = cur_line_ == &m_Lines[0] ? 0 : 1;
  ...
  if (!EndBreakSplitLine(pNextLine, bAllChars, dwStatus)) {
    ...
    if (!pagination_ && m_iAlignment != LineAlignment::Left) {
      EndBreakAlignment(tpos, bAllChars, dwStatus);
    }
  }
  cur_line_ = pNextLine;
  cur_line_->m_iStart = m_iLineStart;
...
bool CFGAS_RTFBreak::EndBreakSplitLine(CFGAS_BreakLine* pNextLine,
  ...
  if (!pagination_) {
    if (bAllChars && !bDone) {
      ...
      GetBreakPos(cur_line_->m_LineChars, bAllChars, true, &endPos);
    }
    return false;
  }

  const CFGAS_Char* pCurChars = cur_line_->m_LineChars.data();
  CFGAS_BreakPiece tp;
  tp.SetChars(&cur_line_->m_LineChars);
  bool bNew = true;
  uint32_t dwIdentity = static_cast<uint32_t>(-1);
  int32_t iLast = fxcrt::CollectionSize<int32_t>(cur_line_->m_LineChars) - 1;
  int32_t j = 0;
  for (int32_t i = 0; i <= iLast;) {
    ...
    if (i == iLast || pTC->m_dwStatus != CFGAS_Char::BreakType::kNone ||
      ...
      tp.SetCharCount(i - j);
      cur_line_->m_LinePieces.push_back(tp);
      bNew = true;
    } else {
      tp.IncrementWidth(pTC->m_iCharWidth);
      ++i;
...
std::deque<CFGAS_Break::TPO> CFGAS_RTFBreak::EndBreakBidiLine(
    CFGAS_Char::BreakType dwStatus) {
  CFGAS_Char* pTC;
  std::vector<CFGAS_Char>& chars = cur_line_->m_LineChars;
  if (!pagination_ && cur_line_->HasArabicChar()) {
    size_t iBidiNum = 0;
    for (size_t i = 0; i < cur_line_->m_LineChars.size(); ++i) {
      pTC = &chars[i];
  ...
  tp.SetStartPos(cur_line_->m_iStart);
  tp.SetChars(&chars);
  ...
  int32_t i = 0;
  int32_t j = 0;
  int32_t iCount = fxcrt::CollectionSize<int32_t>(cur_line_->m_LineChars);
  while (i < iCount) {
    pTC = &chars[i];
    if (iBidiLevel < 0) {
  ...
  int32_t iStartPos = cur_line_->m_iStart;
  for (const auto& it : tpos) {
    CFGAS_BreakPiece& ttp = cur_line_->m_LinePieces[it.index];
...
void CFGAS_RTFBreak::EndBreakAlignment(const std::deque<TPO>& tpos,
                                       bool bAllChars,
                                       CFGAS_Char::BreakType dwStatus) {
  int32_t iNetWidth = cur_line_->m_iWidth;
  int32_t iGapChars = 0;
  bool bFind = false;
  for (const TPO& pos : pdfium::Reversed(tpos)) {
    const CFGAS_BreakPiece& ttp = cur_line_->m_LinePieces[pos.index];
    if (!bFind)
  ...
  if (iGapChars > 0 && (m_iAlignment == LineAlignment::Distributed ||
    ...
    for (const auto& tpo : tpos) {
      CFGAS_BreakPiece& ttp = cur_line_->m_LinePieces[tpo.index];
      if (iStart < 0)
    ...
    if (m_iAlignment == LineAlignment::Center)
      iOffset /= 2;
    if (iOffset > 0) {
      for (auto& ttp : cur_line_->m_LinePieces) {
