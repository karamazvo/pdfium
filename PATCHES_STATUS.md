...
| `pdfium-android-arm64-r25-7-0137-single-pass-spatial-line-run.yml` | `workflow_dispatch` | **R25-7-0137 SINGLE-PASS SPATIAL LINE-RUN BUILD.** Applies after accepted 0136 and removes redundant per-line matrix/rectangle spatial construction from exact translation-origin runs. The existing bounded run now computes shared inflation once and emits aggregate native bounds plus exact 32-command block unions and candidate/no-op bits while it commits each source command. There is no deferred rebuild or second scan. Format 24, native payloads/runs, spatial blocks, candidate masks, replay, pixels, fallback, 3M-line ceiling, and 96 MiB cap remain unchanged. Acceptance requires `spatialLineRunCommands` to cover Q16's bulk commands, identical representation and pixels, lower repeated-median Q16 `parseUs`, and no 11.pdf, EP23, Study Notes, or 6Steps regression. |
