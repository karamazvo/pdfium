...
| `pdfium-android-arm64-r25-2-0100-ordered-line-batches-proven-outer-darken.yml` | `workflow_dispatch` | **R25-2-0100 FAILED COMPILE.** Historical workflow retained for audit. The bundled AGG `path_storage` has no `remove_all()` member, so no 0100 artifact was produced. Use 0101. |
| `pdfium-android-arm64-r25-2-0101-reusable-agg-path-reset.yml` | `workflow_dispatch` | **R25-2-0101 REUSABLE AGG PATH RESET BUILD.** Applies the exact generation-2 stack through 0101. Adds the missing allocation-preserving `path_storage::remove_all()` primitive required by 0100's bounded line packets. Performance acceptance remains nonzero `lineBatchCommands`, materially fewer `lineBatchDispatches`, zero AGG fallback, and nonzero proven `darkenDraws`. |
