...
| `pdfium-android-arm64-r25-9-0139-exact-ordered-line-raster-tape.yml` | `workflow_dispatch` | **R25-9-0139 EXACT ORDERED LINE RASTER TAPE BUILD.** Removes the redundant `agg::path_storage` write/read round trip for exact opaque undashed butt lines. The accepted four-vertex math, float transform order, AGG commands, clip, fill rule, one raster pass, immediate source-over composite, painter order, and fail-closed generic fallback remain unchanged. The existing maximum-256-entry packet is the only tape; no allocation, cache, classifier, lock, JNI/Kotlin path, or UI-thread work is added. `replayUs` is moved before Android log truncation. Acceptance requires identical pixels and counters plus at least 10% lower repeated-median Q16 replay/bitmap time without normal-page, 11.pdf, or EP23 regression. |
| `pdfium-android-arm64-r25-10-0140-direct-stream-acquisition-sink.yml` | `workflow_dispatch` | **R25-10-0140 DIRECT STREAM ACQUISITION SINK BUILD - NOT ACCEPTED.** The representation remained identical to 0138, but the device session reported Q16 acquisition 3.02x slower and total 2.52x slower. The same session made 11.pdf roughly 3x slower even though `parserStreamSinkLines=0`, so it does not isolate 0140 causality. Architecturally, per-line parser callbacks also interleave sequential token scanning with scattered RenderProgram mutation. Keep 0138 as the stable base and do not extend 0140. |
| `pdfium-android-arm64-r25-11-0141-bounded-packed-operator-decoder.yml` | `workflow_dispatch` | **R25-11-0141 BOUNDED PACKED OPERATOR DECODER BUILD - PERFORMANCE NOT ACCEPTED.** The decoder covered all 2,940,110 Q16 eligible commands and preserved representation, but acquisition regressed from about 3,185 ms on 0138 to 4,045 ms and total preview from about 4,651 ms to 6,054 ms. This closes fixed-layout parser scanning as a Pareto direction. Keep 0138 as the accepted base; do not extend 0141. |
| `pdfium-android-arm64-r25-12-0142-direct-compact-line-span-executor.yml` | `workflow_dispatch` | **R25-12-0142 DIRECT COMPACT LINE SPAN EXECUTOR BUILD - PERFORMANCE NOT ACCEPTED.** Restarts from accepted 0138 and excludes 0139-0141. The first Samsung SM-A1660 Q16 run reached 2,365,882 compact commands with zero fallback, but acquisition regressed from about 3,185 ms to 4,162 ms, bitmap time from about 1,465 ms to 1,846 ms, and total preview from about 4,651 ms to 6,009 ms. The path preserved one raster/composite per command, so it did not reduce the dominant 2,590,767 raster passes. Keep 0138 as the accepted experimental base, do not extend 0142, and use 0143 for the next revision. |
| `pdfium-android-arm64-r25-13-0143-exact-axis-aligned-scan-kernel.yml` | `workflow_dispatch` | **R25-13-0143 EXACT AXIS-ALIGNED SCAN KERNEL BUILD - PERFORMANCE REJECTED.** Q16 exercised 1,553,522 exact axis-aligned draws, about 60% of 2,590,767 raster passes, but bitmap time was 1,478 ms versus 1,465 ms on accepted 0138. Acquisition regressed from 3,185 ms to 3,978 ms and total preview from 4,651 ms to 5,456 ms. The mechanism did not reduce the dominant destination work and is excluded from 0144. |
| `pdfium-android-arm64-r25-14-0144-exact-source-color-noop-elision.yml` | `workflow_dispatch` | **R25-14-0144 EXACT SOURCE-COLOR NO-OP ELISION BUILD - PERFORMANCE REJECTED.** Q16 skipped 3,768,260 of 4,183,709 tested pixels (90.1%), but bitmap time regressed from 1,465 ms on accepted 0138 to 1,658 ms; acquisition/total/first-visible also regressed to 3,826/5,484/5,835 ms. This proves final channel compositing is not the dominant Q16 cost. 11 Darken rendering, EP23 fills, and canonical 6Steps pages received effectively no benefit. Keep 0138 as the accepted base and exclude 0144 from the next build. |
| `pdfium-android-arm64-r25-15-0145-exact-zero-coverage-stroke-elision.yml` | `workflow_dispatch` | **R25-15-0145 EXACT ZERO-COVERAGE STROKE ELISION BUILD - PERFORMANCE REJECTED.** Restarts from accepted 0138 and excludes rejected 0139-0144. The exact AGG 24.8 retraced-polygon predicate reported `zeroCoverageStrokes=0` in every observed replay: 40 for 11, 51 for Q16, 64 for EP23, and 11 for 6Steps. Q16 cold acquire/bitmap/total was 3,942/1,967/5,910 ms versus 3,185/1,465/4,651 ms on accepted 0138. The predicate is conservative and preserves the unchanged path when it does not match, but it removes none of the surviving workload. Keep 0138 as the accepted base; do not broaden the predicate with approximate width or coverage thresholds. |
| `pdfium-android-arm64-r25-16-0146-exact-direct-axis-aligned-spans.yml` | `workflow_dispatch` | **R25-16-0146 EXACT DIRECT AXIS-ALIGNED SPANS BUILD - INTEGRATION INVALID, DO NOT EXTEND AS A PERFORMANCE BASE.** Restarts from accepted 0138 and excludes rejected 0139-0145. All 26 11, 40 Q16, 101 EP23, and 17 observed 6Steps replays reported `directAxisAlignedSpanDraws=0`. Q16 retained 2,590,767 raster passes despite 2,365,894 direct butt strokes. Android creates BGRA bitmaps but also always passes `FPDF_REVERSE_BYTE_ORDER`; the 0146 predicate requires `!rgb_byte_order_`, making the executor unreachable in production. Keep 0138 as the accepted base. A new revision may add byte-exact reversed-order RGBA compositing with canonical alpha math and fail-closed fallback; do not amend 0146, remove the JNI flag, or weaken geometry rules. |
| `pdfium-android-arm64-r25-17-0147-exact-reverse-byte-direct-spans.yml` | `workflow_dispatch` | **R25-17-0147 EXACT REVERSE-BYTE DIRECT SPANS BUILD - PERFORMANCE REJECTED.** The Android byte-order correction activated exactly as intended: Q16 executed 1,553,522 direct spans and reduced AGG raster passes from 2,590,767 to 1,037,245. Bitmap time improved only 11.7%, from 1,465 ms on accepted 0138 to 1,293 ms, while cold acquisition regressed from 3,185 to 3,783 ms and total preview regressed from 4,651 to 5,077 ms. One dense zoom tile still admitted 2,388,532 candidates, rendered 2,388,761 native draws, and occupied the sole render lane for 3,601 ms; later sparse tiles then waited up to 4,121 ms. 11.pdf remained on its Darken executor, EP23 p2/p3 received zero direct-span coverage, and all observed 6Steps pages remained `canonical_no_exact_candidate`. Keep 0138 as the accepted experimental base, close the primitive direct-span direction, and reserve 0148 for a different exact mechanism. |
| `pdfium-android-arm64-r25-18-0148-canonical-render-phase-profile.yml` | `workflow_dispatch` | **R25-18-0148 CANONICAL RENDER PHASE PROFILE - DIAGNOSTIC COMPLETE, NOT A PERFORMANCE BASE.** Restarts from accepted 0138 and excludes rejected 0139-0147. Device data showed sub-millisecond bitmap allocation and generally sub-millisecond session waits; 11 and Q16 active bitmap time was accelerator-dominated. Q16 cold acquisition/replay measured about 3,677/1,630 ms. EP23 page 3 reported a Form cache hit yet still spent about 844 ms acquiring the same 28,071-fill, 707,225-point Form because the hit was attached only after canonical child materialization. 6Steps remained image-dominated. Keep 0138 as the performance parent and use the profile only as evidence. |
| `pdfium-android-arm64-r25-19-0149-early-immutable-form-tape.yml` | `workflow_dispatch` | **R25-19-0149 EARLY IMMUTABLE FORM TAPE BUILD.** Restarts from accepted 0138 and excludes 0139-0148. Exact all-native resource-independent Form programs are adopted from the existing 16-entry/96-MiB document cache before content-parser construction. The cache retains exact bounds and alpha metadata; detached replay is allowed only when every command is native and no canonical object lookup is possible. Original invocation state is retained inline so enumeration, editing, mutation fallback, or failed executor preflight performs one canonical reparse. Expected win is repeated EP23-style Form acquisition; Q16 cold root acquisition, 11 Darken replay, and first-use Form compilation are explicitly unchanged. |
| `pdfium-android-arm64-r25-20-0150-early-form-tape-build-fix.yml` | `workflow_dispatch` | **R25-20-0150 EARLY FORM TAPE BUILD FIX.** Applies 0149 and then replaces the stale `nullptr` no-key sentinel with `std::nullopt` for the new optional cache-result type. It changes no rendering, ownership, cache, memory, eligibility, or performance behavior. Use 0150, not the failed 0149 workflow, to test the early immutable Form tape. The transform-sized image work advances to 0151. |
| `pdfium-android-arm64-r25-normal-exp-0003-direct-image-mask-composite.yml` | `workflow_dispatch` | **R25 NORMAL EXP 0003 DIRECT IMAGE-MASK COMPOSITE.** Restarts from accepted 0138 and excludes normal experiments 0001/0002. For exact unit-alpha, Normal-blend, same-size BGRx plus 8-bit mask paints without group knockout or an AGG soft clip, the existing scanline compositor consumes the mask directly as source coverage. A failed predicate returns before pixels to the unchanged BGRA materialization path. The experiment adds no classifier, cache, persistent allocation, lock, thread, JNI/Kotlin path, or UI-thread work. Expected benefit is removal of the measured masked-image materialization pass on eligible normal pages; acceptance requires byte/pixel equivalence and a repeatable 6Steps preview gain without normal-page, 11, Q16, or EP23 regression. |
| `pdfium-android-arm64-r25-normal-exp-0004-exact-opaque-smask-elision.yml` | `workflow_dispatch` | **R25 NORMAL EXP 0004 EXACT OPAQUE SMASK ELISION.** Restarts from accepted 0138 and does not stack marginal experiment 0003. At page-image-cache insertion, an allocation-free scan removes only realized positive-size 8-bit masks whose every logical coverage byte is exactly 255. This exact identity reduction avoids retaining and repeatedly scaling/compositing opaque masks; all other formats, malformed rows, and non-255 masks remain canonical. Corpus inspection found 13 of 32 6Steps soft masks, totaling 6,395,955 decoded pixels, are exact opaque candidates. No classifier, threshold, approximation, new cache, persistent metadata, lock, thread, JNI/Kotlin path, or UI work is added. |
| `pdfium-android-arm64-r25-normal-2-0008-unified-destination-row-image-executor.yml` | `workflow_dispatch` | **R25 NORMAL-2 0008 UNIFIED DESTINATION-ROW IMAGE EXECUTOR.** Extends accepted 0138 plus normal experiments 0005-0007 and explicitly excludes heavy-path experiments 0139-0151. Exact eligible image and identity-gray SMask sources are advanced through two bounded canonical stretchers one destination row at a time, then composed once through the existing AGG scanline compositor. This removes both destination-sized image and mask temporaries while retaining canonical decoder/cache ownership, weight tables, alpha math, clipping, painter order, bounded memory, and fail-closed fallback before the first pixel. |
| `pdfium-android-arm64-r25-image-1-0001-exact-mask-guided-color-execution.yml` | `workflow_dispatch` | **R25 IMAGE-1 0001 EXACT MASK-GUIDED COLOR EXECUTION - PERFORMANCE REJECTED.** The mechanism was destination-side and pixel-exact, but the device run remained dominated by cold JPX decode: 756-wide calls had a 279 ms median with about 261 ms inside native rendering, while the first tile reached 554 ms and later warm tiles fell to 49-61 ms. It did not provide the required Pareto improvement and is excluded from the JPX series. Keep it for traceability, not as a performance parent. |
| `pdfium-android-arm64-r25-jpx-1-0002-bounded-parallel-jpx-decode.yml` | `workflow_dispatch` | **R25 JPX-1 0002 BOUNDED PARALLEL JPX DECODE - PERFORMANCE REJECTED.** The controlled device session did not meet its 20% gate: paired completed 756-pixel median regressed about 11%, first cold zoom tile regressed about 13%, warm tiles were unchanged, and cancellation p95 moved from roughly 20 to 27 ms. Source review also confirmed OpenJPEG already dispatches both Tier-1 and inverse DWT work through the same pool, so extending this design would add scheduling around an already-parallel memory-bound pipeline. Keep it for traceability; do not use it as a parent. |
| `pdfium-android-arm64-r25-jpx-2-0003-exact-arm64-jpx-color-finish.yml` | `workflow_dispatch` | **R25 JPX-2 0003 EXACT ARM64 JPX COLOR FINISH - PERFORMANCE REJECTED.** Against accepted `cancel-1-0003`, first-visible latency regressed from about 735 to 972 ms, cold page 0 from about 351 to 468 ms, and paired completed 756-pixel renders regressed about 49% by mean and 53% by median. Warm tiles were effectively unchanged at 59 versus 60 ms and cancellation p95 moved from about 16 to 19 ms. This proves a separate SIMD MCT pass followed by a separate SIMD packing pass does not remove the dominant memory traffic. Keep it for traceability; do not use it as a parent. |
| `pdfium-android-arm64-r25-jpx-3-0004-exact-fused-jpx-output-sink.yml` | `workflow_dispatch` | **R25 JPX-3 0004 EXACT FUSED JPX OUTPUT SINK - BUILD-READY EXPERIMENT.** Restarts from accepted `cancel-1-0003` and excludes rejected `image-1-0001`, `jpx-1-0002`, and `jpx-2-0003`. For exact single-tile, three-component, unsigned 8-bit, full-resolution irreversible standard-MCT JPX, OpenJPEG writes the final interleaved PDFium BGR bitmap directly from post-DWT float planes in one pass using the same scalar expression order, rounding, level shift, and clamp. Palette/CDEF, custom/reversible MCT, multitile, subsampled, incomplete, non-RGB, or mismatched layouts fail closed before output and use the canonical path. One authoritative final bitmap is allocated; there is no second destination, cache, worker, lock, classifier, or persistent allocation. Cancellation is polled at most every 256 output pixels and partial output is never published. Accept only with byte-identical pixels, at least 15% repeated cold JPX improvement against `cancel-1-0003`, unchanged cancellation p95, and no 6Steps, 11, Q16, or EP23 regression. |
| `pdfium-android-arm64-r25-jpx-probe-0001-fused-output-activation.yml` | `workflow_dispatch` | **R25 JPX PROBE 0001 BOUNDED FUSED-OUTPUT ACTIVATION.** Applies `r25-jpx-3-0004` unchanged and samples the first 64 successful JPX decodes plus every 128th decode thereafter. It reports codec candidacy, PDFium callback result, actual OpenJPEG sink use, reduction level, colorspace, and first-component metadata. It changes no pixels, eligibility, allocation, cache, thread, lock, cancellation, or scheduling behavior and deliberately does not consume functional revision `r25-jpx-3-0005`. Use it only to identify the exact runtime rejection; do not use its logged timings for final acceptance. |
| `pdfium-android-arm64-r25-jpx-6-0007-bounded-exact-jpx-source-window-reuse.yml` | `workflow_dispatch` | **R25 JPX-6 0007 BOUNDED EXACT JPX SOURCE-WINDOW REUSE.** Extends `jpx-5-0006` by retaining one successful clipped JPX bitmap in the existing page image cache. Reuse requires the same stream-owned entry, exact source-rectangle containment, and the identical requested decode size. Full cached images retain priority; disjoint, differently sized, canceled, failed, and oversized requests use the existing path. Retention is capped at 16 MiB per page cache, replacement is single-owner, and no copy, classifier, lock, worker, queue, sparse union, or document-specific policy is added. Cold first preview and first-use disjoint regions are explicitly unchanged; accept only for byte-identical pixels and material repeated/contained-region wins. |
| `pdfium-android-arm64-r25-jpx-7-0008-bounded-single-tile-jpx-decode-session.yml` | `workflow_dispatch` | **R25 JPX-7 0008 BOUNDED SINGLE-TILE JPX DECODE SESSION.** Restarts from accepted `jpx-5-0006` and excludes ineffective `0007`. A proven single-tile, single-component JPX image may retain one OpenJPEG decoder session in the existing page image cache, allowing exact adjacent or disjoint source windows at the same reduction level to reuse immutable stream/header/codestream state. Every window still follows canonical PDFium conversion, mask, stretch, blend, and painter order. Multi-tile, multi-component, full-image, mismatched, failed, canceled, or over-16-MiB sessions fail closed to `0006`; no document classifier, second bitmap cache, lock, worker, queue, or UI work is added. |
