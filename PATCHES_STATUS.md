...
| `pdfium-android-arm64-r25-16-0146-exact-direct-axis-aligned-spans.yml` | `workflow_dispatch` | **R25-16-0146 EXACT DIRECT AXIS-ALIGNED SPANS BUILD.** Restarts from accepted 0138 and excludes rejected 0139-0145. It combines exact AGG 24.8 axis-aligned rectangle coverage with direct opaque normal BGRx/BGRA compositing, removing both the generic scan converter and generic span renderer for that exact primitive. BGRA uses the same destination-alpha and alpha-ratio equations as PDFium's canonical compositor; this is the Android page/tile destination format. Unsupported destination, alpha, backdrop, clip mask, byte order, full-cover mode, cap, dash, or geometry fails closed to unchanged 0138 before pixels. No classifier, threshold, persistent allocation, cache, lock, thread, JNI/Kotlin, UI-thread work, merge, or reorder is added. Acceptance requires pixel identity, material `directAxisAlignedSpanDraws`, and at least 20% repeated-median Q16 bitmap improvement without corpus regressions. |
