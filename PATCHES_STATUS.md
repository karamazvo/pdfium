...
| `pdfium-android-arm64-r25-4-0124-single-pass-path-paint-dispatch.yml` | `workflow_dispatch` | **R25-4-0124 SINGLE-PASS PATH PAINT DISPATCH BUILD.** Starts the exact streaming acquisition generation without replacing the r25-3 sidecar or pixel executor. The path-local parser consumes every standard path paint/end terminator once and invokes the existing handler directly; unrecognized operators retain the prior rewind and outer-parser fallback. Expected Q16 proof: identical command, omission, state, spatial, retained-byte, replay, draw, and pixel results with materially lower `acquireMs`/`compileWindowMs`. RenderProgram format 21 and its 96 MiB cap are unchanged; there is no classifier, cache, new bitmap, lock, JNI/Kotlin work, or UI-thread work. |
