...
| `pdfium-android-arm64-r25-13-0143-exact-axis-aligned-scan-kernel.yml` | `workflow_dispatch` | **R25-13-0143 EXACT AXIS-ALIGNED SCAN KERNEL BUILD.** Restarts from accepted 0138 and excludes rejected 0139-0142. Exact device-axis-aligned butt-stroke rectangles bypass AGG general cell construction and sorting, emit AGG-equivalent edge/interior scanline coverage into the existing renderer, and composite every source operation independently in painter order. Diagonal, rotated, sheared, dashed, non-butt, fill, and unsupported operations remain on the accepted 0138 path before pixels. Reuses clip-width-bounded scanline storage; adds no page classifier, approximation, per-line heap allocation, cache, lock, JNI/Kotlin, or UI-thread work. Acceptance requires pixel-identical unit/device output, meaningful `exactAxisAlignedLineDraws` coverage, at least 20% lower repeated-median Q16 bitmap time, and no normal-page, 11.pdf, EP23, memory, cancellation, or publication regression. |
