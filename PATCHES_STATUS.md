...
| `pdfium-android-arm64-r25-jpx-1-0002-bounded-parallel-jpx-decode.yml` | `workflow_dispatch` | **R25 JPX-1 0002 BOUNDED PARALLEL JPX DECODE - BUILD READY, NOT YET ACCEPTED.** Extends accepted `cancel-1-0003` and excludes rejected `image-1-0001`. Android enables OpenJPEG's existing pthread backend and each canonical JPX decoder requests exactly two workers for independent Tier-1 codeblock jobs, with serial fallback if pool creation fails. Decode area, resolution, component math, PDFium conversion/cache/composition, and painter order remain unchanged. RGB, RGBA, and grayscale fixtures compare serial and two-worker output byte for byte. No classifier, threshold, new cache, second pixel owner, JNI/Kotlin route, or UI-thread work is added. Acceptance requires exact pixels, at least 20% repeated cold decode improvement, cancellation p95 at most 20 ms, and no 6Steps/11/Q16/EP23 regression. |
