...
| `pdfium-android-arm64-r25-2-0100-ordered-line-batches-proven-outer-darken.yml` | `workflow_dispatch` | **R25-2-0100 ORDERED LINE BATCH + PROVEN OUTER DARKEN BUILD.** Applies the exact generation-2 stack through 0100. Q16 proof is nonzero `lineBatchCommands`, materially fewer `lineBatchDispatches`, `maxLineBatch > 1`, and zero AGG fallback. `11.pdf` proof is nonzero `darkenDraws` after the one-time opaque-backdrop proof. Transparent or unsupported contexts fail closed to canonical rendering. |
