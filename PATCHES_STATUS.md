...
| `pdfium-android-arm64-r25-10-0140-direct-stream-acquisition-sink.yml` | `workflow_dispatch` | **R25-10-0140 DIRECT STREAM ACQUISITION SINK BUILD.** Extends accepted 0138 and excludes unaccepted 0139. The transactional exact translation-line tokenizer submits one decoded line directly to the authoritative RenderProgram builder, which commits payload/no-op ownership, source ordinal, native run, aggregate bounds, and spatial block membership in one pass. It removes two fixed 256-entry staging arrays and a later batch rescan while preserving exact rollback to the first uncommitted operator, format 24, replay, pixels, the 256-command work quantum, the 3M-line limit, and the 96 MiB cap. Expected Q16 proof: `parserStreamSinkLines == spatialLineRunCommands`, all representation/replay counters match 0138, repeated cold `parseUs`/`acquireMs` improve by at least 10 percent with 20 percent as the target, and ordinary-page, 11.pdf, and EP23 performance remains unchanged. |
