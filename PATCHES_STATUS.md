...
| `pdfium-android-arm64-r25-4-0125-exact-streaming-line-compiler.yml` | `workflow_dispatch` | **R25-4-0125 EXACT STREAMING LINE COMPILER BUILD.** Supersedes and excludes failed 0124, applying directly after accepted 0123. After one ordinary exact line establishes immutable lowering context, a fixed-scratch scanner transactionally consumes the complete balanced `q a b c d e f cm x0 y0 m x1 y1 l S Q` unit and emits the transformed line through the same exact owned-line lowerer. The balanced wrapper has zero net graphics-state effect; a grammar or content-stream-boundary miss changes no parser state, and a lowering rejection retains a canonical `CPDF_PathObject` at the same source ordinal. No page classifier, count threshold, approximate number conversion, extra bitmap, cache, lock, JNI/Kotlin path, or UI-thread work is added. Expected Q16 proof: `parserFusedLines` approaches the approximately 2.94 million eligible transformed lines, acquisition falls materially, and all representation/replay/pixel counters remain identical to 0123. |
