...
| `pdfium-android-arm64-r25-11-0141-bounded-packed-operator-decoder.yml` | `workflow_dispatch` | **R25-11-0141 BOUNDED PACKED OPERATOR DECODER BUILD.** Restarts from accepted 0138 and excludes 0139/0140. After exact semantic line context is established, a transactional decoder recognizes the common exact byte layout and fills the existing 16-byte worker-stack records for at most 256 commands before one authoritative builder commit. A layout, stream-boundary, state, or budget mismatch rewinds to the first unconsumed command and resumes canonical PDFium. No classifier, persistent allocation, cache, lock, JNI/Kotlin path, UI-thread work, painter-order change, or raster change is added. Acceptance requires identical representation/replay/pixels, high `packedLineRunCommands` coverage, at least 20% lower controlled cold Q16 acquisition, and unchanged 11.pdf, EP23, and normal-page routing. |
