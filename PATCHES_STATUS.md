...
| `pdfium-android-arm64-r25-4-0126-exact-packed-translation-line-compiler.yml` | `workflow_dispatch` | **R25-4-0126 EXACT PACKED TRANSLATION-LINE COMPILER BUILD.** Extends 0125 with an exact transactional scanner for canonical `q 1 0 0 1 tx ty cm 0 0 m dx dy l S Q` units. It converts only `tx/ty/dx/dy`, stores each drawable match in a fixed-chunk 16-byte zero-origin endpoint/translation payload, and reconstructs the exact zero start inside the existing ordered AGG replay. Any spelling, state, transform, budget, or lowering mismatch follows the general 0125 scanner or canonical PDFium without advancing parser state. It adds no classifier, threshold, mutable cache, lock, page-sized temporary allocation, JNI/Kotlin path, or UI-thread work; total native-line and retained-program ceilings remain bounded. Expected Q16 proof: `parserConstantFoldedLines` approaches `parserFusedLines`, `compactTranslationLines` owns most drawable lines, retained bytes fall materially, and acquisition improves with unchanged commands, no-ops, order, candidates, draws, and pixels. |
