...
| `pdfium-android-arm64-r25-17-0147-exact-reverse-byte-direct-spans.yml` | `workflow_dispatch` | **R25-17-0147 EXACT REVERSE-BYTE DIRECT SPANS BUILD.** Restarts from accepted 0138 and excludes rejected 0139-0145 plus integration-invalid 0146. The exact axis-aligned four-vertex butt-stroke compositor now supports both standard BGRx/BGRA and Android reverse-byte RGBx/RGBA memory with canonical alpha equations. Unsupported geometry, alpha, blend, backdrop, clip-mask, or full-cover state falls back before any direct pixel write. Tests compare standard and reversed byte layouts byte-for-byte with canonical `DrawPath()`, including nonzero and zero BGRA destination alpha. No classifier, threshold, persistent allocation, cache, lock, JNI/Kotlin path, or UI-thread work is added. Accept only with substantial nonzero Q16 `directAxisAlignedSpanDraws`, materially lower bitmap time than accepted 0138, identical pixels, and no normal-page, 11, or EP23 regression. |
