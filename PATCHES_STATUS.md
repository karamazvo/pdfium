...
| `pdfium-android-arm64-r25-jpx-2-0003-exact-arm64-jpx-color-finish.yml` | `workflow_dispatch` | **R25 JPX-2 0003 EXACT ARM64 JPX COLOR FINISH EXPERIMENT.** Restarts from accepted `cancel-1-0003` and explicitly excludes rejected `image-1-0001` and `jpx-1-0002`. ARM64 NEON executes the existing irreversible MCT equations four samples at a time without fused arithmetic, then packs exact 8-bit RGB/RGBA component low bytes sixteen pixels at a time with interleaved stores. The common path avoids the two scalar pointer/adjust vectors; unsupported layouts retain the canonical scalar loop. No decoder thread, lock, cache, classifier, persistent allocation, second pixel owner, JNI/Kotlin route, or UI work is added. Accept only with byte-identical pixels, at least 15% repeated cold JPX improvement, unchanged cancellation p95, and no normal-page or heavy-path regression. |
