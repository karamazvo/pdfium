...
| `pdfium-android-arm64-r25-14-0144-exact-source-color-noop-elision.yml` | `workflow_dispatch` | **R25-14-0144 EXACT SOURCE-COLOR NO-OP ELISION BUILD.** Restarts from accepted 0138 and excludes rejected 0139-0143. The existing ordered AGG raster passes remain independent and source ordered. For opaque Normal-blend operations without a backdrop, a destination pixel that already exactly equals the source color, including opaque BGRA alpha, skips only the mathematically redundant channel composite/write. All other pixels use the unchanged compositor. Adds no classifier, geometry merge, allocation, cache, lock, JNI/Kotlin, or UI-thread work. `sourceColorTestedPixels` and `sourceColorNoOpPixels` expose hit ratio; acceptance requires pixel identity and at least 20% repeated-median Q16 bitmap improvement without normal-page, 11, or EP23 regression. |
