...
| `pdfium-android-arm64-r25-15-0145-exact-zero-coverage-stroke-elision.yml` | `workflow_dispatch` | **R25-15-0145 EXACT ZERO-COVERAGE STROKE ELISION BUILD.** Restarts from accepted 0138 and excludes rejected 0139-0144. Before AGG cell generation, the existing exact four-vertex butt-stroke polygon is transformed once to device space and converted with AGG's signed 24.8 `poly_coord()` semantics. A command is skipped only when opposite vertex pairs become exact retraced edges; every other command submits the same device vertices to the existing independent source-order raster/composite path. Adds only stack-local fixed-point values and aggregate `zeroCoverageStrokes` telemetry: no classifier, threshold, persistent allocation, cache, lock, thread, JNI/Kotlin, UI-thread work, paint merge, or reordering. Acceptance requires lossless pixel identity and at least 20% repeated-median Q16 bitmap improvement with no normal-page, 11, EP23, memory, cancellation, or publication regression. |
