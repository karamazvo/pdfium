...
| `pdfium-android-arm64-r25-18-0148-canonical-render-phase-profile.yml` | `workflow_dispatch` | **R25-18-0148 CANONICAL RENDER PHASE PROFILE - DIAGNOSTIC, DEVICE PROFILE PENDING.** Restarts from accepted 0138 and excludes rejected 0139-0147. Adds a fixed-memory, lock-free, exclusive progressive-render profiler that emits one `VeloceCanonicalProfile` summary per render, separating traversal/setup/clip/transparency/text/path/image/shading/Form/background/soft-mask/accelerator/parse time and cancellation checkpoints without changing pixels or routing. The integration app separately reports bitmap allocation and single-session-lane wait. Use this build to select the next performance mechanism from measured dominant cost; do not promote it as a production performance revision. |
