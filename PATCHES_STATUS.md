...
| `pdfium-android-arm64-r25-12-0142-direct-compact-line-span-executor.yml` | `workflow_dispatch` | **R25-12-0142 DIRECT COMPACT LINE SPAN EXECUTOR BUILD.** Restarts from accepted 0138 and excludes 0139-0141. Exact 16-byte translation-origin lines cross the RenderProgram/device boundary as a bounded 256-entry compact packet, avoiding per-line `CFX_OrderedPathDraw`, render-status matrix materialization, and AGG path-storage round trips. AGG validates the complete packet and precomputes an 8 KiB four-vertex stack tape before its first pixel, then preserves one independent raster pass and immediate source-order composite per PDF paint command. Any rejection reconstructs the established ordered packet. No merge, classifier, threshold, heap allocation, persistent cache, lock, JNI/Kotlin path, or UI-thread work is added. Acceptance requires identical pixels/counters, high `compactLineSpanCommands`, zero fallback, at least 10% lower repeated Q16 replay/bitmap time, and no 11.pdf, EP23, or normal-page regression. |
