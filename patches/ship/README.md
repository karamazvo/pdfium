...
| r25-3-0118 | `0118-veloce-parser-direct-line-emission.patch` | Sends parser-recognized two-point strokes directly through the same exact native lowerer, avoiding a temporary `CPDF_PathObject`, duplicate path storage, and immediate decode. Unsupported state and clipping paths still materialize canonical objects. |
...
| r25-4-0128 | `0128-veloce-render-program-success-lazy-sidecar.patch` | Makes RenderProgram activation depend on exact native ownership instead of the first path object. Allocation-free semantic preflight rejects unsupported page-object and parser-direct line candidates before builder construction; the unchanged complete lowerer must record a native command before the builder is installed. A later activation starts after an implicit canonical prefix and preserves source order. Adds one root-holder `acquire_policy` line with parse/finish time, preactivation probes/rejects, activation source/ordinal, native commands, and program bytes; there are no per-object clocks or logs. No classifier, threshold, cache, thread, lock, JNI/Kotlin path, or UI-thread work is added. |
| r25-4-0129 | `0129-veloce-render-program-proportional-line-storage.patch` | Replaces the unconditional 4,096-line first allocation in both native line stores with shared 16/64/256/1,024 prefix chunks followed by fixed 4,096-line dense chunks. Small exact Form programs retain proportional memory, while dense Q16-style programs keep O(1) indexing and the same steady-state chunk size. Payloads never move or copy, parser budget reservations follow the next real chunk, and the existing line-count and 96 MiB caps remain authoritative. Eligibility, commands, painter order, spatial selection, replay, pixels, threading, and canonical fallback are unchanged. |
| r25-6-0136 | `0136-veloce-single-pass-bulk-line-run.patch` | Corrects 0135's violation of its single-pass contract. Exact zero-coverage butt-cap lines remain ordered commands inside the same bounded parser packet, use the existing ranked no-op bit with no geometry payload, and no longer force the parser to rewind and reparse every following suffix. Drawable payload ranks, native runs, spatial ordinals, painter order, canonical fallback, 3M-line limit, and 96 MiB cap remain unchanged. Adds no classifier, cache, allocation layer, lock, thread, JNI/Kotlin path, UI-thread work, or raster behavior. |
| r25-7-0137 | `0137-veloce-single-pass-spatial-line-run.patch` | Starts the structural acquisition series by folding translation-line bounds and spatial-block construction into the existing accepted 256-command run. It computes the exact shared stroke inflation once, updates compact payloads, aggregate native bounds, candidate bits, no-op ordinals, and 32-command block unions in one source-order pass, and removes one matrix, transformed endpoint rectangle, normalization, containment object, and spatial append dispatch per drawable line. Representation format 24, block boundaries, candidate masks, painter order, pixels, canonical fallback, replay, 3M-line ceiling, and 96 MiB cap remain unchanged. It adds no deferred scan, mutable sidecar, cache, classifier, allocation layer, thread, lock, JNI/Kotlin path, UI-thread work, or raster behavior. |
| r25-8-0138 | `0138-veloce-ordered-stroke-renderer-tape.patch` | Reuses one stack-owned immutable AGG stroke renderer across each existing bounded homogeneous ordered packet. Every source stroke still resets its rasterizer, performs one raster pass, and composites immediately in source order; no antialias coverage or geometry is merged. Exact option mismatch rejects before packet pixels. This removes per-stroke renderer, clip-reference, color-conversion, and compositor setup while retaining canonical fallback, format 24, the 256-command cancellation bound, the 96 MiB program cap, and normal-page canonical routing. |
| r25-9-0139 | `0139-veloce-exact-ordered-line-raster-tape.patch` | Feeds the already-proven exact four-vertex opaque butt-line polygon directly into AGG's rasterizer instead of writing and rereading five temporary `path_storage` commands per line. Vertex arithmetic, float transform order, commands, clipping, antialiasing, one raster pass, immediate source-over compositing, painter order, and generic fallback remain unchanged. The existing 256-command ordered packet is the bounded tape, with no new allocation, cache, classifier, thread, lock, JNI/Kotlin path, or UI-thread work. |
| r25-10-0140 | `0140-veloce-direct-stream-acquisition-sink.patch` | Extends accepted 0138 and excludes unaccepted 0139. The existing transactional exact line tokenizer now pulls directly into the authoritative RenderProgram builder one 16-byte command at a time. Payload/no-op ownership, native runs, aggregate bounds, and 32-command spatial blocks are committed in the same source-order pass. This removes two fixed 256-entry parser staging arrays and the builder's later batch rescan without changing eligibility, format 24, command ordinals, pixels, replay, the 256-command work bound, the 3M-line ceiling, or the 96 MiB cap. A failed reader or builder commit rewinds to the first uncommitted PDF operator and resumes canonical PDFium. No classifier, threshold, persistent allocation, cache, thread, lock, JNI/Kotlin path, or UI-thread work is added. |
| r25-19-0149 | `0149-veloce-early-immutable-form-tape.patch` | Restarts from accepted 0138 and excludes 0139-0148. An exact all-native, resource-independent Form program is adopted from the existing bounded document cache before constructing the content parser. Exact cached bounds and alpha metadata preserve parent geometry; zero retained children are legal only when every command is native and no mandatory canonical range exists. Enumeration, editing, mutation fallback, or failed native replay reconstructs canonical children once from the original stream and saved invocation state. The existing 16-entry/96-MiB cache, painter order, cancellation, canonical fallback, threads, and normal-page policy remain unchanged. |
...
blind numeric globbing. For `r25-4-0129`, apply the r25 rendering base through
0031, then only 0051, 0075, 0076, 0092 through 0123, and 0125 through 0129. Do
not apply rejected 0124:

```bash
git apply patches/ship/01-jpeg-downscale-on-decode.patch
# Apply the documented r25 sequence through 0031, skipping deprecated 0027/0028.
...
git apply patches/ship/0128-veloce-render-program-success-lazy-sidecar.patch
git apply patches/ship/0129-veloce-render-program-proportional-line-storage.patch
```

Do not apply `0053..0074`, `0077..0091`, or an invented 0084 to an
`r25-4-0129` build. The machine-checked list lives in the 0129 workflow.
...
Patch 0080 depends on 0079 and keeps execution disabled. It updates one fixed
counter while 0076 already appends each command-kind byte, providing exact
O(1) total, per-kind, and single-kind queries without a second holder scan.
The command index remains the live holder object index, so graphics state and
...
Patch 0085 depends directly on 0083; revision 0084 was not emitted. It keeps
...
Patch 0088 depends on 0087 and attributes the remaining cost without changing
rendering. Patch 0089 depends on 0088 and addresses the measured `11.pdf`
Darken fallback inside the unified executor. It does not re-enable the legacy
path-display-list owner or its BGRA group buffers. Every eligible path is still
...
compile-time clip-free predicate. Device measurements later proved the AGG
driver still rejected every resulting clip mask, so 0091 is the final
generation-1 experiment rather than a completed performance mechanism.

Patch 0092 starts generation 2 directly from 0076 and excludes 0079-0091. It
defines a versioned packed bytecode whose initial opcodes are canonical ordered
barriers. The fixed per-kind summary is populated during the same parser append
and the holder remains the lifetime owner. Native command count is zero and the
